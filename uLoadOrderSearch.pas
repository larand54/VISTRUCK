...
uses
  ...
  Dialogs, DB, StdCtrls, ExtCtrls, ActnList, FMTBcd, DBClient, Provider, SqlExpr,
...
type
  TfLoadOrderSearch = class(TForm)
    ...
    cds_SearchOrderLeverantr: TStringField;
    ...
    Label5: TLabel;
    Label6: TLabel;
    EditProduktMarkning: TEdit;
    cds_SearchOrderAvropsreferens: TStringField;
    ...
    procedure EditAvropsRefEnter(Sender: TObject);
    procedure EditAvropsRefExit(Sender: TObject);
    procedure EditProduktMarkningExit(Sender: TObject);
    procedure EditProduktMarkningEnter(Sender: TObject);
    procedure EditAvropsRefKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EditProduktMarkningKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
    SelectOK  : Boolean ;
    FLONo     : Integer;

    procedure BuildSQL(const LO, LoadNo : Integer;const CDS_REF, SlutKund, AvropsRef, ProduktMarkning : String) ;
  public
...
procedure TfLoadOrderSearch.EditAvropsRefEnter(Sender: TObject);
begin
 EditAvropsRef.Color:= clYellow ;
end;

procedure TfLoadOrderSearch.EditAvropsRefExit(Sender: TObject);
begin
 EditAvropsRef.Color:= clWindow ;
end;

procedure TfLoadOrderSearch.EditAvropsRefKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key <> VK_RETURN then Exit;
  if Length(Trim(EditAvropsRef.Text)) > 0 then
  BuildSQL(-1, -1, '', '', EditAvropsRef.Text, '') ;
end;

procedure TfLoadOrderSearch.EditProduktMarkningEnter(Sender: TObject);
begin
 EditProduktMarkning.Color:= clYellow ;
end;

procedure TfLoadOrderSearch.EditProduktMarkningExit(Sender: TObject);
begin
 EditProduktMarkning.Color:= clWindow ;
end;

procedure TfLoadOrderSearch.EditProduktMarkningKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if Key <> VK_RETURN then Exit;
  if Length(Trim(EditProduktMarkning.Text)) > 0 then
  BuildSQL(-1, -1, '', '', '', EditProduktMarkning.Text) ;
end;
...
procedure TfLoadOrderSearch.BuildSQL(const LO, LoadNo : Integer;const CDS_REF, SlutKund, AvropsRef, ProduktMarkning : String) ;
...
begin
 pCDS_REF       := '%' + CDS_REF + '%' ;
 pSlutKund      := '%' + SlutKund + '%' ;
 pAvropsRef       := '%' + AvropsRef + '%' ;
 pProduktMarkning       := '%' + ProduktMarkning + '%' ;
 SelectOK       := False ;
...
//  grdSearchOrderDBTableView1ALO.Visible        := False ;
  ...
  begin
    ...
    SQL.Add('csh.Reference AS Avropsreferens, ssp.Reference AS Produktmarkning') ;
    ...
    //Address kund referens
    if Length(Trim(CDS_REF)) > 0 then
    sql.Add('Where csd.Reference Like ' + QuotedStr(pCDS_REF)) ;

    if Length(Trim(AvropsRef)) > 0 then
    sql.Add('Where csh.Reference Like ' + QuotedStr(pAvropsRef)) ;

    if Length(Trim(ProduktMarkning)) > 0 then
    sql.Add('Where ssp.Reference Like ' + QuotedStr(pProduktMarkning)) ;
