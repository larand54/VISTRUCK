...
//    FOrderClientNo, FShipping, FShipToInvPointNo, FLoadingLocationNo : Integer ;
 ...
 var  fLoadEntryCSD: TfLoadEntryCSD;
...
uses VidaConst, dlgPickPkg,
    ...
    uScanLoadPkgNo, uEntryField, dmsDataConn, dmcLoadEntryCSD, uSelectLoadPlanDest ,
    dmsVidaProduct, UnitPackageEntry, VidaUtils ,
  uPickVPPkgs;
...
constructor TfLoadEntryCSD.CreateWithExistingLoad(AOwner : TComponent;const CustomerNo, LoadNo, SalesRegionNo : Integer);
var
  Save_Cursor : TCursor;
  x           : Integer ;
...
begin
 ...
 Try
  ...
  dmLoadEntryCSD.Get_LO_LinesMatched (dmLoadEntryCSD.mtLoadpackagesPackageNo.AsInteger,
...
constructor TfLoadEntryCSD.CreateWithNewLoad(AOwner: TComponent; const CustomerNo, SalesRegionNo, LoadNo, LO_NO : Integer);
Var x : Integer ;
begin
...
function TfLoadEntryCSD.AfterAddedPkgNo(Sender: TObject;
const PkgNo : Integer;
...
function TfLoadEntryCSD.IdentifyPackageSupplier(
...
begin
 //check that package is available in inventory and Get supplier code
    PkgSupplierCode := dmLoadEntryCSD.PkgNoToSuppCode(PkgNo,
    SupplierNo,
...
function TfLoadEntryCSD.AddLoadDetailMatchByProductNo(const PkgNo : Integer; const SUPP_CODE : String3;const ProductNo, ProductLengthNo, NoOfLengths : Integer) : Integer ;
begin
...
function TfLoadEntryCSD.ValidatePkg(const PkgNo : Integer;const PkgSupplierCode : String3;
const ProductNo, ProductLengthNo, NoOfLengths  : Integer) : Integer ;
var
  CustCdsNo             : Integer;
  ValidPackage          : Integer;
...
Begin
  ...
  if dmLoadEntryCSD.cds_LoadPackagesPkg_State.AsInteger =  NEW_PACKAGE then
   ...
   Begin
    ...
    dmLoadEntryCSD.cds_LoadPackagesPkg_Function.AsInteger := REMOVE_PKG_FROM_LOAD ;
    dmLoadEntryCSD.cds_LoadPackagesChanged.AsInteger      := 1 ;
    dmLoadEntryCSD.cds_LoadPackages.Post ;
   End ;

  Finally
   dmLoadEntryCSD.ds_LoadPackages2.OnDataChange:= dmLoadEntryCSD.ds_LoadPackages2DataChange ;
   Screen.Cursor := Save_Cursor;  { Always restore to normal }
  End ;
end;

Procedure TfLoadEntryCSD.GetLO_Records_AfterAddingLO_Number(const LO_Number : Integer) ;
 Var x : integer ;
 Begin
   With dmLoadEntryCSD do
   Begin
...
procedure TfLoadEntryCSD.ResolvePkgNoAmbiguity(
  Sender: TObject;
  ADataSource : TDataSource;
  var Choice : String3;
  var SupplierNo : Integer;
...
procedure TfLoadEntryCSD.FormCloseQuery(Sender: TObject;
...
function TfLoadEntryCSD.ValidatePkgInSerie(Sender: TObject; const PkgNo: Integer) : TEditAction ;
var
...
procedure TfLoadEntryCSD.AddLONumberOnOpenTheForm(const LO_NO, SalesRegionNo : Integer);
Var
  Save_Cursor  : TCursor;
begin
 With dmLoadEntryCSD do
...
function TfLoadEntryCSD.ValidatePackage_ver2(
  Var CustcdsNo   : Integer;
  PkgSupplier : string3;
  PkgNo       : Integer;
//  var SuppShipPlanObjectNo      : Integer;
...
begin
 ...
 Try
  Try
  dmsSystem.sq_PkgType_InvoiceByCSD.ParamByName('LoadNo').AsInteger:= dmLoadEntryCSD.cds_LoadHeadLoadNo.AsInteger ;
...
//      ShowMessage(E.Message);
 ...
 Begin
  FormCRViewReport.report.ParameterFields.Item[1].AddCurrentValue(dmLoadEntryCSD.cds_LoadHeadLoadNo.AsInteger);
 ...
 End ;
  Try
  dmsSystem.sq_DelPkgType.ParamByName('LoadNo').AsInteger:= dmLoadEntryCSD.cds_LoadHeadLoadNo.AsInteger ;
...
procedure TfLoadEntryCSD.ValidatePkgNoSuppCode(Sender: TObject; const PkgNo: Integer;
PkgSupplierCode: String3; const ProductNo, ProductLengthNo : Integer);
var
//  PkgSupplierNo: Integer ;
  Action: TEditAction;
...
begin
  ...
  Begin
   AddPkgTo_cds_LoadPackages(Sender, PkgNo,PkgSupplierCode) ;
   if dmLoadEntryCSD.cds_LoadPackages.FindKey([PkgNo, Trim(PkgSuppliercode)]) then
...
function TfLoadEntryCSD.IsPkgReserved(
  const PkgNo : Integer;
  var PkgSupplierCode: String3;
  Var Res_UserName : String) : TEditAction;
const
...
procedure TfLoadEntryCSD.ScanningEgnaPkgNo(Sender: TObject; PkgNo, EgenPkgSupplierCode : String) ;
const
  LF = #10;
var
  NewPkgNo : Integer ;
...
procedure TfLoadEntryCSD.InsertScannedPkgNo(Sender : TObject;const PkgNo : Integer;const PkgSupplierCode : String) ;
Var
  CustcdsNo,
  LO_Number,
  OverrideRL : Integer ;
 ...
 Try
  ...
  Begin
   ...
   Try
    ...
    dmLoadEntryCSD.cds_LoadPackagesSupplierCode.AsString, dmLoadEntryCSD.cds_LoadPackagesProductNo.AsInteger,
     dmLoadEntryCSD.cds_LoadPackagesProductLengthNo.AsInteger, dmLoadEntryCSD.cds_LoadPackagesNoOfLengths.AsInteger) ;
...
procedure TfLoadEntryCSD.acInsertAllPkgsToInventoryExecute(
  Sender: TObject);
Var Save_Cursor : TCursor ;
Begin
 Save_Cursor := Screen.Cursor;
...
//Undo
 ...
 Case dmLoadEntryCSD.cds_LoadPackagesPkg_Function.AsInteger of
  ...
  Begin
   dmLoadEntryCSD.cds_LoadPackages.Edit ;
   dmLoadEntryCSD.cds_LoadPackagesPkg_Function.AsInteger  := ADD_PKG_TO_LOAD ;
...
procedure TfLoadEntryCSD.acSetDefaultMatchOnAllPkgsExecute(
  Sender: TObject);
var
 Save_Cursor  : TCursor;
 CustcdsNo    : Integer ;
...
begin
 ...
 Try
  ...
  frmPkgInfo.SupplierCode:= dmLoadEntryCSD.cds_LoadPackagesSupplierCode.AsString ;
...
begin
 acValidatePkg.Enabled:= (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;

procedure TfLoadEntryCSD.acValidateAllPkgsUpdate(Sender: TObject);
begin
 acValidateAllPkgs.Enabled:= (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;

procedure TfLoadEntryCSD.acInsertPkgToInventoryUpdate(Sender: TObject);
begin
 acInsertPkgToInventory.Enabled:=  (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;

procedure TfLoadEntryCSD.acInsertAllPkgsToInventoryUpdate(Sender: TObject);
begin
 acInsertAllPkgsToInventory.Enabled:=  (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;

procedure TfLoadEntryCSD.acRemovePkgFromSystemUpdate(Sender: TObject);
begin
 acRemovePkgFromSystem.Enabled:=  (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;

procedure TfLoadEntryCSD.acRemoveAllPkgsFromSystemUpdate(Sender: TObject);
begin
 acRemoveAllPkgsFromSystem.Enabled:=  (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;

procedure TfLoadEntryCSD.acUndoPkgOperationUpdate(Sender: TObject);
begin
 acUndoPkgOperation.Enabled:=  (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;

procedure TfLoadEntryCSD.acSetDefaultMatchOnAllPkgsUpdate(Sender: TObject);
begin
 acSetDefaultMatchOnAllPkgs.Enabled:=  (dmLoadEntryCSD.cds_LoadPackages.RecordCount > 0) and (LoadEnabled) ;
end;
...
//   dmLoadEntryCSD.LogicalTransferInventoryNo:=    dmLoadEntryCSD.cds_LoadHeadPIPNo.AsInteger ;
  ...
  End //if SQLTimeStampToDateTime(cds_LoadHeadLoadedDate.AsSQLTimeStamp) < Date then
    ...
    ShowMessage('Utlastningsdatum får inte vara större än dagen datum, var vänlig justera.') ;
...
procedure TfLoadEntryCSD.acKopplaPaketMotLOManuelltExecute(
  Sender: TObject);
begin
 With dmLoadEntryCSD do
 Begin
...
begin
 acDeleteLoad.Enabled:= (dmLoadEntryCSD.cds_LoadPackages.Active) and (dmLoadEntryCSD.cds_LoadPackages.RecordCount = 0) ;
end;
...
begin
 ...
 if FormCRViewReport.ReportFound then
 Begin
  FormCRViewReport.report.ParameterFields.Item[1].AddCurrentValue(dmLoadEntryCSD.cds_LSPShippingPlanNo.AsInteger);
//  FormCRViewReport.report.ParameterFields.Item[2].AddCurrentValue(dmLoadEntryCSD.ShippingPlanSupplierNo.AsInteger);
  FormCRViewReport.CRViewer91.ReportSource:= FormCRViewReport.Report ;
...
procedure TfLoadEntryCSD.Get_LO_LookUp_SQL (const LO_NO, SalesRegionNo, CustomerNo : Integer) ;
Begin
...
//  mtLoadPackages.DisableControls ;
  ...
  Begin
   Application.ProcessMessages ;
   ShowMessage('Du har valt Lastplannr : ' + cds_LoadPlanDestLoadingNo.AsString
   +  ' || DestinationsNr : ' + cds_LoadPlanDestLoadPlanDestRowNo.AsString);
   ...
   Begin
    ...
    Begin
     ...
     Begin
      if MessageDlg('Du frågar efter '+Trim(sq_GetLoadPlanPkgsNoOfPkgsLoaded.AsString)+' paket ' + sq_GetLoadPlanPkgsPackageCodeNo.AsString +  ' med det finns endast '+inttostr(noofpkgs)
      +' i lager. Vill du lägga de till lasten?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      InsertPkgNo(Sender) ;
     End
     else
     InsertPkgNo(Sender) ;
...
procedure TfLoadEntryCSD.ValidatePkgNoSuppCode_WhenPickPkgNo(Sender: TObject; const PkgNo: Integer;
PkgSupplierCode: String3; const ProductNo, ProductLengthNo : Integer);
var
//  PkgSupplierNo: Integer ;
  Action: TEditAction;
...
// if dmLoadEntryCSD.FAvrop_CustomerNo <> -1 then
 ...
 Begin //Purchase packages get external supplier SupplierCode
  dmsProduct.SupplierNo     := dmLoadEntryCSD.cds_LSPCustomerNo.AsInteger ;//FOrderClientNo ;//dmLoadEntryCSD.FAvrop_CustomerNo ;
  dmsProduct.SupplierCode   := //dmsContact.GetSupplierCode(dmLoadEntryCSD.mtLoadShippingPlanLoadingLocationNo.AsInteger) ;
  dmsContact.GetClientCode(dmLoadEntryCSD.cds_LSPCustomerNo.AsInteger) ;//dmLoadEntryCSD.FAvrop_CustomerNo) ;

  dmsProduct.OrderType          := c_Purchase ;
  dmsProduct.InventoryOwnerNo   := dmLoadEntryCSD.cds_LSPSalesRegionNo.AsInteger ;//dmLoadEntryCSD.FCustomerNo ;//dmLoadEntryCSD.FSupplierNo ;

  if dmLoadEntryCSD.cds_LoadHeadPIPNo.AsInteger > 0 then
  dmsProduct.InventoryNo        :=   dmLoadEntryCSD.cds_LoadHeadPIPNo.AsInteger ;
...
//  dmsProduct.InventoryNo      := dmLoadEntryCSD.cds_LSPShipToInvPointNo.AsInteger ;
  dmsProduct.SupplierNo       := dmLoadEntryCSD.cds_LSPSalesRegionNo.AsInteger ;//dmLoadEntryCSD.FSupplierNo ;
  dmsProduct.SupplierCode     := dmsContact.GetSupplierCode(dmLoadEntryCSD.cds_LSPLoadingLocationNo.AsInteger) ;

  dmsProduct.InventoryOwnerNo := dmLoadEntryCSD.cds_LSPSalesRegionNo.AsInteger ;//dmLoadEntryCSD.FSupplierNo ;
 ...
 Begin
  ...
  Try
  For x := 0 to 27 do
    dmsProduct.mtLO_Records.Fields.Fields[x].AsVariant  := dmLoadEntryCSD.cdsLORows.Fields.Fields[x].AsVariant ;
    dmsProduct.mtLO_RecordsMATCH.AsInteger              := 0 ;
    dmsProduct.mtLO_RecordsINCH_THICK.AsFloat           := StrToFloatDef(dmLoadEntryCSD.cdsLORowsINCH_THICK.AsString,0) ;
    dmsProduct.mtLO_RecordsINCH_WIDTH.AsFloat           := StrToFloatDef(dmLoadEntryCSD.cdsLORowsINVCH_WIDTH.AsString,0) ;
    dmsProduct.mtLO_RecordsOverrideRL.AsInteger         := dmLoadEntryCSD.cdsLORowsOverrideRL.AsInteger ;
    dmsProduct.mtLO_RecordsVaruSlag.AsInteger           := dmLoadEntryCSD.cdsLORowsVaruSlag.AsInteger ;
...
begin
 ...
 Begin
  ...
  End
    ...
    Begin
     ShowMessage('Lasten kan inte sättas till preliminär för den är fakturerad.');
     SetLoadEnabled ;
    End ;
 End ;
end;
