...
type
 TAmbiguityEvent = procedure(
  ...
  TdmLoadEntryCSDTemp = class(TDataModule)
  ...
  public
   ...
   procedure GetDefault_cds_LoadDelAdress_Values ;
...
var
  dmLoadEntryCSDTemp: TdmLoadEntryCSDTemp;
...
Procedure TdmLoadEntryCSDTemp.Get_LO_LinesMatched (const PackageNo : Integer;const Supp_Code : String3) ;
 Begin
...
//     cdsLORows.Next ;
    ...
    End ;
      ...
      if cdsLORows.FindKey([cds_LoadPackagesDefaultCustShipObjectNo.AsInteger]) then
      Begin
       cdsLORows.Edit ;
...
function TdmLoadEntryCSDTemp.CheckIfLoadNoIsOK(const LoadNo : Integer) : Boolean ;
Begin
...
procedure TdmLoadEntryCSDTemp.SaveLOData(LoadNo: Integer);
...
function TdmLoadEntryCSDTemp.IS_Load_OK : Word ;
Begin
 Result := 2 ;
 ...
 Begin
  ...
  AND (dmLoadEntryCSDTemp.cds_LoadPackagesPkg_Function.AsInteger <> REMOVE_PKG_FROM_LOAD))
  ...
  AND (dmLoadEntryCSDTemp.cds_LoadPackagesPkg_Function.AsInteger <> REMOVE_PKG_FROM_LOAD)) then
   Result := 1 ;
  cds_LoadPackages.Next ;
 End ;
End ;

function TdmLoadEntryCSDTemp.SaveLoadHeader(const WhenPosted : TDateTime;const LoadNo: Integer) : Boolean ;
const
  ALWAYS_ZERO = 0;
begin
...
procedure TdmLoadEntryCSDTemp.ModifyLoadHeader(const WhenPosted : TDateTime;const LoadNo:Integer);
const
  ALWAYS_ZERO = 0;
begin
...
procedure TdmLoadEntryCSDTemp.SaveLoadPkgs(const WhenPosted : TDateTime;const LoadNo:Integer);
var
  Save_Cursor : TCursor;
begin
 Save_Cursor := Screen.Cursor;
...
function TdmLoadEntryCSDTemp.PkgNoToSuppCode(
...
procedure TdmLoadEntryCSDTemp.getPkgsByInvOwner(const PkgNo, InventoryOwner, PIPNo : Integer);
begin
...
procedure TdmLoadEntryCSDTemp.DataModuleCreate(Sender: TObject);
begin
 GlobalLoadDetailNo := 1 ;
end;

function TdmLoadEntryCSDTemp.DeletePackage(LoadNo : Integer)   : Boolean ;
begin
 Result := False ;
 //Delete Package
 Try
...
procedure TdmLoadEntryCSDTemp.RemovePkgFromLoad(const Status, Operation : Integer) ;
Begin
      Try 
...
procedure TdmLoadEntryCSDTemp.ProcessPkgAND_Log(const Status, Operation : Integer) ;
Begin
...
//Make entry to PackageNumberLog
      ...
      sp_ProcessPkgAND_Log.ParamByName('@RegistrationPointNo').AsInteger     := cds_LoadHeadLoadNo.AsInteger ; // ingen mätpunkt utan visa LastNr i loggen.
...
procedure TdmLoadEntryCSDTemp.DeleteONELoad(const LoadNo : Integer);
var
  Save_Cursor   : TCursor;
begin
  Save_Cursor := Screen.Cursor;
...
function TdmLoadEntryCSDTemp.Is_Load_Confirmed (const LoadNo : Integer) : Boolean ;
Begin
  Result:= False ;
...
function TdmLoadEntryCSDTemp.IS_Packages_OK : Boolean ;
begin
...
function TdmLoadEntryCSDTemp.PkgExistInLoad : Boolean ;
Begin
...
function TdmLoadEntryCSDTemp.GetPkgsNos(const packagecodeno : String;const noofpkgs, LogicalInventoryPointNo : Integer) : Integer ;
Var x : Integer ;
Begin
 x := 0 ;
...
Function TdmLoadEntryCSDTemp.DoesLOHavePackages (const LONo : Integer) : Boolean ;
Begin
...
procedure TdmLoadEntryCSDTemp.ds_LoadPackages2DataChange(Sender: TObject;
  Field: TField);
begin
 dmLoadEntryCSDTemp.Get_LO_LinesMatched (cds_LoadpackagesPackageNo.AsInteger, Trim(cds_LoadPackagesSupplierCode.AsString)) ;
end;

procedure TdmLoadEntryCSDTemp.dspLORowsGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: String);
begin
 TableName:= 'LoadShippingPlan' ;
end;

procedure TdmLoadEntryCSDTemp.dsp_LoadPackagesGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: String);
begin
 TableName:= 'LoadDetail' ;
end;

procedure TdmLoadEntryCSDTemp.ds_LSPDataChange(Sender: TObject; Field: TField);
begin
 With dmsContact do
 Begin
...
procedure TdmLoadEntryCSDTemp.cds_LoadPackages1PostError(DataSet: TDataSet;
  E: EDatabaseError; var Action: TDataAction);
begin
...
procedure TdmLoadEntryCSDTemp.dsp_LSPGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: String);
begin
 TableName:= 'LoadShippingPlan' ;
end;

Function TdmLoadEntryCSDTemp.IsLoadMadeInAvrop(const LoadNo : Integer) : Boolean ;
Begin
 ...
 Begin
  if sq_IsLoadMadeInAvropConfirmed_LoadNo.AsInteger <> sq_IsLoadMadeInAvropNewLoadNo.AsInteger then
  Result  := False
   else
    Result  := True ;
 End
...
procedure TdmLoadEntryCSDTemp.cds_LoadHeadAfterInsert(DataSet: TDataSet);
begin
...
procedure TdmLoadEntryCSDTemp.cds_LSPAfterInsert(DataSet: TDataSet);
begin
...
procedure TdmLoadEntryCSDTemp.cds_LoadHeadSenderLoadStatusChange(
  Sender: TField);
begin
...
procedure TdmLoadEntryCSDTemp.cds_LoadPackagesAfterInsert(DataSet: TDataSet);
begin
...
procedure TdmLoadEntryCSDTemp.cds_LoadPackagesBeforePost(DataSet: TDataSet);
begin
...
procedure TdmLoadEntryCSDTemp.cds_LoadHeadPIPNoChange(Sender: TField);
begin
...
procedure TdmLoadEntryCSDTemp.csdUnit_OpenLagerLookup ;
Begin
  With dmLoadEntryCSDTemp do
  Begin
...
procedure TdmLoadEntryCSDTemp.cds_LoadHeadSupplierNoChange(Sender: TField);
begin
...
procedure TdmLoadEntryCSDTemp.cds_LoadHeadAfterScroll(DataSet: TDataSet);
begin
 cds_DepartAdress.Active := False ;
 cds_DepartAdress.ParamByName('ClientNo').AsInteger  := cds_LoadHeadSupplierNo.AsInteger ;
 cds_DepartAdress.Active := True ;
...
procedure TdmLoadEntryCSDTemp.cds_LoadDelAdressAfterInsert(
  DataSet: TDataSet);
begin
 cds_LoadDelAdressLoadNo.AsInteger  := cds_LoadHeadLoadNo.AsInteger ;
end;

procedure TdmLoadEntryCSDTemp.cds_LoadHeadCustomerNoChange(Sender: TField);
begin
 cds_DelAdress.Active := False ;
 cds_DelAdress.ParamByName('ClientNo').AsInteger  := cds_LoadHeadCustomerNo.AsInteger ;
 cds_DelAdress.Active := True ;
...
procedure TdmLoadEntryCSDTemp.GetDefault_cds_LoadDelAdress_Values ;
Begin
 if cds_LoadDelAdress.State in [dsBrowse] then
 cds_LoadDelAdress.Edit ;


 sq_GetCSHValues.Active := False ;
 Try
 sq_GetCSHValues.ParamByName('SHIPPINGPLANNO').AsInteger  := cds_LSPShippingPlanNo.AsInteger ;
 sq_GetCSHValues.Active := True ;
 cds_LoadDelAdressEndUserRequisition.AsString := sq_GetCSHValuesReference.AsString ;
 cds_LoadDelAdressNoteLine2.AsString          := sq_GetCSHValuesNoteForLoadSheet.AsString ;
 Finally
  sq_GetCSHValues.Active := False ;
 End ;
End ;
