...
//    procedure Build_LegoLoads_SQL(Sender: TObject);
    ...
    function  InsertTo_cdsPaymentLoads(const TypeOf, PaymentNo : Integer) : Boolean ;
...

{
  ...
  cdsArrivingLoads.SQL.Add('Left Outer join dbo.Users	US	ON	US.UserID = cl.CreatedUser') ;
  ...
  cdsArrivingLoads.SQL.Add('L.LoadedDate > '+QuotedStr('2009-12-01 00:00:00.000')) ;
...
begin
 ...
 Begin
  ...
  cdsArrivingLoads.SQL.Add('INNER JOIN dbo.Client Cust			ON	Cust.ClientNo 		= SSP.CustomerNo') ;
  cdsArrivingLoads.SQL.Add(' WHERE') ;
  cdsArrivingLoads.SQL.Add('L.LoadedDate > '+QuotedStr('2006-12-31 00:00:00.000')) ;
//if Inlaster köp then
  if cds_PropsVerkNo.AsInteger > 0 then
  cdsArrivingLoads.SQL.Add('AND SSP.CustomerNo = ' + cds_PropsVerkNo.AsString) ; // to get list of loads to pay
  cdsArrivingLoads.SQL.Add('AND L.SenderLoadStatus = 2') ; // to get list of loads to pay
...
function TfrmSkapaAvrakning.ChangeInventoryLocationInPackages(Sender : TObject) : Boolean ;
Var
...
function TfrmSkapaAvrakning.GetNewCustomersInventoryPoint (Sender : TObject) : Integer ;
...
Begin
 ...
 Begin
  cds_SSP.Close ;
  cds_SSP.ParamByName('CustomerNo').AsInteger:= cdsArrivingLoadsCustomerNo.AsInteger ;
  cds_SSP.Active:= True ;
  cds_Update_SSP.Close ;
  cds_Update_SSP.ParamByName('LoadNo').AsInteger     := fLoadNo ;
  cds_Update_SSP.ParamByName('CustomerNo').AsInteger := cdsArrivingLoadsCustomerNo.AsInteger ;
...
function TfrmSkapaAvrakning.SSP_Exist(Var SupplierShipPlanObjectNo: Integer) : Boolean ;
Begin
 With dm_Avrakning do
 Begin
...
begin
 ...
 Try
  frmPkgInfo.PackageNo    := dm_Avrakning.cdsArrivingPackagesPACKAGE_NO.AsInteger ;
  frmPkgInfo.SupplierCode := dm_Avrakning.cdsArrivingPackagesSupplierCode.AsString ;
...
begin
 ...
 if dmsSystem.DeleteAssigned('TfrmSkapaAvrakning', 'dmArrivingLoads') = True then
 Begin
...
//9168
  ...
  Begin
   if MessageDlg('Kunden kommer att ändras från '+cdsArrivingLoadsINT_CUSTOMER.AsString+' till ' + lcVerk.Text + ' i denna last, fortsätta?',
    mtConfirmation, [mbYes, mbNo], 0) = mrNo then
    Exit ;
  End ;
...
function TfrmSkapaAvrakning.CustomerOnLoadsAreNotDuplicates(Sender: TObject) : Boolean ;
 ...
 Screen.Cursor  := crSQLWait;    { Show hourglass cursor }
  ...
  Try
   ...
   Begin
    ...
    if (I <> 0) and (IntCustomer <> ADataSet.FieldByName('INT_CUSTOMER').AsString) then
    Result:= True ;
   End ;
 ...
 Screen.Cursor  := crSQLWait;    { Show hourglass cursor }
  ...
  Try
   ...
   Begin
    ...
    if (I <> 0) and (Kredit <> ADataSet.FieldByName('Krediterad').AsInteger) then
    Result:= False ;
   End ;
...
5)Alla inlaster som saknar pris}
    ...
    if (cds_PropsStatus.AsInteger = 0) or (cds_PropsStatus.AsInteger = 1) or (cds_PropsStatus.AsInteger = 3) or (cds_PropsStatus.AsInteger = 4) then
    dmArrivingLoads.GetIntPrice(cdsArrivingLoadsSUPPLIER_NO.AsInteger, 0,-1, RecID, True)
    else
    dmArrivingLoads.GetIntPrice(cdsArrivingLoadsCUSTOMERNO.AsInteger, 0,-1, RecID, True) ;

//    dmArrivingLoads.GetIntPrice(fSupplierNo, 0,-1, RecID, True) ;

    if (cds_PropsStatus.AsInteger = 0) or (cds_PropsStatus.AsInteger = 1) or (cds_PropsStatus.AsInteger = 3) or (cds_PropsStatus.AsInteger = 4) then
    Begin
     if dm_Avrakning.priceOk(RecID, cdsArrivingLoadsSUPPLIER_NO.AsInteger) then
     Begin
    ...
    Begin
     if dm_Avrakning.priceOk(RecID, cdsArrivingLoadsCUSTOMERNO.AsInteger) then
     Begin
      if ADataSet.Locate('VIS_FS', RecID,[]) then
      Begin
       ADataSet.Edit ;
...
function TfrmSkapaAvrakning.InsertTo_cdsPaymentLoads(const TypeOf, PaymentNo : Integer) : Boolean ;
 Var  i, RecIDX   : Integer ;
      RecID       : Variant ;
      fRollBack   : Boolean ;
Begin
  ...
  Try
   ...
   Begin
    ...
    Begin
     ...
     if cdsPaymentLoads.ApplyUpdates(0)  > 0 then fRollBack := True
     else
      cdsPaymentLoads.CommitUpdates ;

     if fRollBack = False then
     if cdsLastAvr.ApplyUpdates(0)  > 0 then fRollBack := True
      else
    ...
    Begin
     Try
     sq_UpdLoadDtlValSingleLoad.ParamByName('SetPaymentNo').AsInteger       := PaymentNo ;
     sq_UpdLoadDtlValSingleLoad.ParamByName('PaymentNo').AsInteger          := -1 ;
     sq_UpdLoadDtlValSingleLoad.ParamByName('PaymentType').AsInteger        := 1 ;
     sq_UpdLoadDtlValSingleLoad.ParamByName('LoadNo').AsInteger             := RecID ;
     sq_UpdLoadDtlValSingleLoad.ParamByName('SupplierNo').AsInteger         := cds_PropsVerkNo.AsInteger ;
...
begin
 With dm_Avrakning do
 Begin
  acChangeCustomer.Enabled:=  (ThisUser.UserID in [4,5,7,8])
  and (cdsArrivingLoads.Active)
  and (cdsArrivingLoads.RecordCount > 0)
  and ((cds_PropsStatus.AsInteger <> 4) and (cds_PropsStatus.AsInteger <> 5)) ;
 End ;
end;

procedure TfrmSkapaAvrakning.acCreatePaymentsUpdate(Sender: TObject);
begin
 acCreatePayments.Enabled:=  ((cds_PropsStatus.AsInteger = 2) and (ThisUser.UserID in [4,7,8]))
 or (cds_PropsStatus.AsInteger <> 2)
...
begin
 ...
 Begin
  ...
  End ; 
    ...
    if (cds_PropsStatus.AsInteger = 0) or (cds_PropsStatus.AsInteger = 1) or (cds_PropsStatus.AsInteger = 3) or (cds_PropsStatus.AsInteger = 4) then
    Begin
     if dm_Avrakning.priceOk(cdsArrivingLoadsVIS_FS.AsInteger, cdsArrivingLoadsSUPPLIER_NO.AsInteger) then
     Begin
      if cdsArrivingLoads.Locate('VIS_FS', cdsArrivingLoadsVIS_FS.AsInteger,[]) then
      Begin
    ...
    Begin
     if dm_Avrakning.priceOk(cdsArrivingLoadsVIS_FS.AsInteger, cdsArrivingLoadsCUSTOMERNO.AsInteger) then
     Begin
      if cdsArrivingLoads.Locate('VIS_FS', cdsArrivingLoadsVIS_FS.AsInteger,[]) then
      Begin
       cdsArrivingLoads.Edit ;
...
begin
 ...
 Begin
  ...
  ((cdsArrivingPackages.State in [dsEdit]) or (cdsArrivingPackages.ChangeCount > 0)) ;
 End ;
end;
...
begin
 With dm_Avrakning do
 Begin
  acGetIntPrices.Enabled:= (cdsArrivingPackages.Active)
  and (cdsArrivingPackages.State in [dsBrowse])
  and (cdsArrivingPackages.ChangeCount = 0)
  and (cdsArrivingLoads.Active)
  and (cdsArrivingLoads.RecordCount > 0)
  and (grdLoadsDBTableView1.Controller.SelectedRecordCount > 0) ;
 End ;
...
// dm_Avrakning.cdsArrivingPackages.DisableControls ;
 ...
 Begin
  ShowMessage('En eller flera laster saknar pris för vissa produkter, se röda rader!') ;
  Exit ;
 End ;
...
// else
 if cds_PropsStatus.AsInteger in [0,1,3] then
  cdsPayHead.ParamByName('SupplierNo').AsInteger   := cds_PropsVerkNo.AsInteger
   else
    cdsPayHead.ParamByName('SupplierNo').AsInteger := VIDA_WOOD_COMPANY_NO ;
...
// else
  ...
  if cds_PropsStatus.AsInteger = 2 then
   cdsPayHead.ParamByName('CustomerNo').AsInteger   := cds_PropsVerkNo.AsInteger
    else
    ...
    if cds_PropsStatus.AsInteger = 3 then
     cdsPayHead.ParamByName('CustomerNo').AsInteger:= cdsArrivingLoadsCUSTOMERNO.AsInteger
       ...
       cdsPayHead.ParamByName('CustomerNo').AsInteger:= VIDA_WOOD_COMPANY_NO ;
...
//inte som tidigare att den gick till en öppen kreditavräkning om lasten tidigare krediterats.
 ...
 cdsPayHead.ParamByName('PaymentType').AsInteger  := 1 ; //**cdsArrivingLoadsKrediterad.AsInteger ;
 ...
 Begin
  ...
  if cds_PropsStatus.AsInteger = 2 then
   cdsPayHeadCustomerNo.AsInteger:= cds_PropsVerkNo.AsInteger
    ...
    //är det en försäljning är VW alltid customer om inte INTERN!
    if cds_PropsStatus.AsInteger = 3 then
     cdsPayHeadCustomerNo.AsInteger:= cdsArrivingLoadsCUSTOMERNO.AsInteger
      else
 ...
 End ;//if cdsPayHead...
  ...
  Begin
   Try
    if InsertTo_cdsPaymentLoads(cds_PropsStatus.AsInteger, PaymentNo) = False then
     RemoveLoads ;
...
begin
 ...
 and ((cds_PropsStatus.AsInteger <> 4) and (cds_PropsStatus.AsInteger <> 5))
 and (not cds_PropsVerkNo.isNull)
 and (cds_PropsVerkNo.AsInteger > 0) ;
end;
...
Begin
 ...
 Begin
  ...
  SQL.Add('SSP.SupplierNo AS SSP_SupplierNo, LSP.ShippingPlanNo		AS	LO, LD.PackageNo AS	PACKAGE_NO, LD.SUPPLIERCODE	AS 	SUPPLIERCODE,') ;
  SQL.Add('PTD.m3Actual	AS 	M3_NET, PTD.m3Nominal	AS M3_NOM, PR.ProductDisplayName	AS	PRODUCT_DESCRIPTION,') ;
  SQL.Add('PTD.NoOfPieces		    AS	PCS, LD.PACKAGEOK			    AS	PACKAGEOK, LD.ProblemPackageLog	AS	PACKAGE_LOG,') ;
  ...
  SQL.Add('INNER JOIN dbo.Client OC				ON	OC.ClientNo 		= CSH.CustomerNo') ;
