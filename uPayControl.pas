...
begin
 ...
 Begin
  ...
  if MessageDlg('Ändringar ej sparade.  vill du spara ?',
...
begin
 With dmModule1 do
 Begin
  if (cds_InvoicePayStatus.ChangeCount > 0 ) or (cds_InvoicePayStatus.State in [dsEdit, dsInsert]) then
   Result:= True
    else
     Result:= False ;
 End ;
...
Begin
 ...
 Begin
  ...
  cds_InvoicePayStatus.SQL.Add('IH.QuickInvoice, IH.InternalInvoiceNo AS IH_InternalInvoiceNo,') ;
  cds_InvoicePayStatus.SQL.Add('IH.InvoiceDate AS FAKTURA_DATUM, CR.idXOR AS KUND, IH.AgentName AS AGENT,') ;
  cds_InvoicePayStatus.SQL.Add('IH.PaymentDescription AS BETALVILLKOR,') ;
  cds_InvoicePayStatus.SQL.Add('ROUND(CAST((  IH.Inv_Value_To_Be_Paid_2    ) As decimal(18,2)),2) AS BELOPP,') ;
  cds_InvoicePayStatus.SQL.Add('ROUND(CAST((  IH.Discount    ) As decimal(18,2)),2) AS KASSARABATT,') ;
  cds_InvoicePayStatus.SQL.Add('ROUND(CAST((  IH.Inv_Value_To_Be_Paid_2-IH.Discount    ) As decimal(18,2)),2) AS BELOP_EXKL_KASSA,') ;
  cds_InvoicePayStatus.SQL.Add('isnull(IP.Paid,0) AS Paid, IP.DatePaid, IP.AmountPaid, IP.Note,') ;
  ...
  cds_InvoicePayStatus.SQL.Add('IH.Total_Product_Value_No_Freight AS VARUVARDE_NO_FREIGHT,') ;
  cds_InvoicePayStatus.SQL.Add('IH.Commission AS PROVISION, IH.Trading, ') ;
  cds_InvoicePayStatus.SQL.Add(' (Select SUM(InD.ProductValue) from dbo.InvoiceDetail InD') ;
  cds_InvoicePayStatus.SQL.Add('WHERE') ;
  cds_InvoicePayStatus.SQL.Add('InD.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  ...
  cds_InvoicePayStatus.SQL.Add('Left Outer Join dbo.InvoiceNos inos on inos.InternalInvoiceNo = IH.InternalInvoiceNo') ;

  Case bcInvoiceType.ItemIndex of
   0 : cds_InvoicePayStatus.SQL.Add('Inner join dbo.InvoiceNumber INV on INV.InternalInvoiceNo = IH.InternalInvoiceNo') ;
   1 : cds_InvoicePayStatus.SQL.Add('Inner join dbo.ProformaInvoiceNumber INV on INV.InternalInvoiceNo = IH.InternalInvoiceNo') ;
   2 : cds_InvoicePayStatus.SQL.Add('Inner join dbo.ProformaInvoiceNumber INV on INV.InternalInvoiceNo = IH.InternalInvoiceNo') ;
   3 : cds_InvoicePayStatus.SQL.Add('Inner join dbo.InvoiceNumber_PO INV on INV.InternalInvoiceNo = IH.InternalInvoiceNo') ;
   4 : cds_InvoicePayStatus.SQL.Add('Inner join dbo.InvoiceNo_USA INV on INV.InternalInvoiceNo = IH.InternalInvoiceNo') ;
   5 : cds_InvoicePayStatus.SQL.Add('Inner join dbo.InvNo_FW INV on INV.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  End ;


  cds_InvoicePayStatus.SQL.Add('INNER JOIN  dbo.ClientRole    CR ON CR.ClientNo = IH.CustomerNo') ;
  cds_InvoicePayStatus.SQL.Add('LEFT OUTER JOIN dbo.InvoicePayStatus IP ON IP.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  ...
  if bcFakturaStatus.ItemIndex = 2 then //FULL BETALDA
   cds_InvoicePayStatus.SQL.Add(' AND IH.InvoiceDate >= :StartInvoiceDate') ;
...
//FULL BETALDA
  ...
  Begin
   ...
   cds_InvoicePayStatus.SQL.Add('(Select  InternalInvoiceNo  FROM dbo.InvoicePayStatus WHERE Paid = 1)') ;
  End
  else
  if bcFakturaStatus.ItemIndex = 1 then //ENDAST DEL BETALDA
  Begin
   cds_InvoicePayStatus.SQL.Add('AND IH.InternalInvoiceNo IN') ;
   cds_InvoicePayStatus.SQL.Add('(Select  InternalInvoiceNo  FROM dbo.InvoicePayStatus WHERE Paid = 0 AND AmountPaid > 0)') ;
  End
  else
  if bcFakturaStatus.ItemIndex = 2 then //FULL BETALDA
  Begin
  ...
  if bcFakturaStatus.ItemIndex = 2 then //FULL BETALDA
   cds_InvoicePayStatus.ParamByName('StartInvoiceDate').AsSQLTimeStamp:= DateTimeToSQLTimeStamp(deFOM.Date) ;

  End
  else
   if InvoiceNo > -1 then
   Begin
    cds_InvoicePayStatus.SQL.Add('AND inos.InvoiceNo = ' + IntToStr(InvoiceNo)) ;
   End ;
...
begin
  ...
  with dmModule1 do
  Begin
   OpenInvoice(grdInvPaidDBTableView1.DataController.DataSet.FieldByName('IH_InternalInvoiceNo').AsInteger, -1, -1) ;
  End ;
end;
...
begin
 ...
 With dmModule1 do
 Begin
  acOpenInvoice.Enabled := (grdInvPaidDBTableView1.DataController.DataSet.Active) and (grdInvPaidDBTableView1.DataController.DataSet.RecordCount > 0) ;
 End ;
end;
...
begin
 ...
 With dmModule1 do
 Begin
  acExportXLS.Enabled := (grdInvPaidDBTableView1.DataController.DataSet.Active) and (grdInvPaidDBTableView1.DataController.DataSet.RecordCount > 0) ;
 End ;
end;

procedure TfPayControl.OpenInvoice(const IntInvNo, IntInvNoToAttestAgainst, DelKredit  : Integer) ;
...
begin
 ...
 dmsSystem.LoadGridLayout(ThisUser.UserID, grdInvPaidDBTableView1.Name, grdInvPaidDBTableView1) ;
end;
...
procedure TfPayControl.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
  inherited;
 dmsSystem.StoreGridLayout(ThisUser.UserID, grdInvPaidDBTableView1.Name, grdInvPaidDBTableView1) ;
end;
