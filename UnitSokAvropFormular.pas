...
uses
  ...
  dxCntner, dxEditor, dxExEdtr,
  dxEdLib, StdCtrls, dxTL, dxDBCtrl, dxDBGrid, dxDBTLCl,
  dxGrClms, db, SqlTimSt, Buttons, Menus, cxPC, cxControls,
...
type
  TfrmSokAvropFormular = class(TForm)
    ...
    grdAvropSokDBBandedTableView1PreliminaryRequestedPeriod: TcxGridDBBandedColumn;
    grdAvropSokDBBandedTableView1SHIPPER: TcxGridDBBandedColumn;
    grdAvropSokDBBandedTableView1SHIPPINGCOMPANYBOOKINGID: TcxGridDBBandedColumn;
...
Procedure TfrmSokAvropFormular.CreateCo(Sender: TObject;const CompanyNo: Integer);
var
  Save_Cursor : TCursor;
//  x : Integer ;
begin
...
//  cds_MakeSokAvrop.SQL.Add('WHERE  SPP.ShippingPlanNo = CSH.ShippingPlanNo ) AS NoOfSuppliers') ;
  ...
  cds_MakeSokAvrop.SQL.Add(') AS NoOfSuppliers, CSH.CustomerNo, OH.OrderType, Bk.VoyageNo') ;
...
//   0 :  cds_MakeSokAvrop.SQL.Add('CSH.ShippingPlanStatus <> 9 ') ;
  ...
  cds_MakeSokAvrop.SQL.Add('DT.DeliveryTerm,  ST_ADR.PostalCode, ST_AdrCY.CityName, ST.Reference, BK.PreliminaryRequestedPeriod,');
 ...
 Begin
  ...
  Try
   ...
   Begin
    ...
    cds_Props.ParamByName('Form').AsString          := cds_mallForm.AsString ;// cds_mallName.AsString ;
...
//     dmsSystem.LoadGridLayout_Special(ThisUser.UserID, Self.Name, cds_mallName.AsString + '/' + cds_mallForm.AsString, grdAvropSokDBBandedTableView1) ;
     dmsSystem.LoadGridLayout_Special(ThisUser.UserID, Self.Name, cds_PropsName.AsString + '/' + cds_PropsForm.AsString, grdAvropSokDBBandedTableView1) ;
    End ;
...
begin
 ...
 try
 FormBookingForm.CreateCo(grdAvropSokDBBandedTableView1.DataController.DataSource.DataSet.FieldByName('LO').AsInteger) ;
...
begin
 grdAvropSokDBBandedTableView1.OptionsView.GroupByBox:= not grdAvropSokDBBandedTableView1.OptionsView.GroupByBox ;
end;
...
begin
 grdAvropSokDBBandedTableView1.OptionsView.Footer:= not grdAvropSokDBBandedTableView1.OptionsView.Footer ;
end;

procedure TfrmSokAvropFormular.grdAvropSokDBBandedTableView1StylesGetContentStyle
...
//  if ARecord.Values[AColumn.Index] > 0 then    AStyle := cxStyle1 ;
 ...
 AStyle:= cxStyle1 ;
  ...
  if ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('NoOfSuppliers').Index] <> null then
  Begin
  AColumn := (Sender as TcxGridDBBandedTableView).GetColumnByFieldName('NoOfSuppliers') ;
...
//    dmsSystem.LoadGridLayout_Special(ThisUser.UserID, Self.Name, cds_PropsName.AsString, grdAvropSokDBBandedTableView1) ;
  ...
  End //if (fSokAvropMall.ShowModal = mrOK) and (cds_mall.RecordCount > 0) AND (cds_mallName.AsString > '') then
    ...
    Begin
     ...
     if cds_Props.Active then cds_Props.Active:= False ;
    End ;
 End ;//With dm_SokFormular do
end;
...
begin
 With dm_SokFormular do
 Begin
 MailToAddress:= dmsContact.GetEmailAddressForSpeditorByLO(cds_MakeSokAvropLO.AsInteger) ;
...
// if dmVidaInvoice.cdsInvoiceListINT_INVNO.AsInteger < 1 then exit ;
 ...
 Try
  ...
  FormCRExportOneReport.CreateCo(cds_MakeSokAvropCustomerNo.AsInteger, ReportType, A, ExcelDir + 'LONo ' + cds_MakeSokAvropLO.AsString) ;
...
// Attach[1]        := ExcelDir + 'Specification '+dmVidaInvoice.cdsInvoiceListINVOICE_NO.AsString+'.pdf' ;
 ...
 Try
  dm_SendMapiMail.SendMail('Transportorder. LOnr: ' + cds_MakeSokAvropLO.AsString,
...
procedure TfrmSokAvropFormular.acMailaTrpOrderAvropDKExecute(
  Sender: TObject);
const
  LF = #10;
Var FormCRExportOneReport   : TFormCRExportOneReport ;
...
begin
 acBooking.Enabled:=
 (grdAvropSokDBBandedTableView1.DataController.DataSource.DataSet.Active)
 and (grdAvropSokDBBandedTableView1.DataController.DataSource.DataSet.RecordCount > 0)
 and (grdAvropSokDBBandedTableView1.DataController.DataSource.DataSet.FieldByName('LO').AsInteger > 0) ;
end;
...
//  if (fSokAvropMall.ShowModal = mrOK) and
  if (cds_Props.RecordCount > 0) AND (cds_PropsName.AsString > '') then
  Begin
   if MessageDlg('Vill du spara aktuella inställningar som mall ' + cds_PropsForm.AsString + '?',
   mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  Begin

   dmsSystem.StoreGridLayout_Special(ThisUser.UserID, Self.Name, cds_PropsName.AsString + '/' + cds_PropsForm.AsString, grdAvropSokDBBandedTableView1) ;
...
procedure TfrmSokAvropFormular.acMailaManyTrpOrdersExecute(
  Sender: TObject);
const
  LF = #10;
Var FormCRExportOneReport   : TFormCRExportOneReport ;
...
//  A[0]:= cds_MakeSokAvropLO.AsInteger ;
 ...
 Begin
  A[0]  := AA[I] ;
  S     := AA[I] ; //A[0] ;
  FormCRExportOneReport.CreateCo(cds_MakeSokAvropCustomerNo.AsInteger, ReportType, A, ExcelDir + 'LONo ' + S) ;
  if FormCRExportOneReport.ReportFound = False then Exit ;
...
//Välj alla lastrader
  ...
  Try
   ADataSet := grdAvropSokDBBandedTableView1.DataController.DataSource.DataSet ;
   For I := 0 to grdAvropSokDBBandedTableView1.Controller.SelectedRecordCount - 1 do
   Begin
    RecIDx      := grdAvropSokDBBandedTableView1.Controller.SelectedRecords[i].RecordIndex ;
    ColIdx      := grdAvropSokDBBandedTableView1.DataController.GetItemByFieldName('SHIPPER').Index;
    SHIPPER     := grdAvropSokDBBandedTableView1.DataController.Values[RecIdx, ColIdx];
    if I = 0 then
     OldSHIPPER := SHIPPER ;
    ...
    RecIDx  := grdAvropSokDBBandedTableView1.Controller.SelectedRecords[i].RecordIndex ;
    RecID   := grdAvropSokDBBandedTableView1.DataController.GetRecordId(RecIdx) ;
...
//  if (cds_Props.RecordCount > 0) AND (cds_PropsName.AsString > '') then
  Begin
   if MessageDlg('Vill du spara aktuella inställningar som mall ' + fEntryField.eNoofpkgs.Text + '?',
   mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  Begin
   ...
   Begin
    if not cds_Props.Active then cds_Props.Active  := True ;
    cds_Props.Insert ;
    cds_PropsForm.AsString := fEntryField.eNoofpkgs.Text ;
   End ;
...
procedure TfrmSokAvropFormular.acAngeSomStandardMallExecute(
  Sender: TObject);
begin
 //
end;

procedure TfrmSokAvropFormular.cbShowProductPropertiesChange(
  Sender: TObject);
begin
    if cds_PropsRegPointNo.AsInteger = 1 then
    Begin
