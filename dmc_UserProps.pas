...
uses
  SysUtils, Classes, FMTBcd, DB, DBClient, Provider, SqlExpr, kbmMemTable, SqlTimSt,
...
type
  ...
  public
    ...
    procedure SetAppFormSize (const Form : String;const Height, Width, Top, Left : Integer) ;
    procedure GetAppFormSize (const Form : String;var Height, Width, Top, Left : Integer) ;
  end;
...
//  mtuserpropProducerNo.OnChange  := nil ;
  ...
  mtuserprop.FieldByName('MarketRegionNo').AsInteger    := cds_PropsMarketRegionNo.AsInteger ;
  mtuserprop.FieldByName('OrderTypeNo').AsInteger       := cds_PropsOrderTypeNo.AsInteger ;
  mtuserprop.FieldByName('Status').AsInteger            := cds_PropsStatus.AsInteger ;
  mtuserprop.FieldByName('FilterOrderDate').AsInteger   := cds_PropsFilterOrderDate.AsInteger ;
  mtuserprop.FieldByName('ClientNo').AsInteger          := cds_PropsClientNo.AsInteger ;
  mtuserprop.FieldByName('SalesPersonNo').AsInteger     := cds_PropsSalesPersonNo.AsInteger ;

  mtuserprop.FieldByName('VerkSupplierNo').AsInteger    := cds_PropsVerkSupplierNo.AsInteger ;
  mtuserprop.FieldByName('VerkKundNo').AsInteger        := cds_PropsVerkKundNo.AsInteger ;
  mtuserprop.FieldByName('LOObjectType').AsInteger      := cds_PropsLOObjectType.AsInteger ;
  mtuserprop.FieldByName('AgentNo').AsInteger           := cds_PropsAgentNo.AsInteger ;
  mtuserprop.FieldByName('LoadingLocationNo').AsInteger := cds_PropsLoadingLocationNo.AsInteger ;
  mtuserprop.FieldByName('ShipperNo').AsInteger         := cds_PropsShipperNo.AsInteger ;
  mtuserprop.FieldByName('BookingTypeNo').AsInteger     := cds_PropsBookingTypeNo.AsInteger ;
  mtuserprop.FieldByName('CustomerNo').AsInteger        := cds_PropsCustomerNo.AsInteger ;
  mtuserprop.FieldByName('ShowProduct').AsInteger       := cds_PropsShowProduct.AsInteger ;
...
//  cds_PropsProducerNo.AsInteger ;mtuserprop.FieldByName('RunNo').AsInteger           := -1 ;
  ...
  cds_PropsMarketRegionNo.AsInteger     := mtuserprop.FieldByName('MarketRegionNo').AsInteger ;
  cds_PropsOrderTypeNo.AsInteger        := mtuserprop.FieldByName('OrderTypeNo').AsInteger ;
  cds_PropsStatus.AsInteger             := mtuserprop.FieldByName('Status').AsInteger ;
  cds_PropsFilterOrderDate.AsInteger    := mtuserprop.FieldByName('FilterOrderDate').AsInteger ;
  cds_PropsClientNo.AsInteger           := mtuserprop.FieldByName('ClientNo').AsInteger ;
  cds_PropsSalesPersonNo.AsInteger      := mtuserprop.FieldByName('SalesPersonNo').AsInteger ;

  cds_PropsVerkSupplierNo.AsInteger     := mtuserprop.FieldByName('VerkSupplierNo').AsInteger ;
  cds_PropsVerkKundNo.AsInteger         := mtuserprop.FieldByName('VerkKundNo').AsInteger ;
  cds_PropsLOObjectType.AsInteger       := mtuserprop.FieldByName('LOObjectType').AsInteger ;
  cds_PropsAgentNo.AsInteger            := mtuserprop.FieldByName('AgentNo').AsInteger ;
  cds_PropsLoadingLocationNo.AsInteger  := mtuserprop.FieldByName('LoadingLocationNo').AsInteger ;
...
procedure Tdm_UserProps.GetAppFormSize (const Form : String;var Height, Width, Top, Left : Integer) ;
Begin
...
procedure Tdm_UserProps.SetAppFormSize (const Form : String;const Height, Width, Top, Left : Integer) ;
Begin
 cds_Props.ParamByName('UserID').AsInteger := ThisUser.UserID ;
 cds_Props.ParamByName('Form').AsString    := Form ;
 cds_Props.Active:= True ;
