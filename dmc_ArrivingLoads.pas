...
type
 ...
 TAmbiguityEvent = procedure(
  ...
  private
    ...
    function  GetPriceOfOrderRowForAddLO(const Inkop : Boolean;const LoadedDate : TSQLTimeStamp;const SupplierNo, defsspno, ProductNo, ProductLengthNo : Integer) : Double ;
...
//    procedure LoadUserProps (const Form : String) ;
    ...
    Function  ARINTADDLOLoadToLager(const OldLoadNo, LIPNo : Integer;const ChangeToIMPProduct : Integer) : Boolean ;
...
//Är lasten AR av Extern Kund, i så fall måste den externa kunden ångra AR först!
  ...
  Begin
   ShowMessage('Ångra ankomstregistrering av kundlast först: '
   +LF+' Lastnr: ' + sq_IsEXTLoadConfirmedConfirmed_LoadNo.AsString
   +LF+' LO: ' + sq_IsEXTLoadConfirmedConfirmed_ShippingPlanNo.AsString
   +LF+' Ankomstregistrerad av : ' + sq_IsEXTLoadConfirmedUserName.AsString) ;
   CommitChanges:= False ;
   Exit ;
  End ;
  Finally
...
//Kolla att det finns en confirmed package log fil
  ...
  Begin
   ShowMessage('Det finns ingen loggfil för lastnr '+cdsArrivingLoadsLOADNO.AsString+' kan inte återställas.') ;
   CommitChanges:= False ;
   Exit ;
  End ;
...
function TdmArrivingLoads.GetPriceOfOrderRowForAddLO(const Inkop : Boolean;const LoadedDate : TSQLTimeStamp;const SupplierNo, defsspno, ProductNo, ProductLengthNo : Integer) : Double ;
Var vwCost,
    Price,
    RandomLengthPrice,
    FixedLengthPrice    : Double ;
...
//*********************** Gå igenom alla lastrader ***************************
 ...
 Begin
  if cds_LoadRowPrice.AsFloat < 1 then//om priset finns angivet på LO tar vi det annars körs nedan rutin
  Begin
     ...
     //annat fall om lastorder är av normal typ tas priset från orderrad
      if cds_LoadRowObjectType.AsInteger = 2 then
       Price  := GetPriceOfOrderRow(Inkop, cds_LoadRowLoadedDate.AsSQLTimeStamp, SupplierNo, cds_LoadRowdefsspno.AsInteger, cds_LoadRowProductNo.AsInteger, cds_LoadRowProductLengthNo.AsInteger)
         ...
         if cds_LoadRowObjectType.AsInteger = 1 then
          Price := GetPriceOfOrderRowForAddLO(Inkop, cds_LoadRowLoadedDate.AsSQLTimeStamp, SupplierNo, cds_LoadRowdefsspno.AsInteger, cds_LoadRowProductNo.AsInteger, cds_LoadRowProductLengthNo.AsInteger)
           else
            Price := 0 ;
  End
  else
...
(**  Insert record to LoadDtlVal table **)
  ...
  Begin
   XrateAtDateOfDelivery := GetXrateAtDateOfDelivery(cds_LoadRowLoadNo.AsInteger) ;
...
Function TdmArrivingLoads.GetXrateAtDateOfDelivery(const LoadNo : Integer)  : Double ;
Begin
 Result := 0 ;
...
begin
 ...
 cdsPortArrivingPackages.ParamByName('Reference').AsString        := cdsPortArrivingLoadsMRKNING.AsString ;
...
begin
 Try
 sq_PkgsInLoad.Close ;
 sq_PkgsInLoad.ParamByName('LoadNo').AsInteger          := cdsArrivingLoadsLOADNO.AsInteger ;
 sq_PkgsInLoad.ParamByName('ShippingPlanNo').AsInteger  := cdsArrivingLoadsLO.AsInteger ;
 sq_PkgsInLoad.Open ;
...
Begin
 ...
 sp_CopyLOLoadToSales.ParamByName('@CreateUser').AsInteger             := ThisUser.UserID ;
...
Function TdmArrivingLoads.ARINTADDLOLoadToLager(const OldLoadNo, LIPNo : Integer;const ChangeToIMPProduct : Integer) : Boolean ;
Begin
   //START A TRANSACTION
 dmsConnector.StartTransaction ;
 ...
 sp_CopyARIntAddLOLoad.ParamByName('@CreateUser').AsInteger    := ThisUser.UserID ;
 sp_CopyARIntAddLOLoad.ParamByName('@LIPNo').AsInteger         := LIPNo ;
  ...
  Begin
   ...
   sp_ChangeToIMPProduct.ParamByName('@UserID').AsInteger  := ThisUser.UserID ;
   sp_ChangeToIMPProduct.ExecProc ;
  End
  else
  if ChangeToIMPProduct = 2 then
  Begin
   sp_ChangeToIMPProductForIntLoad.ParamByName('@LoadNo').AsInteger  := OldLoadNo ;
   sp_ChangeToIMPProductForIntLoad.ParamByName('@UserID').AsInteger  := ThisUser.UserID ; ;
...
Function TdmArrivingLoads.ARPOLoadToLager(const OldLoadNo, LIPNo : Integer) : Boolean ;
Begin
   //START A TRANSACTION
 dmsConnector.StartTransaction ;
...
Function TdmArrivingLoads.AR_TRADING_PO_Loads(const OldLoadNo, LIPNo : Integer) : Boolean ;
Begin
   //START A TRANSACTION
 dmsConnector.StartTransaction ;
...
//Är lasten avräknad ?
  ...
  if Not sq_IsLoadAvraknad.Eof then
  Begin
   ShowMessage('Kan inte uppdatera priser här, gå till avräkning och uppdatera där - Avräkningsnr: ' + sq_IsLoadAvraknadPaymentNo.AsString) ;
   Result := True ;
  End ;
  Finally
   sq_IsLoadAvraknad.Close ;
...
function TdmArrivingLoads.AR_ExternLoad(const LoadNo, Status, LIPNo, CreatedUser : Integer) : Boolean ;
Begin
 Result := True ;
 Try
