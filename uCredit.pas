...
uses
  ...
  cxLookAndFeelPainters, cxGridCustomPopupMenu, cxGridPopupMenu ;
...
type
  ...
  private
    ...
    procedure AddNewGroup(Sender: TObject; const ClientNo : Integer; const ClientName : String) ;
...
begin
 dmsSystem.LoadGridLayout(ThisUser.UserID, grdKreditDBTableView1.Name, grdKreditDBTableView1) ;
end;
...
procedure TfrmCredit.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
 CanClose := True ;
 if DataSaved  = False then
  if MessageDlg('Ändringar ej sparade.  Exit now?',
 ...
 Begin
  ...
  End ;
  dmsSystem.StoreGridLayout(ThisUser.UserID, grdKreditDBTableView1.Name, grdKreditDBTableView1) ;
 End ;
end;
...
Begin
 ...
 Begin
  ...
  cds_CreLim.SQL.Add('CI.CreditLimit AS KREDITGRANS, CR.idXOR AS IDXOR, CR.typeXOR,') ;
  cds_CreLim.SQL.Add('CR.RoleType, CI.ClientNo AS CI_ClientNo, C.SearchName AS KORTNAMN,') ;
  ...
  cds_CreLim.SQL.Add('Inner Join dbo.CreditInsurance CI ON CI.ClientNo = C.ClientNo')
  else
  cds_CreLim.SQL.Add('Left Outer Join dbo.CreditInsurance CI ON CI.ClientNo = C.ClientNo') ;

  cds_CreLim.SQL.Add('Inner Join  dbo.ClientRole CR ON CR.ClientNo = C.ClientNo') ;
  cds_CreLim.SQL.Add('Left Outer Join CreditLimitSEK CLS ON CLS.ClientNo = C.ClientNo') ;

  cds_CreLim.SQL.Add('WHERE   CR.RoleType = :RoleType') ;
  if bcInsured.ItemIndex = 1 then
  cds_CreLim.SQL.Add('AND C.ClientNo not in (Select ClientNo FROM dbo.CreditInsurance) ') ;
 End ;//With
End ;

procedure TfrmCredit.AddNewGroup(Sender: TObject; const ClientNo : Integer; const ClientName : String) ;
begin
 With dmModule1 do
 Begin
  cds_CreLim.Insert ;
...
begin
 ...
 Try
  ...
  AddNewGroup (Sender, frmSelectCustomer.cds_ClientsClientNo.AsInteger, frmSelectCustomer.cds_ClientsClientName.AsString) ;
 Finally
...
//   frmCreditGrpClients.grpNo          := dmModule1.cds_CreLimgrpno.AsInteger ;
   frmCreditGrpClients.Panel1.Caption := 'GRUPP: '+dmModule1.cds_CreLimName.AsString ;
...
begin
 With dmModule1 do
 Begin
  acDelete.Enabled:= (cds_CreLim.Active) and ((cds_CreLim.RecordCount > 0)
  or (cds_CreLim.State in [dsEdit, dsInsert])) ;
 End ;
end;
...
begin
 ...
 Try
  if frmSelectCustomer.ShowModal = mrOk then
  GroupName := dmModule1.GetCreditGroupName(frmSelectCustomer.cds_ClientsClientNo.AsInteger) ;
