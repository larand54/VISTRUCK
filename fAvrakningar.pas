...
type
  ...
  private
    ...
    Procedure InsertTo_cdsPaymentLoads(const TypeOf, NewPaymentNo, OldPaymentNo : Integer) ;
    Procedure UppdateraPriser ;
    function  AreLoadsAlreadyCredited : Boolean ;
    procedure UpdateAllPricesExecute ;
    Procedure Set_CreditToPaymentNo_To_One_In_LastAvrDtl(const OldPaymentNo : Integer) ;
  public
    { Public declarations }
...
begin
 ...
 dmsSystem.StoreGridLayout(ThisUser.UserID, grdPayment.Name, grdPaymentDBTableView1) ;
 dmsSystem.StoreGridLayout(ThisUser.UserID, grdAvrRows.Name, grdAvrRowsDBTableView1) ;
...
begin
  ...
  dmsSystem.LoadGridLayout(ThisUser.UserID, grdPayment.Name, grdPaymentDBTableView1) ;
  dmsSystem.LoadGridLayout(ThisUser.UserID, grdAvrRows.Name, grdAvrRowsDBTableView1) ;
end;


procedure TfrmGjordaAvrakningar.CreateCo(const CompanyNo, TypeOfPayment: Integer);
//Var x : Integer ;
begin
...
procedure TfrmGjordaAvrakningar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 Action:= caFree ;
end;
...
begin
 frmGjordaAvrakningar:= Nil ;
  ...
  if dmsSystem.DeleteAssigned('TfrmGjordaAvrakningar', 'dmArrivingLoads') = True then
  Begin
   dmArrivingLoads.Free ;
   dmArrivingLoads := Nil ;
  End ;

  if dmsSystem.DeleteAssigned('TfrmGjordaAvrakningar1', 'dm_Avrakning') = True then
  Begin
   dm_Avrakning.Free ;
   dm_Avrakning := Nil ;
  End ;
...
begin
 With dm_Avrakning do
 Begin
  if (AllLoadsAttesterad) or (cdsPaymentHeadSenderStatus.AsInteger = 0)
  or (cdsPaymentHeadSenderStatus.AsInteger = 1) then
  Begin
   if cdsPaymentHead.State in [dsEdit, dsInsert] then
    cdsPaymentHead.Post ;
...
begin
 ...
 Begin
  ...
  cdsPaymentHead.ParamByName('SupplierNo').AsInteger    := cds_PropsVerkNo.AsInteger ;
  cdsPaymentHead.ParamByName('SenderStatus').AsInteger  := cds_PropsStatus.AsInteger ;
...
begin
 grdPayment.SetFocus ;
 if dm_Avrakning.cdsPaymentHeadPaymentNo.AsInteger < 1 then exit ;
 FormCRViewReport:= TFormCRViewReport.Create(Nil);
 Try
 FormCRViewReport.CreateCo('AVRAKNING_ver_II.RPT') ;
 if FormCRViewReport.ReportFound then
 Begin
  FormCRViewReport.report.ParameterFields.Item[1].AddCurrentValue(dm_Avrakning.cdsPaymentHeadPaymentNo.AsInteger);
...
begin
 if MessageDlg('Vill du ta bort?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
 With dm_Avrakning do
 Begin

  sq_UpdateLoadDtlVal.ParamByName('SetPaymentNo').AsInteger       := -1 ;
  sq_UpdateLoadDtlVal.ParamByName('PaymentNo').AsInteger          := cdsPaymentHeadPaymentNo.AsInteger ;
  sq_UpdateLoadDtlVal.ParamByName('PaymentType').AsInteger        := 0 ;
  sq_UpdateLoadDtlVal.ParamByName('LoadDtlValPaymentNo').AsInteger:= cdsPaymentHeadPaymentNo.AsInteger ;
...
begin
 With dm_Avrakning do
 Begin
  acExportToExcel.Enabled:= (cdsPaymentHead.Active) and (cdsPaymentHead.RecordCount > 0) ;
 End ;
end;
...
begin
 With dm_Avrakning do
 Begin
  acDeleteAvrakning.Enabled:= (cdsPaymentHead.Active)
  and (cdsPaymentHead.RecordCount > 0)
  and (cdsPaymentHeadSenderStatus.AsInteger < 2)
  and (cdsPaymentHeadPaymentType.AsInteger = 1) ;
 End ;
end;
...
Var PaymentNo   : Integer ;
    myRollBack  : Boolean ;
    Save_Cursor : TCursor;
begin
 if MessageDlg('Vill du kreditera markerade laster?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
 With dm_Avrakning do
 Begin
 Save_Cursor := Screen.Cursor;
 Try
...
// cdsLastAvr.Active      := True ;
 ...
 Try
  ...
  cdsPayHead.ParamByName('SupplierNo').AsInteger := cdsPaymentHeadSupplierNo.AsInteger ;
  cdsPayHead.ParamByName('CustomerNo').AsInteger := cdsPaymentHeadCustomerNo.AsInteger ;
  cdsPayHead.ParamByName('TypeOf').AsInteger     := cdsPaymentHeadTypeof.AsInteger ;
  ...
  cdsPayHeadSupplierNo.AsInteger      := cdsPaymentHeadSupplierNo.AsInteger ;
  //Är det ett köp är customer det verk som är valt i droplistan.
  cdsPayHeadCustomerNo.AsInteger      := cdsPaymentHeadCustomerNo.AsInteger ;
  cdsPayHead.Post ;
 ...
 End ;//if cdsPayHead...
  ...
  Begin
   Try
    GetMarkedLoads ;
    InsertTo_cdsPaymentLoads(cdsPaymentHeadTypeof.AsInteger, PaymentNo, cdsPaymentHeadPaymentNo.AsInteger) ;
    ...
    if myRollBack = False then
    Begin
     Set_CreditToPaymentNo_To_One_In_LastAvrDtl(cdsPaymentHeadPaymentNo.AsInteger) ;
    End ;
...
//    RecID:= grdAvrRowsDBTableView1.DataController.GetRecordId(RecIdx) ;
    ...
    RecIDx          := grdAvrRowsDBTableView1.Controller.SelectedRecords[i].RecordIndex ;
    ColIdx          := grdAvrRowsDBTableView1.DataController.GetItemByFieldName('VIS_FS').Index;
    LoadNo          := grdAvrRowsDBTableView1.DataController.Values[RecIdx, ColIdx];
    ColIdx          := grdAvrRowsDBTableView1.DataController.GetItemByFieldName('PaymentType').Index;
    PaymentType     := grdAvrRowsDBTableView1.DataController.Values[RecIdx, ColIdx];
...
Procedure TfrmGjordaAvrakningar.InsertTo_cdsPaymentLoads(const TypeOf, NewPaymentNo, OldPaymentNo : Integer) ;
Begin
 With dm_Avrakning do
 Begin
   mtLoadNos.First ;
   ...
   Begin
    ...
    sq_InsLoadDtlVal.ParamByName('LoadNo').AsInteger             := mtLoadNosLoadNo.AsInteger ;
    ...
    sq_UpdCredInLoadDtlVal.ParamByName('OldPaymentNo').AsInteger       := OldPaymentNo ;
//    sq_InsLoadDtlVal.ParamByName('NewPaymentNo').AsInteger       := -1 ;
//    sq_InsLoadDtlVal.ParamByName('NewPaymentType').AsInteger     := 0 ;
    sq_UpdCredInLoadDtlVal.ParamByName('LoadNo').AsInteger             := mtLoadNosLoadNo.AsInteger ;
...
//    cdsLastAvr.Post ;
    ...
    sq_DelLastAvr.ParamByName('PaymentNo').AsInteger    := mtLoadNosPaymentNo.AsInteger ;
    sq_DelLastAvr.ParamByName('LoadNo').AsInteger       := mtLoadNosLoadNo.AsInteger ;
...
Procedure TfrmGjordaAvrakningar.Set_CreditToPaymentNo_To_One_In_LastAvrDtl(const OldPaymentNo : Integer) ;
Begin
 With dm_Avrakning do
 Begin
   mtLoadNos.First ;
    ...
    Begin
     ...
     if cds_PropsVerkNo.AsInteger = 741 then
      SupplierNo := cdsPaymentHeadCustomerNo.AsInteger
        ...
        SupplierNo:= cds_PropsVerkNo.AsInteger ;
     dmArrivingLoads.GetIntPrice(SupplierNo, mtLoadNosPaymentType.AsInteger, mtLoadNosPaymentNo.AsInteger, mtLoadNosLoadNo.AsInteger, False) ;
    End ;
    mtLoadNos.Next ;
   End ;//while
  mtLoadNos.Active:= False ;
...
Begin
 ...
 Begin
   ...
   While not mtLoadNos.Eof  do
   Begin
    sq_IsLoadCredited.ParamByName('LoadNo').AsInteger     := mtLoadNosLoadNo.AsInteger ;
    sq_IsLoadCredited.ParamByName('PaymentType').AsInteger:= -1 ;
    sq_IsLoadCredited.Open ;
    if not sq_IsLoadCredited.Eof then
    Begin
     ShowMessage('Lastnr '+mtLoadNosLoadNo.AsString+' är redan krediterad. Se avräkningnr '+sq_IsLoadCreditedPaymentNo.AsString);
     Result:= True ;
    End ;
...
begin
 With dm_Avrakning do
 Begin
  acUppdateraPriser.Enabled:= (cdsPaymentHeadSenderStatus.AsInteger < 2)
  and (cdsPaymentLoadListII.Active)
  and (cdsPaymentLoadListII.RecordCount > 0)
...
begin
 if ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('VALUE').Index] <> null then
 Belopp:= ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('VALUE').Index] ;
 if ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('Pris1').Index] <> null then
 Pris1:= ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('Pris1').Index] ;
 ...
 if ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('Attesterad').Index] <> null then
 Attesterad:= ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('Attesterad').Index]
 else
 Attesterad:= -1 ;
 ...
 if ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('CreditToPaymentNo').Index] <> null then
 CreditToPaymentNo:= ARecord.Values[TcxGridDBTableView(Sender).GetColumnByFieldName('CreditToPaymentNo').Index] ;
...
begin
 if MessageDlg('Vill du ta bort markerade laster från avräkningen?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
 With dm_Avrakning do
 Begin
  cdsPaymentLoads.Active:= False ;
  cdsPaymentLoads.ParamByName('PaymentNo').AsInteger:= cdsPaymentHeadPaymentNo.AsInteger ;
  cdsPaymentLoads.Active:= True ;
...
//**har man krediterat fel last kan man lägga lasten oförändrad till korrigeringsfakturan och får då ett noll belopp.
    ...
    sq_UpdLoadDtlValSingleLoad.ParamByName('PaymentNo').AsInteger          := mtLoadNosPaymentNo.AsInteger ;
    sq_UpdLoadDtlValSingleLoad.ParamByName('PaymentType').AsInteger        := 0 ;
    sq_UpdLoadDtlValSingleLoad.ParamByName('LoadNo').AsInteger             := mtLoadNosLoadNo.AsInteger ;
    sq_UpdLoadDtlValSingleLoad.ParamByName('SupplierNo').AsInteger         := mtLoadNosSupplierNo.AsInteger ;
...
begin
 With dm_Avrakning do
 Begin
  acCancelLoad.Enabled:= (cdsPaymentHeadSenderStatus.AsInteger < 2)
  and (cdsPaymentLoadListIIPaymentType.AsInteger = 1)
  and (cdsPaymentHeadPaymentType.AsInteger = 1) ;
 End ;
end;
...
//då är det ingen risk att fel lager väljs
        //dmcOrder.SupplierNo,
        LSupplierNo,
        cdsPaymentLoadListIIVIS_FS.AsInteger,
        0,
        0,
        0,
...
begin
 With dm_Avrakning do
 Begin
  acOpenLoad.Enabled:= (cdsPaymentLoadListII.Active) and (cdsPaymentLoadListII.RecordCount > 0) ;
 End ;
end;
...
begin
 With dm_Avrakning do
 Begin
  acUpdateAllPrices.Enabled  := (cdsPaymentLoadListII.Active) and (cdsPaymentLoadListII.RecordCount > 0)
  and  (cds_PropsVerkNo.AsInteger <> 0) and (cds_PropsStatus.AsInteger <> 4) ;
 End ;// With dm_Avrakning do
end;
