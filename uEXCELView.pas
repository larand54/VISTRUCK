...
type
  ...
  TfEXCELView = class(TForm)
    ...
    cds_StorageGroupsGrupp: TStringField;
...
procedure TfEXCELView.PopulateCheckBoxMallar ;
Begin
...
procedure TfEXCELView.PopulateCheckBoxMallarProdSUM ;
Begin
 cbProdSUM.Clear ;
...
procedure TfEXCELView.ClearProductFilter ;
Begin
...
procedure TfEXCELView.SetCheckComboBoxes ;
Begin
 With dmInventory do
 Begin
...
procedure TfEXCELView.SetCheckComboBoxes_Where_PktNrLevKod_Required (const PktNrLevKod : String) ;
Begin
 With dmInventory do
 Begin
...
procedure TfEXCELView.sq_UserProfileAfterInsert(DataSet: TDataSet);
begin
 sq_UserProfileUserID.AsInteger := ThisUser.UserID ;
end;

procedure TfEXCELView.LoadCheckBox(const aFieldName, aCheckItemFld, aCheckItem: string;
...
procedure TfEXCELView.LoadCheckBoxWithSalesRegion;
begin
...
procedure TfEXCELView.LoadCheckBoxWithSTorageArea;
Var
  Count, x: Integer;
  SGList: TCMComboBoxItems;
  CLList: TCMComboBoxItems;
  s1,s2: string;
  SQLSave: TStrings;
begin
  cds_StorageAreas.Active := False;
  SQLSave := cds_StorageAreas.SQL;
  try
    ...
    begin
      ...
      begin
        if CLList = nil then
          CLList := TCMComboBoxItems.Create;
        s1 := cbOwner.Properties.Items[x].Description;
        s2 := cbOwner.Properties.Items[x].ShortDescription;
        CLList.Add(s1,s2);
      end;
    end;
    ...
    begin
      ...
      begin
        if SGList = nil then
          SGList := TCMComboBoxItems.Create;
        s1 := cbStorageGroup.Properties.Items[x].Description;
        s2 := cbStorageGroup.Properties.Items[x].ShortDescription;
        SGList.Add(s1,s2);
      end;
    end;
    if (assigned(SGList) AND (SGList.items.Count > 0))
    OR (assigned(CLList) AND (CLList.items.Count > 0))  then
    begin
      ...
      if (assigned(SGList) AND (SGList.items.Count > 0)) then
      begin
        cds_StorageAreas.SQL.Add('AND ((LIP.Grupp = ' + SGList.Items[0].Description.quotedString + ')');
        for x := 1 to SGList.items.Count-1 do
        begin
          cds_StorageAreas.SQL.Add(' OR (LIP.Grupp = '+ SGList.Items[x].Description.quotedString + ')');
        end;
        cds_StorageAreas.SQL.Add(')');
      end;
      cds_StorageAreas.SQL.Add('INNER JOIN Area ON Area.PIPNo = PIP.PhysicalInventoryPointNo');
      if (assigned(CLList) AND (CLList.items.Count > 0)) then
      begin
        cds_StorageAreas.SQL.Add('WHERE  (C.ClientNo = ' + CLList.Items[0].ShortDescription + ')');
        for x := 1 to CLList.items.Count-1 do
          cds_StorageAreas.SQL.Add('OR (C.ClientNo = ' + CLList.Items[x].ShortDescription + ')');
      end;
   //   cds_StorageAreas.SQL.Add(' )');
...
procedure TfEXCELView.LoadCheckBoxWithStorageGroup;
begin
  LoadCheckBox('Grupp','','',cds_StorageGroups,cbStorageGroup);
end;

procedure TfEXCELView.LoadCheckBoxWithStoragePos;
Var
  Count, x: integer;
  s: string;
begin
...
procedure TfEXCELView.LoadCheckBoxWithVerk ;
Var
  Count, x: Integer;
...
Procedure TfEXCELView.CreateCo(Sender: TObject; CompanyNo: Integer);
var
  Save_Cursor : TCursor;
begin
...
procedure TfEXCELView.FormClose(Sender: TObject;
...
procedure TfEXCELView.FormCreate(Sender: TObject);
begin
...
procedure TfEXCELView.FormDestroy(Sender: TObject);
begin
// if Assigned(dmInventory) then
// FreeAndNil(dmInventory);
end;

procedure TfEXCELView.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfEXCELView.acCloseExecute(Sender: TObject);
begin
 Close ;
end;

procedure TfEXCELView.FormCloseQuery(Sender: TObject;
...
procedure TfEXCELView.grdPkgTblDBBandedTableView1L1GetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: String);
begin
...
procedure TfEXCELView.grdProdPaketNrDBBandedTableView1DataControllerSummaryAfterSummary(
  ASender: TcxDataSummary);
...
procedure TfEXCELView.acPkgInfoExecute(Sender: TObject);
...
procedure TfEXCELView.acCollapseAllProdPaketNrExecute(Sender: TObject);
begin
 grdProdPaketNrDBBandedTableView1.ViewData.Collapse(True);
end;

procedure TfEXCELView.acCollapseAllProdSumExecute(Sender: TObject);
begin
 grdProdSUMDBBandedTableView1.ViewData.Collapse(True);
end;

procedure TfEXCELView.acDeleteProdPaketNrMallExecute(Sender: TObject);
begin
 ...
 Begin
  ...
  Begin
   sq_UserProfile.Active := False ;
   sq_UserProfile.ParamByName('UserID').AsInteger := ThisUser.UserID ;
   sq_UserProfile.ParamByName('Form').AsString    := fEXCELView.Name + '1' ;
...
procedure TfEXCELView.acDeleteProdSUMMallExecute(Sender: TObject);
begin
 With dmInventory do
 Begin
  ...
  Begin
   sq_UserProfile.Active := False ;
   sq_UserProfile.ParamByName('UserID').AsInteger := ThisUser.UserID ;
   sq_UserProfile.ParamByName('Form').AsString    := fEXCELView.Name + '2' ;
   sq_UserProfile.Active := True ;
   Try
...
procedure TfEXCELView.acExpandAllProdPaketNrExecute(Sender: TObject);
begin
...
procedure TfEXCELView.acExpandAllProdSUMExecute(Sender: TObject);
begin
 grdProdSUMDBBandedTableView1.ViewData.Expand(True);
end;

procedure TfEXCELView.acExportExecute(Sender: TObject);
var
  Save_Cursor:TCursor;
begin
 Save_Cursor := Screen.Cursor;
...
Function TfEXCELView.SelectedOwnersOK : Boolean ;
Var
    VidaWood            : Boolean ;
...
Function TfEXCELView.GetComboFilterText(const Kolumn : String;combo : TcxCheckComboBox) : String ;
Var
...
Function TfEXCELView.ComboBoxFilterChecked(const Kolumn : String;combo : TcxCheckComboBox) : Boolean ;
Var
    APCheckStates : ^TcxCheckStates;
    x             : Integer ;
Begin
...
procedure TfEXCELView.mtUserPropAfterInsert(DataSet: TDataSet);
begin
...
procedure TfEXCELView.acNewProdSUMMallExecute(Sender: TObject);
...
procedure TfEXCELView.grdPkgNoTblDBBandedTableView1L1GetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: String);
begin
 if (AText = '0') or (AText = '.0') or (AText = ',0') then
...
procedure TfEXCELView.acPrintExecute(Sender: TObject);
...
procedure TfEXCELView.SetDateFields ;
...
procedure TfEXCELView.acRefreshProdPaketNrExecute(Sender: TObject);
var
  SQLView: TSQLView;
  SQLBUILD: TSQLBuild;
begin
...
procedure TfEXCELView.ccbATPropertiesClickCheck(Sender: TObject;
  ItemIndex: Integer; var AllowToggle: Boolean);
begin
 TcxCheckComboBox(Sender).DroppedDown := False;
end;

procedure TfEXCELView.eATExit(Sender: TObject);
Var x     : Integer ;
    Found : Boolean ;
Begin
...
procedure TfEXCELView.eABExit(Sender: TObject);
Var x     : Integer ;
    Found : Boolean ;
Begin
 ccbAB.Clear ;
...
procedure TfEXCELView.eALExit(Sender: TObject);
Var x     : Integer ;
    Found : Boolean ;
Begin
 ccbAL.Clear ;
...
procedure TfEXCELView.acSaveProdPaketNrMallExecute(Sender: TObject);
begin
 if cbProdPaketNr.ItemIndex > -1 then
 Begin
  dmsSystem.StoreGridLayout_Special( ThisUser.UserID,   fEXCELView.Name + '1',
...
procedure TfEXCELView.acSaveProdSUMMallExecute(Sender: TObject);
begin
 if cbProdSUM.ItemIndex > -1 then
 Begin
  dmsSystem.StoreGridLayout_Special( ThisUser.UserID,   fEXCELView.Name + '2',
...
procedure TfEXCELView.acNewProdPaketNrMallExecute(Sender: TObject);
//var  fAngeNyMall: TfAngeNyMall;
//     NewIndex : Integer ;
begin
(*
...
procedure TfEXCELView.OpenStandardMall(Sender: TObject);
begin
 With dmInventory do
 Begin
...
procedure TfEXCELView.mInvNosEnter(Sender: TObject);
begin
 //dmsSystem.MemoInAction:= True ;
end;

procedure TfEXCELView.mInvNosExit(Sender: TObject);
begin
 //dmsSystem.MemoInAction:= False ;
end;


Function TfEXCELView.CheckIfAnyCheckedItemsInComboFilter(combo : TcxCheckComboBox) : Boolean ;
Var
    APCheckStates : ^TcxCheckStates;
    x             : Integer ;
Begin
...
procedure TfEXCELView.acExportToExcelExecute(Sender: TObject);
Var FileName  : String ;
begin
...
procedure TfEXCELView.cbExpandedPropertiesChange(Sender: TObject);
begin
 if cbExpanded.Checked then
  grdProdPaketNrDBBandedTableView1.ViewData.Expand(True)
   else
...
procedure TfEXCELView.cbOwnerPropertiesCloseUp(Sender: TObject);
Var x           : Integer ;
    PktNrLevKod : String ;
begin
  LoadCheckBoxWithSTorageArea;
...
procedure TfEXCELView.cbProdPaketNrPropertiesChange(Sender: TObject);
begin
...
procedure TfEXCELView.Open_UserProfile ;
Begin
 sq_UserProfile.Active  := False ;
 sq_UserProfile.ParamByName('UserID').AsInteger := ThisUser.UserID ;
 sq_UserProfile.ParamByName('Form').AsString    := fEXCELView.Name + '1' ;
...
procedure TfEXCELView.cbProdSUMPropertiesChange(Sender: TObject);
begin
//Open prod sum grid Mall
 OpenProdSUMMall ;
end;

procedure TfEXCELView.cbSalesRegionPropertiesCloseUp(Sender: TObject);
begin
  LoadCheckBoxWithVerk;
  LoadCheckBoxWithSTorageArea;
  LoadCheckBoxWithStoragePos;
...
procedure TfEXCELView.cbStorageAreaPropertiesCloseUp(Sender: TObject);
begin
  LoadCheckBoxWithStoragePos;
end;

procedure TfEXCELView.cbStorageGroupPropertiesCloseUp(Sender: TObject);
begin
  LoadCheckBoxWithSTorageArea;
  LoadCheckBoxWithStoragePos;
end;

procedure TfEXCELView.OpenProdPaketNrMall ;
begin
//Open Mall
 if cbProdPaketNr.ItemIndex > -1 then
 Begin
  dmsSystem.LoadGridLayout_Special( ThisUser.UserID, fEXCELView.Name + '1',
  cbProdPaketNr.Properties.Items.Strings[cbProdPaketNr.ItemIndex]
  + '/' + fEXCELView.Name + '1', grdProdPaketNrDBBandedTableView1,'fEXCELView') ;
 End;
end;

procedure TfEXCELView.OpenProdSUMMall ;
begin
//Open Mall
 if cbProdSUM.ItemIndex > -1 then
 Begin
  dmsSystem.LoadGridLayout_Special( ThisUser.UserID, fEXCELView.Name + '2',
  cbProdSUM.Properties.Items.Strings[cbProdSUM.ItemIndex]
  + '/' + fEXCELView.Name + '2', grdProdSUMDBBandedTableView1,'fEXCELView') ;
 End;
end;

procedure TfEXCELView.cxButton1Click(Sender: TObject);
begin
 Close ;
end;

procedure TfEXCELView.cxButton2Click(Sender: TObject);
begin
 ClearProductFilter ;
end;

procedure TfEXCELView.eNTExit(Sender: TObject);
Var x     : Integer ;
    Found : Boolean ;
Begin
...
procedure TfEXCELView.eNBExit(Sender: TObject);
Var x     : Integer ;
    Found : Boolean ;
Begin
...
procedure TfEXCELView.SaveUserProfile ;
Begin
 sq_UserProfile.Active  := False ;
 sq_UserProfile.ParamByName('UserID').AsInteger := ThisUser.UserID ;
 sq_UserProfile.ParamByName('Form').AsString    := fEXCELView.Name + '1' ;
