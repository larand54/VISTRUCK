...
uses
  ...
  dxPScxCommon, dxPScxGrid6Lnk, FMTBcd, 
...
{   deStart.Date  := RecodeHour(deStart.Date,0) ;
  ...
  cds_LoadFC.SQL.Add('0 AS Markerad, 0.0 AS Belopp,') ;
  cds_LoadFC.SQL.Add('0.0 AS PrisPerAM3SEK,') ;
  ...
  cds_LoadFC.SQL.Add('AND  L.LoadNo NOT IN (Select LoadNo FROM dbo.LoadFreightCost)') ;
...
Begin
 ...
 if (cds_PropsShipperNo.AsInteger > 0) and (cds_PropsVerkNo.AsInteger > 0) then
 Begin
  Save_Cursor := Screen.Cursor;
