...
type
  ...
  public
    ...
    function  GetLOByInvNo(const InvoiceNo : Integer;Var InternalInvoiceNo : Integer) : Integer ;
...
begin
 ...
 cdsNormalLO.ParamByName('CustShipPlanDetailObjectNo').AsInteger:= cdsAvropCustShipPlanDetailObjectNo.AsInteger ;
...
Begin
 ...
 cdsLoads.ParamByName('ShippingPlanNo').AsInteger :=  cdsAvropShippingPlanNo.AsInteger ;
 cdsLoads.ParamByName('CustomerNo').AsInteger     :=  cdsAvropCLIENTNO.AsInteger ; // CSH.CustomerNo
 cdsLoads.ParamByName('SupplierNo').AsInteger     :=  cdsAvropSALESREGIONNO.AsInteger ;
...
// TempDataSet.CloneCursor(cdsLoads, True);
 ...
 Begin
  if (TempDataSet.FieldByName('INTERNAL_INVOICENO').AsInteger = -1) AND (TempDataSet.FieldByName('SenderLoadStatus').AsInteger = 2)
  AND (TempDataSet.FieldByName('ACT_M3').IsNull = False) then
  Begin
   Result:= True ;
   Exit ;
  End ;
...
function TdaMoLM1.GetLOByInvNo(const InvoiceNo : Integer;Var InternalInvoiceNo : Integer) : Integer ;
Begin
 Try
...
Begin
 ...
 sq_ValidLONr.ParamByName('SupplierNo').AsInteger := cdsAvropSALESREGIONNO.AsInteger ;
 sq_ValidLONr.Open ;
 Try
 if (not sq_ValidLONr.Eof) and (sq_ValidLONrShippingPlanNo.AsInteger > 0) then
  Begin
   if (sq_ValidLONrSalesRegionNo.AsInteger = cdsAvropSALESREGIONNO.AsInteger) then
