...
type
  TfImportedPackages = class(TForm)
    ...
    grdDelShipDelMsgRefDBTableView1DeliveryShipmentLineItemNumber: TcxGridDBColumn;
    grdDelShipDelMsgRefDBTableView1DeliveryMessageReference: TcxGridDBColumn;
    grdDelShipDelMsgRefDBTableView1DeliveryMessageReferenceType: TcxGridDBColumn;
...
begin
 ...
 Begin
  ...
  Try
   ...
   cds_ImpClientMap.ParamByName('DeliveryMessageNumber').AsString  := DeliveryMessageNumber ;
   cds_ImpClientMap.Active:= True ;

   cds_DSDMR2.Active:= False ;
   cds_DSDMR2.ParamByName('DeliveryMessageNumber').AsString        := DeliveryMessageNumber ;
   cds_DSDMR2.Active:= True ;

   cds_ImpProdMap.Active:= False ;
   cds_ImpProdMap.ParamByName('DeliveryMessageNumber').AsString    := DeliveryMessageNumber ;
   cds_ImpProdMap.ParamByName('SalesRegionNo').AsInteger           := mtUserPropOwnerNo.AsInteger ;
...
begin
  dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name+'/'+grdImpPkgs.Name, grdImpPkgsDBTableView1) ;
  dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name+'/'+grdImpLengths.Name, grdImpLengthsDBTableView1) ;
end;

procedure TfImportedPackages.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
 dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name + '/' + grdImpPkgs.Name, grdImpPkgsDBTableView1) ;
 dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name + '/' + grdImpLengths.Name, grdImpLengthsDBTableView1) ;
...
begin
 if MessageDlg('Vill du avsluta utan att spara?',
...
begin
 ...
 Begin
  ...
  Try
   if (cdsClientPartyIdentifier.IsNull) or (Length(cdsClientPartyIdentifier.AsString) = 0) then
   Begin
    ShowMessage('Klientkoppling ej fullständig,  Party identifier saknas. Åtgärda genom att spara klientkoppling');
    Exit ;
   End ;
   if (cdsClientPartyIdentifierType.IsNull) or (Length(cdsClientPartyIdentifierType.AsString) = 0) then
   Begin
    ShowMessage('Klientkoppling ej fullständig,  Party identifier typ saknas. Åtgärda genom att spara klientkoppling');
    Exit ;
   End ;

   if not IsClientConnectionValid then
   Begin
    ShowMessage('WoodX och VIS klienten matchar inte, kontrollera att WoodX klienten är densamma som leverantören i VIS leveransen');
    Exit ;
   End ;
...
//   mtImportedProductLengths.Active  := False ;
    ...
    On E: Exception do
    Begin
     dmsSystem.FDoLog(E.Message+' :TfImportedPackages.acCreatePkgsExecute') ;
...
begin
 ...
 Begin
  mtUserPropSupplierCode.AsString := dmsContact.GetClientCode(mtUserPropVerkNo.AsInteger) ;
 End ;
end;
...
procedure TfImportedPackages.acSetPrefixOnMarkedPkgsExecute(
  Sender: TObject);
...
begin
 ...
 Begin
    if dm_ImportWoodx.cds_ImpProdMap.Locate('Identifier', mtPkgNosPackageNo.AsInteger,[]) then
    Begin
     dm_ImportWoodx.cds_ImpProdMap.Edit ;
     dm_ImportWoodx.cds_ImpProdMap.FieldByName('ClientCode').AsString  :=  Prefix ;
...
begin
 ...
 Try
   ...
   Begin
    RecIDx:= grdImpPkgsDBTableView1.Controller.SelectedRecords[i].RecordIndex ;
...
{    if ADataSet.Locate('Identifier', RecID,[]) then
    ...
    Begin
     mtPkgNos.Insert ;
     mtPkgNosPackageNo.AsInteger := ADataSet.FieldByName('Identifier').AsInteger ;
     mtPkgNosSupp_Code.AsString  := ADataSet.FieldByName('ClientCode').AsString ;
...
procedure TfImportedPackages.acSaveClientConnectionExecute(
  Sender: TObject);
begin
 With dm_ImportWoodx do
 Begin
  EditClient(cds_ImpClientMapWoodXPartyIdentifier.AsString, cds_ImpClientMapWoodXPartyIdentifierType.AsString) ;
