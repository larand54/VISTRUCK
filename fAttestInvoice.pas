...
uses
  ...
  OleServer, dxDBEdtr,
  Buttons, dxDBTLCl, dxGrClms, SqlTimSt, dxBar, dxBarExtItems, Math,
  ...
  cxMemo, cxGridCustomPopupMenu, cxGridPopupMenu, cxSplitter ;
...
type
  TfrmAttestInvoice = class(TForm)
    ...
    dxDBGrid1: TdxDBGrid;
    dxDBGrid1InternalInvoiceNo: TdxDBGridMaskColumn;
    dxDBGrid1ShippingPlanNo: TdxDBGridMaskColumn;
    dxDBGrid1SequensNo: TdxDBGridMaskColumn;
    dxDBGrid1InvoiceDetailNo: TdxDBGridMaskColumn;
    CheckBox1: TCheckBox;
    ...
    procedure eInvNoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure grdInvDtlDBTableView1CheckedPropertiesChange(
      Sender: TObject);
...
begin
  dmVidaInvoice.mt_InvDtl_Att.Filter:= 'InternalInvoiceNo = ' + dmVidaInvoice.cdsInvoiceHead_IIInternalInvoiceNo.AsString+
  ...
  dmVidaInvoice.RefreshCHECKED ;
   ...
   dmVidaInvoice.cdsInvoiceLO.Filter:= 'InternalInvoiceNo = ' + dmVidaInvoice.cdsInvoiceHead_IIInternalInvoiceNo.AsString+
  ' AND ShippingPlanNo = ' + TabControl1.Tabs[TabControl1.TabIndex] ;
...
begin
 ...
 Begin
  ...
  if cdsBooking.Eof then
   ...
   Label42.Caption:= Trim(cdsInvoiceHead_IIAddressLine1.AsString)+', '+Trim(cdsInvoiceHead_IIAddressLine2.AsString)+', '+
   Trim(cdsInvoiceHead_IIAddressLine3.AsString)+', '+Trim(cdsInvoiceHead_IIAddressLine4.AsString)+', '+
   Trim(cdsInvoiceHead_IICityName.AsString)+', '+Trim(cdsInvoiceHead_IIStateOrProvince.AsString)+', '+
   Trim(cdsInvoiceHead_IIPostalCode.AsString)+', '+Trim(cdsInvoiceHead_IICountryName.AsString) ;

   Label43.Caption:= Trim(cdsInvoiceHead_IIAGENT_SHIPTO_ADDRESSLINE1.AsString)+', '+Trim(cdsInvoiceHead_IIAGENT_SHIPTO_ADDRESSLINE2.AsString)+', '+
   Trim(cdsInvoiceHead_IIAGENT_SHIPTO_ADDRESSLINE3.AsString)+', '+Trim(cdsInvoiceHead_IIAGENT_SHIPTO_ADDRESSLINE4.AsString)+', '+
   Trim(cdsInvoiceHead_IIAGENT_SHIPTO_CITY.AsString)+', '+Trim(cdsInvoiceHead_IIAGENT_SHIPTO_PROVINCE.AsString)+', '+
   Trim(cdsInvoiceHead_IIAGENT_SHIPTO_POSTALCODE.AsString)+', '+Trim(cdsInvoiceHead_IIAGENT_SHIPTO_COUNTRY.AsString) ;
...
//   grdInvoiceRowsPriceUnit.Items.Add(sq_PriceUnitTemplateUnitName.AsString) ;
  ...
  sq_PriceUnit.Close ;
   ...
   dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name + '/' + grdInvDtlDBTableView1.Name, grdInvDtlDBTableView1) ;
   dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name + '/' + grdAttDBTableView1.Name, grdAttDBTableView1) ;   
 End ; //with
end;
...
procedure TfrmAttestInvoice.eInvNoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
  Var IntInvNo, InvoiceType : Integer ;
begin
 ...
 With dmVidaInvoice do
 Begin
  IntInvNo  := GetInternalInvoiceNoByINVOICENO (StrToIntDef(eInvNo.Text,0), InvoiceType) ;
  ...
  Case cbInvoiceType.ItemIndex of
   ...
   3 : Begin
        ...
        Begin
         tlbAttestServicesClick(cds_PurchaseInvNoInternalInvoiceNo.AsInteger) ;
         TabControl1Change(Sender) ;
        End
        else
          ShowMessage('Hittar inte fakturanr '+eInvNo.Text) ;
...
//  if cdsInvoiceHead_II.FindKey([InternalInvoiceNo]) then
  ...
  ' AND ShippingPlanNo = ' + Self.TabControl1.Tabs[0] ;
  cdsInvoiceLO.Filtered:= True ;



  dmVidaInvoice.cds_Att_Ext_ServicesII.Active:= False ;
  dmVidaInvoice.cds_Att_Ext_ServicesII.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
//  dmVidaInvoice.cds_Att_Ext_ServicesII.ParamByName('ShippingPlanNo').AsInteger    := StrToIntDef(Self.TabControl1.Tabs[0],-1) ;
  ...
  dmVidaInvoice.cds_InvDtl_Att.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
  ...
  sq_GetAttestRow.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
  ...
  End
    ...
    ShowMessage('Internalfakturanr '+IntToStr(InternalInvoiceNo)+' finns inte.') ;
 End ; // with
end;
...
procedure TfrmAttestInvoice.nfSearchLOKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
 Var InternalInvoiceNo, InvoiceType : Integer ;
begin
 if Key <> VK_RETURN then Exit;
 IsDataUnSaved(Sender) ;
 With dmVidaInvoice do
 Begin
  InternalInvoiceNo := SearchLONo(StrToIntDef(nfSearchLO.Text,0), InvoiceType) ;
...
Begin
  Case cbInvoiceType.ItemIndex of
   0  : Begin
         if dmVidaInvoice.cdsInvoiceNumber.Locate('InternalInvoiceNo', InternalInvoiceNo,[]) then
         eFakturaNr.Text:= dmVidaInvoice.cdsInvoiceNumberInvoiceNo.AsString
          else
           ShowMessage('No luck') ;
        End ;
  1,2 : Begin
         if dmVidaInvoice.cdsProformaInvNo.Locate('InternalInvoiceNo', InternalInvoiceNo,[]) then
         eFakturaNr.Text:= dmVidaInvoice.cdsProformaInvNoProformaInvoiceNo.AsString
          else
           ShowMessage('No luck') ;
        End ;
   3  : Begin
         if dmVidaInvoice.cds_PurchaseInvNo.Locate('InternalInvoiceNo', InternalInvoiceNo,[]) then
         eFakturaNr.Text:= dmVidaInvoice.cds_PurchaseInvNoPO_InvoiceNo.AsString
         else
          eFakturaNr.Text:= '' ;
        End ;
   4  : Begin
        ...
        End ;
   5  : Begin
         if dmVidaInvoice.cdsFWInvNo.Locate('InternalInvoiceNo', InternalInvoiceNo,[]) then
...
begin
 ...
 Begin
  ...
  Begin
    ...
    cds_Att_Ext_ServicesIIAmountAttested.AsFloat          := RoundTo(cdsInvoiceHead_IICommission.AsFloat, -2) ;
    cds_Att_Ext_ServicesIISupplier_InvoiceNo.AsString     := '' ;
    cds_Att_Ext_ServicesIICurrencyNo.AsInteger            := cdsInvoiceHead_IICurrencyNo.AsInteger ;
    cds_Att_Ext_ServicesIISupplierNo.AsInteger            := cdsInvoiceHead_IIAgentNo.AsInteger ;
    cds_Att_Ext_ServicesIISupplier_InvoiceDate.AsDateTime := Date ;
    cds_Att_Ext_ServicesIIDescription.AsString            := 'Komm' ;
    cds_Att_Ext_ServicesIIQuick_InvoiceNo.AsInteger       := cdsInvoiceHead_IIInternalInvoiceNo.AsInteger ;
    cds_Att_Ext_ServicesII.Post ;
...
procedure TfrmAttestInvoice.GetInvoice(Sender: TObject; const InvoiceNo : Integer);
begin
 IsDataUnSaved(Sender) ;
...
*)
  ...
  Begin
   Try
   sq_Ins_AttestRow.ParamByName('InternalInvoiceNo').AsInteger  := dmVidaInvoice.mt_AttestRowInternalInvoiceNo.AsInteger ;
   sq_Ins_AttestRow.ParamByName('ShippingPlanNo').AsInteger     := dmVidaInvoice.mt_AttestRowShippingPlanNo.AsInteger ;
   sq_Ins_AttestRow.ParamByName('SequensNo').AsInteger          := dmVidaInvoice.mt_AttestRowSequensNo.AsInteger ;
   sq_Ins_AttestRow.ParamByName('InvoiceDetailNo').AsInteger    := dmVidaInvoice.mt_AttestRowInvoiceDetailNo.AsInteger ;
...
begin
 if MessageDlg('Vill du ta bort attesten?',
    mtConfirmation, [mbYes, mbNo], 0) = mrYes then
 Begin

 With dmVidaInvoice do
 Begin
  cds_AttestRow.ParamByName('SequensNo').AsInteger := cds_Att_Ext_ServicesIISequensNo.AsInteger ;
  Try
  ...
  cds_Att_Ext_LO.ParamByName('SequensNo').AsInteger := cds_Att_Ext_ServicesIISequensNo.AsInteger ;
  Try
...
begin
 ...
 Begin
  ...
  Begin
   SequensNo:= InvoiceAttested(frmSelectInvoice.cdsInvoiceListINT_INVNO.AsInteger) ;
   if SequensNo > 0 then
   ShowMessage('Fakturan är redan attesterad på attest nr '+IntToStr(SequensNo))
   else
   Begin
    cds_Att_Ext_ServicesII.Insert ;
    cds_Att_Ext_ServicesIIAmountAttested.AsFloat          := frmSelectInvoice.cdsInvoiceListInv_Value_After_Deduct_DIS_COMM.AsFloat ;
    cds_Att_Ext_ServicesIISupplier_InvoiceNo.AsString     := frmSelectInvoice.cdsInvoiceListINVOICE_NO.AsString ;
    cds_Att_Ext_ServicesIICurrencyNo.AsInteger            := frmSelectInvoice.cdsInvoiceListCurrencyNo.AsInteger ;
    cds_Att_Ext_ServicesIISupplierNo.AsInteger            := frmSelectInvoice.cdsInvoiceListCustomerNo.AsInteger ;
    cds_Att_Ext_ServicesIISupplier_InvoiceDate.AsDateTime := frmSelectInvoice.cdsInvoiceListINV_DATE.AsDateTime ;
    cds_Att_Ext_ServicesIIDescription.AsString            := 'Kreditfaktura '+frmSelectInvoice.cdsInvoiceListINVOICE_NO.AsString ;
    cds_Att_Ext_ServicesIIQuick_InvoiceNo.AsInteger       := frmSelectInvoice.cdsInvoiceListINT_INVNO.AsInteger ;
    if cds_Att_Ext_ServicesIIAmountAttested.AsFloat < 0 then
    cds_Att_Ext_ServicesIIAmountAttested.AsFloat := cds_Att_Ext_ServicesIIAmountAttested.AsFloat * (-1) ;
    cds_Att_Ext_ServicesII.Post ;
...
begin
 ...
 Begin
  ...
  Supplier_InvoiceDate  := cds_Att_Ext_ServicesIISupplier_InvoiceDate.AsDateTime ;
  ...
  cds_Att_Ext_ServicesIISupplier_InvoiceDate.AsDateTime := Supplier_InvoiceDate ;
...
begin
 With dmVidaInvoice do
 Begin
  acDeleteAttest.Enabled  := (cds_Att_Ext_ServicesII.Active)
  and (cds_Att_Ext_ServicesII.RecordCount > 0)
  and (cbDescription.Enabled) ;
 End ;//With
end;
...
begin
 With dmVidaInvoice do
 Begin
  if (cds_Att_Ext_ServicesIIDescription.AsString = 'Arbete')
  or (cds_Att_Ext_ServicesIIDescription.AsString = 'Vara') then
