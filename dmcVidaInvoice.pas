...
uses
  ...
  QExport3XLS, QExport3ASCII, clTcpClient, clFtp, Forms,
  clFTPUtils, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error,
  FireDAC.DatS, FireDAC.Phys.Intf, FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt,
  FireDAC.Comp.DataSet, FireDAC.Comp.Client, QImport2, QImport2DataSet, QImport2ASCII,
  StdCtrls, Math, DBCtrls, DateUtils ;
...
type
  TdmVidaInvoice = class(TDataModule)
    ...
    QImport2ASCII550: TQImport2ASCII;
    ...
    sp_InvTotalsFraktKonto: TStringField;
    sp_InvTotalsProduktvardeFOR: TFloatField;
    sp_InvTotalsProduktvardeSEK: TBCDField;
    ...
    sp_InvTotalsInvoiceTotal: TBCDField;
    sp_InvTotalsXRATE: TFloatField;
    sp_InvTotalsInvoiceTotal_SEK: TBCDField;
    sp_InvTotalsFRAKT_CURR_InclInPrice: TBCDField;
    sp_InvTotalsFRAKT_CURR_NOTInclInPrice: TBCDField;
    sp_InvTotalsFRAKT_SEK_InclInPrice: TBCDField;
    sp_InvTotalsFRAKT_SEK_NOTInclInPrice: TBCDField;
    sp_InvTotalsVAT_FOR: TBCDField;
    sp_InvTotalsVAT_SEK: TBCDField;
    sp_InvTotalsSkatteupplag: TIntegerField;
    sp_InvTotalsCurr: TStringField;
    sp_InvTotalsNM3: TBCDField;
    ...
    sp_InvTotalsVaruGrupp: TIntegerField;
    sp_InvTotalsPO_CountryNo: TIntegerField;
    sp_InvTotalsinvoicetype: TIntegerField;
    sp_InvTotalsFRAKTTOTAL_CURR: TBCDField;
    sp_InvTotalsFRAKTTOTAL_SEK: TBCDField;
    ...
    sp_InvTotalsDebit_Kredit: TIntegerField;
    ...
    procedure QExport3ASCII2BeforeExportRow(Sender: TObject;
      Row: TQExportRow; var Accept: Boolean);
  ...
  private
    ...
    lbList: TStringList ;
    function  AssignAGENTInvoiceNumber(const InternalInvoiceNo : Integer) : Integer ;
    ...
    Function  GetFileNameForEfterfragadLastnr(const FragarEfterLoadNo : Integer) : String ;
    ...
    procedure SetHTFPackageStatus550(const LocalDir : String) ;
    procedure UpdateHTFPackages(const FileName : String;const LONo, LoadNo : Integer) ;
    procedure UpdateHTFPackages550(const FileName : String;const LONo, LoadNo : Integer) ;
    procedure Download500FilesFromFTP(const LocalDir : String) ;
//    procedure Get500FilesFromFTP ;
    procedure FillDirList;

    procedure StoreTransferFile(const FileName, HTF_FS : String;const TransferType, LONo, LoadNo, InOut : Integer) ;
  ...
  public
    ...
    vouno, logno : Integer ;
    serie : String ;
    ldglogwrite_logerror,
    vourowlog_logerror : Integer ;
    procedure SummarizeWithOutOpenInvoiceForm ;
    function  GetTotalAM3PerLO(const InternalInvoiceNo, ShippingPlanNo : Integer) : Double ;
    Procedure CalcAndUpdateNetProductValueInInvoiceDetail ;
    function  GetFreigthCost(const InternalInvoiceNo, ShippingPlanNo, InclInPrice, InclInInvoiceTotal  : Integer) : Double ;
    procedure InsertVerifikatLogg (const Test : Boolean;const InvoiceNo, InternalInvoiceNo : Integer) ;
    ...
    procedure DeleteRecievedFiles ;
    procedure DeleteAttestRowKoppling (const InternalInvoiceNo, ShippingPlanNo, SequensNo, InvoiceDetailNo : Integer) ;
    procedure PrepareForAttest_DELKREDIT (const Quick_InvoiceNo : Integer) ;

procedure EXEC_xp_ldglogwrite (
...
const amount, foramount, vat : Double;
const vatcode : Integer;
const account : String;
const approval1 : String;
const logno, vouno : Integer;
const serie, Object2, Object5 : String) ;



//verifikationshuvud
procedure EXEC_sp_VIS_xp_voulogwrite (
const cid : String;
const logtype : Integer;
const logdate : TSQLTimeStamp;
const enumerator : String;
const invno : Integer;
Const FreeText  : String) ;

//verifikationsrad
procedure EXEC_xp_vourowlogwrite
( const logno : Integer;
const account : String;
const logdate : TSQLTimeStamp;
const currency : String;
const amount : double;
const foramount : double;
const vatcode : Integer;
const creator : String;
const Object2, Object5 : String;
Const NM3       : Double;
Const FreeText  : String)  ;

    procedure ChangeVISPaketInCaseDifferentNoOfPiecesFromHTF ;
    procedure SetTransferFileCompleteValue(const FileName : String;const TransferType, Complete : Integer) ;
    Function  HamtaEnUtleveransFran_PackageStatusHTF(const EfterfragadLastnr : Integer) : String ;
    procedure UpLoadNewFileToFTP (const UpOrDown : Integer) ;
    ...
    procedure HamtaFilerFranFTPOchUppdatera ;
    Function  GetDueDate(const InvoiceDate : TDateTime;const NoOfDaysDiscount2, DueDateBlank, FreeDelMonth  : Integer)  : TDateTime ;
    ...
    function  CreateDELCreditInvoiceByCopyDebitInvoice(const InvoiceNo, InternalInvoiceNo : Integer;const DatumString : String) : Integer ;
    ...
    procedure ExportTypSoftSet(const LoadNo, LONo, RecordType  : Integer;const LOTyp : String;const Purpose : Integer) ;
    ...
    function  CreateCreditInvoiceByCopyDebitInvoice(const InvoiceNo, InternalInvoiceNo : Integer;const DatumString : String) : Integer ;
...
Uses dmsDataConn, recerror, //UnitEnterInvoiceNo,
VidaConst, VidaUser,
  //fMoveInvPkgToInventory,
  dmc_ArrivingLoads,
  dmsVidaContact , uSelectMultInvoice,
  dmsVidaSystem, VidaUtils ,
...
Begin
 case cdsInvoiceHeadInvoiceType.AsInteger of
   ...
   AGENT_INVOICE                        : AssignAGENTInvoiceNumber(InternalInvoiceNo) ;
 End ;
End ;
...
Begin
 Try
 sq_GetOrgInvoiceNoByCredit.ParamByName('NewInternalInvoiceNo').AsInteger := InternalInvoiceNo ;
...
// Begin
 Try
  sq_PkgType_InvoiceForCredit.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
  ...
  Begin
   ...
   Begin
    ...
    //START A TRANSACTION
     ...
     Begin
      InsertInvoiceNo(StrToInt(FormEnterInvoiceNo.eFakturanr.Text), InternalInvoiceNo) ;
      if (cdsInvoiceHeadDebit_Credit.AsInteger = cCredit) and (cdsInvoiceHeadDelKredit.AsInteger = 0) then
      Begin
...
procedure TdmVidaInvoice.vis_CopyLoad(const InternalInvoiceNo, NewInternalInvoiceNo  : Integer) ;
Begin
 Try
 sp_vis_CopyLoad.ParamByName('@CreateUser').AsInteger           := ThisUser.UserID ;
 sp_vis_CopyLoad.ParamByName('@InternalInvoiceNo').AsInteger    := InternalInvoiceNo ;
 sp_vis_CopyLoad.ParamByName('@NewInternalInvoiceNo').AsInteger := NewInternalInvoiceNo ;
...
procedure TdmVidaInvoice.InsertInvoiceNo(const InvoiceNo, InternalInvoiceNo : Integer) ;
Begin
 Try
 ...
 sq_InsInvNo.ParamByName('DateCreated').AsSQLTimeStamp    := DateTimeToSQLTimeStamp(Now) ;
 sq_InsInvNo.ExecSQL ;
 except
  On E: Exception do
  Begin
...
procedure TdmVidaInvoice.RemoveLoadFromInvoice(const InternalInvoiceNo : Integer) ;
Begin
 if cdsInvoiceHeadSendInvoiceToAgent.AsInteger = 0 then //FULLKredit
 Begin
  Try
  sq_RemInvLoadII.ParamByName('NewInternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
...
//    formfMoveInvPkgToInventory.ShowModal ;
    ...
    Begin;
     ...
     cdsProformaInvNoProformaInvoiceNo.AsInteger  := StrToInt(FormEnterInvoiceNo.eFakturanr.Text) ;
 ...
 Begin
  ...
  Begin
    ...
    Begin;
     ...
     cds_PurchaseInvNoPO_InvoiceNo.AsInteger      := StrToInt(FormEnterInvoiceNo.eFakturanr.Text) ;
...
Begin
 ...
 Begin
  ...
  Begin
    ...
    formfMoveInvPkgToInventory:= TformfMoveInvPkgToInventory.Create(Nil);
    Try
    formfMoveInvPkgToInventory.LONo               := cdsInvoiceLOShippingPlanNo.AsInteger ;
    formfMoveInvPkgToInventory.InternalInvoiceNo  := InternalInvoiceNo ;
...
function TdmVidaInvoice.CreateCreditInvoiceByCopyDebitInvoice(const InvoiceNo, InternalInvoiceNo : Integer;const DatumString : String) : Integer ;
Var
...
//  Begin
  ...
  cdsInvoiceHead.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
  cdsInvoiceHead.Open ;
...
//copy and insert InvoiceLO
  cdsInvoiceLO.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
...
//   cdsInvoiceDetailVatValue.AsFloat                             := cdsInvoiceDetailVatValue.AsFloat * (-1) ;
   cdsInvoiceDetailintNM3.AsFloat                               := cdsInvoiceDetailintNM3.AsFloat * (-1) ;
   cdsInvoiceDetailNoOfPieces.AsInteger                         := cdsInvoiceDetailNoOfPieces.AsInteger * (-1) ;
   cdsInvoiceDetailLinealMeter.AsFloat                          := cdsInvoiceDetailLinealMeter.AsFloat * (-1) ;
   cdsInvoiceDetailNominalM3.AsFloat                            := cdsInvoiceDetailNominalM3.AsFloat * (-1) ;
   cdsInvoiceDetailActualNetM3.AsFloat                          := cdsInvoiceDetailActualNetM3.AsFloat * (-1) ;
   cdsInvoiceDetailVolume_OrderUnit.AsFloat                     := cdsInvoiceDetailVolume_OrderUnit.AsFloat * (-1) ;
   cdsInvoiceDetailProductValueWOFreight.AsFloat                := cdsInvoiceDetailProductValueWOFreight.AsFloat * (-1) ;
   cdsInvoiceDetailNoOfPkgs.AsFloat                             := cdsInvoiceDetailNoOfPkgs.AsFloat * (-1) ;
   cdsInvoiceDetailVatValue.AsFloat                             := cdsInvoiceDetailVatValue.AsFloat * (-1) ;
   cdsInvoiceDetailFreightCostCurr.AsFloat                      := cdsInvoiceDetailFreightCostCurr.AsFloat  * (-1) ;
...
//copy and insert InvoiceDetail
  ...
  cdsInvoiceShipTo.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
...
Begin
 ...
 Case InvoiceType of
  0 : Begin
      cdsInvoiceHead_II.SQL.Add('Inner Join dbo.InvoiceNumber invNo on invNo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
      End ;
  1,2 : cdsInvoiceHead_II.SQL.Add('Inner Join dbo.ProformaInvoiceNumber invNo on invNo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  3 :   cdsInvoiceHead_II.SQL.Add('Inner Join dbo.InvoiceNumber_PO invNo on invNo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  4 :   cdsInvoiceHead_II.SQL.Add('Inner Join dbo.InvoiceNo_USA invNo on invNo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  5 :   cdsInvoiceHead_II.SQL.Add('Inner Join dbo.InvNo_FW invNo on invNo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
 End ;
 cdsInvoiceHead_II.SQL.Add('WHERE') ;
 cdsInvoiceHead_II.SQL.Add('IH.InvoiceType = '+inttostr(InvoiceType)) ;
 cdsInvoiceHead_II.SQL.Add('AND IH.InternalInvoiceNo = '+inttostr(InvoiceNo)) ;
...
Begin
 ...
 Case InvoiceType of
  0 :   cds_NoOfInvoices.SQL.Add('INO.InvoiceNo AS Fakturnr,') ;
  1 :   cds_NoOfInvoices.SQL.Add('INO.ProformaInvoiceNo AS Fakturnr,') ;
  2 :   cds_NoOfInvoices.SQL.Add('INO.ProformaInvoiceNo AS Fakturnr,') ;
  3 :   cds_NoOfInvoices.SQL.Add('INO.PO_InvoiceNo AS Fakturnr,') ;
  4 :   cds_NoOfInvoices.SQL.Add('INO.InvoiceNo AS Fakturnr,') ;
  5 :   cds_NoOfInvoices.SQL.Add('INO.InvoiceNo AS Fakturnr,') ;
 End ;//Case
 cds_NoOfInvoices.SQL.Add('IL.ShippingPlanNo AS LO, IH.InvoiceDate AS Fakturadatum, IL.InternalInvoiceNo') ;
 cds_NoOfInvoices.SQL.Add('FROM') ;
 cds_NoOfInvoices.SQL.Add('dbo.InvoiceLO IL') ;
 cds_NoOfInvoices.SQL.Add('Inner Join dbo.InvoiceHeader IH ON IH.InternalInvoiceNo = IL.InternalInvoiceNo') ;
 Case InvoiceType of
  0 : cds_NoOfInvoices.SQL.Add('Inner Join dbo.InvoiceNumber INo ON INo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  1 : cds_NoOfInvoices.SQL.Add('Inner Join dbo.ProformaInvoiceNumber INo ON INo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  2 : cds_NoOfInvoices.SQL.Add('Inner Join dbo.ProformaInvoiceNumber INo ON INo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  3 : cds_NoOfInvoices.SQL.Add('Inner Join dbo.InvoiceNumber_PO INo ON INo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  4 : cds_NoOfInvoices.SQL.Add('Inner Join dbo.InvoiceNo_USA INo ON INo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
  5 : cds_NoOfInvoices.SQL.Add('Inner Join dbo.InvNo_FW INo ON INo.InternalInvoiceNo = IH.InternalInvoiceNo') ;
 End ;
...
// dsp_LoadPackages.DataSet   := sq_LoadPackages ;
 cds_LoadPackages.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
...
begin
  ...
  cds_LoadPackagesII.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
...
begin
  ...
  cds_LoadPackagesIII.ParamByName('InternalInvoiceNo').AsInteger:= InternalInvoiceNo ;
...
Begin
 ...
 cds_ExportTyp1.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
...
procedure TdmVidaInvoice.StoreTransferFile(const FileName, HTF_FS : String;const TransferType, LONo, LoadNo, InOut : Integer) ;
Begin
 Try
 ...
 sq_StoreTransferFile.ParamByName('CreatedUser').AsInteger  := ThisUser.UserID ;
...
//  cds_LoadPackages.Post ;
  ...
  Begin
   LastInvNr  := dmsSystem.IsRegDateBeforeInvDate(cds_LoadPackagesLIPNo.AsInteger, SQLTimeStampToDateTime(cdsInvoiceHeadInvoiceDate.AsSQLTimeStamp), InvDate, MaxDateMsg) ;
   if LastInvNr > -1 then
   Begin
...
begin
 cdsInvoiceDetailInternalInvoiceNo.AsInteger          := cdsInvoiceHeadInternalInvoiceNo.AsInteger ;
 cdsInvoiceDetailInvoiceDetailNo.AsInteger            := dmsConnector.NextSecondMaxNo('Invoice', cdsInvoiceHeadInternalInvoiceNo.AsInteger) ;
...
Begin
 ...
 Begin
  ...
  cds_KontoLogik.ParamByName('SalesmangroupNo').AsInteger    := cdsInvoiceHeadSalesGroupNo.AsInteger ;
  cds_KontoLogik.ParamByName('SalesregionNo').AsInteger      := cdsInvoiceHeadSupplierNo.AsInteger ;
  cds_KontoLogik.ParamByName('CustomerCountryNo').AsInteger  := GetCountryRegion(cdsInvoiceHeadSupplierNo.AsInteger, cdsInvoiceHeadCountryNo.AsInteger) ;
  cds_KontoLogik.ParamByName('VarugruppNo').AsInteger        := cdsInvoiceDetailArticleNo.AsInteger ;
  cds_KontoLogik.ParamByName('Moms').AsInteger               := cdsInvoiceHeadMoms.AsInteger ;

  if cdsInvoiceHeadTrading.AsInteger = 1 then
  Begin
   PO_CountryNo                                               := GetPOCountryNo(cdsInvoiceDetailShippingPlanNo.AsInteger) ;
   cds_KontoLogik.ParamByName('Trading').AsInteger            := cdsInvoiceHeadTrading.AsInteger ;
  ...
  Begin
   ...
   PO_CountryNo                                               := GetPOCountryNoPartII(cdsInvoiceDetailShippingPlanNo.AsInteger) ;
   cds_KontoLogik.ParamByName('Trading').AsInteger            := cdsInvoiceHeadTrading.AsInteger ;
   cds_KontoLogik.ParamByName('PO_CountryNo').AsInteger       := PO_CountryNo ;
   cds_KontoLogik.Active := True ;
  ...
  Begin
   ...
   cdsInvoiceDetailKonto.AsString       := cds_KontoLogikForsaljningsKonto.AsString ;
   cdsInvoiceDetailVatCode.AsInteger    := cds_KontoLogikVatCode.AsInteger ;
   cdsInvoiceDetailmoms_konto.AsString  := cds_KontoLogikMomsKonto.AsString ;
   cdsInvoiceDetail.Post ;
  End
  else
  Begin
...
Function TdmVidaInvoice.GetCountryRegion(const SalesRegionNo, CountryNo  : Integer) : Integer ;
Var Region  : String ;
...
Begin
 ...
 if SalesRegionNo = VIDA_WOOD_COMPANY_NO then
 Begin
  if CountryNo = Sweden then
   Result := Sweden
    else
...
function TdmVidaInvoice.CreateDELCreditInvoiceByCopyDebitInvoice(const InvoiceNo, InternalInvoiceNo : Integer;const DatumString : String) : Integer ;
Var
 NewInternalInvoiceNo, i,x       : Integer ;
 InvoiceHead                     : Array of variant ;
 InvoiceLO                       : Array of array of variant ;
...
//copy and insert InvoiceLO
  ...
  While not cdsInvoiceLO.Eof do
  Begin
   if amt_Credit.Locate('LONo', cdsInvoiceLOShippingPlanNo.AsInteger, []) then
   Begin
    SetLength(InvoiceLO, x) ;
    SetLength(InvoiceLO[x-1], cdsInvoiceLO.FieldCount) ;
    for i:= 0 to cdsInvoiceLO.FieldCount-1 do
...
procedure TdmVidaInvoice.LoadMemTableWithImportPackages550(const FileName : String);
begin
 dmVidaInvoice.mtImport.Active  := False ;
 dmVidaInvoice.mtImport.Active  := True ;
...
procedure TdmVidaInvoice.GetPackageStatusOfHampenTEST550(const FileName : String) ;
Begin
...
Function TdmVidaInvoice.GetDueDate(const InvoiceDate : TDateTime;const NoOfDaysDiscount2, DueDateBlank, FreeDelMonth  : Integer)  : TDateTime ;
Begin
...
begin
 ...
 sq_TransferFiles.ParamByName('TransferType').AsInteger := -1 ;//ALLA
 sq_TransferFiles.ParamByName('Complete').AsInteger     := 0 ;
 sq_TransferFiles.ParamByName('InOut').AsInteger        := 0 ; //0 = "Leverans från Hampen"
 sq_TransferFiles.Active := True ;
 sq_TransferFiles.First ;
 Try
 While not sq_TransferFiles.Eof do
 Begin
  LoadMemTableWithImportPackages(LocalDir + sq_TransferFilesFileName.AsString) ;

  //Lägger in paketnr i tabellen PackageStatusHTF
  UpdateHTFPackages(sq_TransferFilesFileName.AsString, sq_TransferFilesLONo.AsInteger, sq_TransferFilesLoadNo.AsInteger) ;
...
procedure TdmVidaInvoice.UpdateHTFPackages(const FileName : String;const LONo, LoadNo : Integer) ;
...
Begin
 ...
 if Length(mtImportKlarForLeverans.AsString) > 0 then
 Begin
  Result      := 4 ;
  StatusDate  := mtImportKlarForLeverans.AsString ;
 End
 else
 if Length(mtImportProduktionsDatum.AsString) > 0 then
 Begin
  Result      := 3 ;
  StatusDate  := mtImportProduktionsDatum.AsString ;
 End
 else
 if Length(mtImportKlarTillProduktion.AsString) > 0 then
 Begin
  Result      := 2 ;
  StatusDate  := mtImportKlarTillProduktion.AsString ;
 End
 else
 if Length(mtImportMottaget.AsString) > 0 then
 Begin
  Result      := 1 ;
...
//UpdateHTFPackages
 ...
 Begin
  ...
  sq_PackageStatusHTF.ParamByName('Paketnr').AsInteger    := mtImportPaketnr.AsInteger ;
  sq_PackageStatusHTF.ParamByName('Prefix').AsString      := mtImportPrefix.AsString ;
  sq_PackageStatusHTF.ParamByName('FileName').AsString    := FileName ;
  sq_PackageStatusHTF.ParamByName('RecordType').AsInteger := mtImportRecordType.AsInteger ;
  ...
  Begin
   ...
   and (not mtImportPaketnr.Isnull) and  (mtImportPaketnr.AsInteger > 0) then
   Begin
...
//    SQLDate := sq_PackageStatusHTFStatusDatum.AsSQLTimeStamp ;
   ...
   sq_PackageStatusHTFOriginalStyck.AsInteger     := dmsSystem.GetCurrentStyckInPackage(mtImportPaketnr.AsInteger, mtImportPrefix.AsString, PackageTypeNo, ProductNo, ALMM) ;
   ...
   sq_PackageStatusHTFHTF_FS.AsString             := mtImportUtlevereringsJournal.AsString ;
...
procedure TdmVidaInvoice.UpdateHTFPackages550(const FileName : String;const LONo, LoadNo : Integer) ;
Var StatusDate    : String ;
    SQLDate       : TSQLTimeStamp ;
    PackageStatus : Integer ;
    PackageTypeNo : Integer ;
...
//   sq_PackageStatusHTFOriginalStyck.AsInteger     := -1 ;
   ...
   sq_PackageStatusHTFHTF_FS.AsString             := mtImportUtlevereringsJournal.AsString ;
   sq_PackageStatusHTFProductCode.AsString        := mtImportProduktKod.AsString ;
   sq_PackageStatusHTFProductNo.AsInteger         := dmsSystem.GetProductNoByProductCode(sq_PackageStatusHTFProductCode.AsString) ;
   sq_PackageStatusHTFPEFC.AsInteger              := mtImportPEFC.AsInteger ;
...
procedure TdmVidaInvoice.CreateLoadAndEntryTo_Load_Imp(const LONo, LoadNo : Integer) ;
Var Save_Cursor   : TCursor ;
...
Begin
 ...
 sq_GetLDLIPNo.ParamByName('PackageNo').AsInteger     := sq_PackageStatusHTFByFileNamePaketnr.AsInteger ;
  sq_GetLDLIPNo.ParamByName('SupplierCode').AsString  := sq_PackageStatusHTFByFileNamePrefix.AsString ;
...
//Lägg till Paket som INTE finns i lasten men i htf
 ...
 Begin
  if not sq_LD.FindKey([sq_PackageStatusHTFByFileNamePaketnr.AsInteger, sq_PackageStatusHTFByFileNamePrefix.AsString]) then
  Begin
   sq_LD.Insert ;
   ...
   sq_LDPackageTypeNo.AsInteger   := sq_PackageStatusHTFByFileNamePackageTypeNo.AsInteger ;
  ...
  if sq_LDPackageTypeNo.AsInteger <> sq_PackageStatusHTFByFileNamePackageTypeNo.AsInteger then
  Begin
   sq_LD.Edit ;
//   sq_LDPackageNo.AsInteger       := sq_PackageStatusHTFByFileNamePaketnr.AsInteger ;
//   sq_LDSupplierCode.AsString     := sq_PackageStatusHTFByFileNamePrefix.AsString ;
...
Function TdmVidaInvoice.GetFileNameOfLoadNoFileName(const LoadNo : Integer) : String ;
Begin
...
Function TdmVidaInvoice.HamtaEnUtleveransFran_PackageStatusHTF(const EfterfragadLastnr : Integer) : String ;
Var //LocalDir      : String ;
    Save_Cursor   : TCursor ;
begin
 Save_Cursor := Screen.Cursor;
...
//RESKONTRA
procedure TdmVidaInvoice.EXEC_xp_ldglogwrite (
const cid, person : String;
const stype : Integer;
const enum : String;
const invno, seq : Integer ;
...
Try
 ...
 sp_xp_ldglogwrite.ParamByName('@org').AsString            := org ; //Ursprungligt fakturanummer., varchar 30
 sp_xp_ldglogwrite.ParamByName('@bookdate').AsSQLTimeStamp := Bookdate ; //datetime
 sp_xp_ldglogwrite.ParamByName('@invdate').AsSQLTimeStamp  := invdate ; //datetime
 sp_xp_ldglogwrite.ParamByName('@duedate').AsSQLTimeStamp  := duedate ; //datetime
 sp_xp_ldglogwrite.ParamByName('@currency').AsString       := currency ; //varchar 3
 ...
 sp_xp_ldglogwrite.ParamByName('@vatcode').AsSmallInt      := vatcode ; //tinyint
...
// sp_xp_ldglogwrite.ParamByName('mop').AsString:= mop ; //varchar 8
 sp_xp_ldglogwrite.ParamByName('@approval1').AsString      := approval1 ; //varchar 8
...
//VERIFIKATIONSHUVUD
procedure TdmVidaInvoice.EXEC_sp_VIS_xp_voulogwrite (
const cid : String;
const logtype : Integer;
const logdate : TSQLTimeStamp;
const enumerator : String;
...
// ShowMessage('procedure TdmVidaInvoice.EXEC_sp_VIS_xp_voulogwrite');
 ...
 sp_VIS_xp_voulogwrite.ParamByName('@logdate').AsSQLTimeStamp  := logdate ; //datetime Bokföringsdag
 sp_VIS_xp_voulogwrite.ParamByName('@enumerator').AsString     := enumerator ; //Reskontraserie tex VWK1
...
//VERIFIKATIONSRAD
procedure TdmVidaInvoice.EXEC_xp_vourowlogwrite
( const logno : Integer;
const account : String;
const logdate : TSQLTimeStamp;
const currency : String;
...
// ShowMessage('Before EXEC_xp_vourowlogwrite');
 Try
 sp_VIS_xp_vourowlogwrite.ParamByName('@logno').AsInteger      := logno ;//From temp table
...
// sp_VIS_xp_vourowlogwrite.ParamByName('logtype').AsInteger:= 3 ;//Var loggen kommer ifrån, 3 ?
 sp_VIS_xp_vourowlogwrite.ParamByName('@account').AsString     := account ; //Konto. Refererar till account.account. tex 1510=Kundfordringar
 sp_VIS_xp_vourowlogwrite.ParamByName('@date').AsSQLTimeStamp  := logdate ; //Transaktionsdatum
 sp_VIS_xp_vourowlogwrite.ParamByName('@currency').AsString    := currency ; //Valuta. Refererar till currency.currency.
 ...
 sp_VIS_xp_vourowlogwrite.ParamByName('@number').AsFloat       := NM3 ;   //antal av vadå?
 ...
 sp_VIS_xp_vourowlogwrite.ParamByName('@o2').AsString          := Object2 ; //varchar 8
 sp_VIS_xp_vourowlogwrite.ParamByName('@o5').AsString          := Object5 ; //varchar 8
 sp_VIS_xp_vourowlogwrite.ParamByName('@FreeText').AsString    := FreeText ; //varchar
 sp_VIS_xp_vourowlogwrite.ExecProc ;
 vourowlog_logerror := sp_VIS_xp_vourowlogwrite.ParamByName('@logerror').AsInteger ;
     except
      On E: Exception do
      Begin
       dmsSystem.FDoLog(E.Message) ;
...
procedure TdmVidaInvoice.cdsInvoiceDetailVolume_OrderUnitChange(
  Sender: TField);
begin
  if (cdsInvoiceHeadQuickInvoice.AsInteger = 1)
  or (cdsInvoiceHeadSendInvoiceToAgent.AsInteger = 1) then
...
procedure TdmVidaInvoice.PrepareForAttest_DELKREDIT(const Quick_InvoiceNo : Integer) ;
 Var  Save_Cursor   : TCursor;
      TotalAmount   : Double ;
      OldFilter     : String ;
...
Function  TdmVidaInvoice.GetPOCountryNo (const SalesShippingPlanNo : Integer) : Integer ;
Begin
 sq_GetPOCountry.ParamByName('SalesShippingPlanNo').AsInteger  := SalesShippingPlanNo ;
 Try
 sq_GetPOCountry.Open ;
 if not sq_GetPOCountry.Eof then
...
procedure TdmVidaInvoice.DeleteAttestRowKoppling (const InternalInvoiceNo, ShippingPlanNo, SequensNo, InvoiceDetailNo : Integer) ;
Begin
 Try
 sq_Del_AttestKoppling.ParamByName('InternalInvoiceNo').AsInteger   := InternalInvoiceNo ;
 sq_Del_AttestKoppling.ParamByName('ShippingPlanNo').AsInteger      := ShippingPlanNo ;
 sq_Del_AttestKoppling.ParamByName('SequensNo').AsInteger           := SequensNo ;
 sq_Del_AttestKoppling.ParamByName('InvoiceDetailNo').AsInteger     := InvoiceDetailNo ;
...
function TdmVidaInvoice.GetNettoPris(const OrderNo, OrderLineNo : Integer) : Double ;
Begin
...
function TdmVidaInvoice.GetBruttoPris(const OrderNo, OrderLineNo : Integer) : Double ;
Begin
...
begin
 ...
 Begin
  ...
  Begin
   ...
   cdsInvoiceDetailProductValue.AsFloat  := Abs(cdsInvoiceDetailProductValue.AsFloat) * -1 ;
  End
  else
  Begin
   cdsInvoiceDetailTypeOfRow.AsInteger := 2 ;
   cdsInvoiceDetailProductValue.AsFloat  := Abs(cdsInvoiceDetailProductValue.AsFloat) ;
  End ;
 End
 else
 Begin
...
function TdmVidaInvoice.GetFreigthCost(const InternalInvoiceNo, ShippingPlanNo, InclInPrice, InclInInvoiceTotal  : Integer) : Double ;
Begin
 Try
 sq_GetTotalFreigthInPrice.ParamByName('InternalInvoiceNo').AsInteger   := InternalInvoiceNo ;
 sq_GetTotalFreigthInPrice.ParamByName('ShippingPlanNo').AsInteger      := ShippingPlanNo ;
 sq_GetTotalFreigthInPrice.ParamByName('InclInPrice').AsInteger         := InclInPrice ;
...
function TdmVidaInvoice.GetTotalAM3PerLO(const InternalInvoiceNo, ShippingPlanNo : Integer) : Double ;
Begin
 Try
 sq_GetTotalAM3PerLO.ParamByName('InternalInvoiceNo').AsInteger   := InternalInvoiceNo ;
 sq_GetTotalAM3PerLO.ParamByName('ShippingPlanNo').AsInteger      := ShippingPlanNo ;
...
Var TotalFraktKostnadPerLOIncludedInPrice,
    TotalAM3LO,
...
//  sq_LONoInInvoice.First ;
  ...
  Begin
   ...
   TotalFraktKostnadPerLOIncludedInPrice  := GetFreightCostInCDS(1) ; //dmVidaInvoice.GetFreigthCost(cdsInvoiceHeadInternalInvoiceNo.AsInteger, sq_LONoInInvoiceShippingPlanNo.AsInteger, 1, {InclInPrice} 0 {InclInInvoiceTotal}) ;
   TotalAM3LO                             := GetAM3PerLO ;// GetTotalAM3PerLO(cdsInvoiceHeadInternalInvoiceNo.AsInteger, sq_LONoInInvoiceShippingPlanNo.AsInteger) ;
   cdsInvoiceDetail.Filter    := 'ShippingPlanNo = ' + cdsInvoiceLOShippingPlanNo.AsString + ' AND (ArticleNo = 1 OR ArticleNo = 3)' ;
   cdsInvoiceDetail.Filtered  := True ;
   cdsInvoiceDetail.First ;
   Try
   While not cdsInvoiceDetail.Eof do
...
Begin
 ...
 begin
  OldFilter := cdsInvoiceDetail.Filter ;
  Try
   cdsInvoiceDetail.Filter:= '(ShippingPlanNo = ' + cdsInvoiceLOShippingPlanNo.AsString
   + ' AND ((ArticleNo = 2) OR (ArticleNo = 4))) '
   + ' AND InclInPrice = ' + inttostr(InclInPrice) ;
   cdsInvoiceDetail.Filtered  := True ;
   cdsInvoiceDetail.First ;
   While not cdsInvoiceDetail.Eof do
   Begin
...
//  SavePlace:= cdsInvoiceDetail.GetBookmark ;
 ...
 Begin
  ...
  if ((cdsInvoiceDetailArticleNo.AsInteger = 2) or (cdsInvoiceDetailArticleNo.AsInteger = 4)) then
   cdsInvoiceHeadSUM_FreigthCost.AsFloat := cdsInvoiceHeadSUM_FreigthCost.AsFloat + Abs(cdsInvoiceDetailProductValue.AsFloat) * -1 ;
  cdsInvoiceDetail.Next ;
 End ;
