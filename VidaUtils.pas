...
//
// -----------------------------------------------------------------------------
//function ColumnIsGrouped(Col: TdxTreeListColumn; Grid: TCustomdxDBGrid) : Boolean;
...
begin
  Result := False;
  iGrpCol := 0;
  while (iGrpCol < Grid.GroupColumnCount)
        and
        not Result do
  begin
    if Grid.GroupColumns[iGrpCol].Index = Col.Index then
      Result := true
    else
      Inc(iGrpCol)
  end;
