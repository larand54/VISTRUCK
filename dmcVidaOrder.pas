...
type
  ...
  public
    ...
    Function  LoadStatusOK(const SupplierNo : Integer) : Boolean ;
...
procedure TdmcOrder.SetIntHdrStatus (const sspNo, Status : Integer) ;
Begin
 sp_SetIntHdrStatus.ParamByName('@sspNo').AsInteger     := sspNo ;
 sp_SetIntHdrStatus.ParamByName('@Status').AsInteger    := Status ;
 Try
 sp_SetIntHdrStatus.ExecProc ;
except
  On E: Exception do
  Begin
   dmsSystem.FDoLog(E.Message) ;
...
Function TdmcOrder.LoadStatusOK(const SupplierNo : Integer) : Boolean ;
Begin
 Result := True ;
...
begin
 ...
 upd_LO.Apply(ARequest, AAction, AOptions) ;
...
function TdmcOrder.LO_Status(const ShippingPlanNo: Integer) : Integer ;
Begin
 Result  := 0 ;
 sp_GetLO_Avrop_Status.ParamByName('@LONo').AsInteger  := ShippingPlanNo ;
 sp_GetLO_Avrop_Status.Active := True ;
 Try
 if not sp_GetLO_Avrop_Status.Eof then
 Begin
  while not sp_GetLO_Avrop_Status.Eof do
  Begin
   if Result = 0 then
   Begin
    if sp_GetLO_Avrop_Status.FieldByName('SSP_STATUS').AsInteger in [1,3,6,9] then
     Result  := 1
      else
       Result  := 0 ;
   End;
    ...
    if Result = 1 then
    Begin
     if (sp_GetLO_Avrop_Status.FieldByName('AVROP_STATUS').IsNull)
     or (sp_GetLO_Avrop_Status.FieldByName('AVROP_STATUS').AsInteger = 1) then
      Result := 3
       else
        Result := 2 ;
    End ;
   sp_GetLO_Avrop_Status.Next ;
  End ;
 End
 else
 Result := -1 ;
 Finally
  sp_GetLO_Avrop_Status.Active  := False ;
 End ;
End ;

function TdmcOrder.GetNewPackage_Size(var PackageSizeName : String) : Integer ;
