...
uses
  ...
  dxSkinsdxBarPainter,  dxCore, cxNavigator, dxSkinMetropolis,
  dxSkinMetropolisDark, dxSkinOffice2013DarkGray, dxSkinOffice2013LightGray,
  dxSkinOffice2013White, siComp, siLngLnk, System.Actions ;

type
  TfVardaBortPkt = class(TForm)
    ...
    siLangLinked_fVardaBortPkt: TsiLangLinked;
...
procedure TfVardaBortPkt.CreateCo ;
var
  Save_Cursor:TCursor;
begin
...
procedure TfVardaBortPkt.CloseDataSets;
begin
   With dmPkgs do
   Begin
...
procedure TfVardaBortPkt.lbCloseClick(Sender: TObject);
begin
 Close ;
end;

procedure TfVardaBortPkt.AddPkgToGrid(Sender: TObject;PkgNo : Integer;PkgSupplierCode : String3;ProductNo : Integer)  ;
var
 x : Integer ;
 Res_UserName : String ;
Begin
...
function TfVardaBortPkt.IdentifyPackageSupplier(
  const PkgNo : Integer;
  var PkgSupplierCode: String3;
  Var ProductNo : Integer;
  Var Res_UserName : String) : TEditAction;
...
procedure TfVardaBortPkt.FormCreate(Sender: TObject);
begin
...
procedure TfVardaBortPkt.FormCloseQuery(Sender: TObject;var CanClose: Boolean);
begin
 CanClose:= True ;
...
procedure TfVardaBortPkt.lbPkgInfoClick(Sender: TObject);
...
procedure TfVardaBortPkt.InsertPkgSerie(Sender: TObject) ;
...
procedure TfVardaBortPkt.FormDestroy(Sender: TObject);
begin
 FreeAndNil(dmPkgs) ; //Free ;
end;

procedure TfVardaBortPkt.bbRemoveAllRowsClick(Sender: TObject);
begin
 //Remove all rows
 With dmPkgs do
 Begin
...
procedure TfVardaBortPkt.bbClearPkgGridClick(Sender: TObject);
begin
 bbRemoveAllRowsClick(Sender) ;
end;

procedure TfVardaBortPkt.RemoveAllSuccespkgs(Sender: TObject);
begin
 //Remove all rows
 With dmPkgs do
 Begin
...
procedure TfVardaBortPkt.ePktNrKeyPress(Sender: TObject; var Key: Char);
begin
 With dmPkgs do
 Begin
  if Key = #13 then
...
procedure TfVardaBortPkt.ScanningPkgNo(Sender: TObject; PkgNo : String) ;
const
  LF = #10;
var
  NewPkgNo        : Integer ;
...
{function TfVardaBortPkt.AddPkgTo_mtPackage(Sender: TObject;const LIP, PkgNo : Integer;PkgSupplierCode : String3;ProductNo : Integer) : TEditAction ;
var
 x : Integer ;
// Res_UserName : String ;
Begin
...
procedure TfVardaBortPkt.ScanningEgnaPkgNo(Sender: TObject; PkgNo : String) ;
const
  LF = #10;
var
  NewPkgNo          : Integer ;
...
procedure TfVardaBortPkt.mtUserPropOwnerNoChange(Sender: TField);
begin
...
procedure TfVardaBortPkt.mtUserPropPIPNoChange(Sender: TField);
begin
 With dm_UserProps do
 Begin
...
procedure TfVardaBortPkt.mtUserPropProducerNoChange(Sender: TField);
begin
 With dm_UserProps do
 Begin
...
procedure TfVardaBortPkt.mtUserPropAfterInsert(DataSet: TDataSet);
begin
...
procedure TfVardaBortPkt.acAvregistreraPaketExecute(Sender: TObject);
const
  LF = #10;
Begin
...
procedure TfVardaBortPkt.acStartBarCodeScanningExecute(Sender: TObject);
begin
...
procedure TfVardaBortPkt.acCloseExecute(Sender: TObject);
begin
 Close ;
end;

procedure TfVardaBortPkt.acAddRowExecute(Sender: TObject);
begin
 dmPkgs.mtLoadPackages.Append ;
 grdPkgs.SetFocus ;
end;

procedure TfVardaBortPkt.acRemoveRowExecute(Sender: TObject);
begin
 // remove one row
 With dmPkgs do
 if ((mtLoadPackages.Active) AND (mtLoadPackages.RecordCount > 0))
...
procedure TfVardaBortPkt.acDeletePkgFromSystemExecute(Sender: TObject);
begin
(* if mtUserProp.State in [dsEdit, dsInsert] then
  mtUserProp.Post ;
...
procedure TfVardaBortPkt.acRemoveRowUpdate(Sender: TObject);
begin
 // remove one row
 With dmPkgs do
...
procedure TfVardaBortPkt.acAddRowUpdate(Sender: TObject);
begin
...
procedure TfVardaBortPkt.acAvregistreraPaketUpdate(Sender: TObject);
begin
 With dmPkgs do
 Begin
...
procedure TfVardaBortPkt.RemotePkgEntry (PkgNos : TkbmMemTable) ;
Var     //NoOfPkgsInSerie,
        x                   : Integer ;
        ResultButton        : word ;
        Save_Cursor         : TCursor;
...
procedure TfVardaBortPkt.acSearchRunNoExecute(Sender: TObject);
...
procedure TfVardaBortPkt.acRemoveAllRowsExecute(Sender: TObject);
begin
 // remove one row
 With dmPkgs do
 if ((mtLoadPackages.Active) AND (mtLoadPackages.RecordCount > 0))
...
procedure TfVardaBortPkt.acRemoveAllRowsUpdate(Sender: TObject);
begin
 With dmPkgs do
...
function TfVardaBortPkt.ControlInvDate(Sender: TObject) : Boolean ;
const
  LF = #10;
Var
    LastInvNr   : Integer ;
...
procedure TfVardaBortPkt.grdPkgsDBBandedTableView1StylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
var
