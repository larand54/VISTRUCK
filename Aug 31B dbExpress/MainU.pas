...
type
  ...
  TXMLImportExport = class(TForm)
    ...
    procedure acExportTallyExecute(Sender: TObject);
  private
    WoodXDir, ImportDir, DeliveryWoodMsg_XSD_File : String ;
    procedure DoExportTally(const aLoadNo: integer) ;
   ...
   Public
    ...
    LoadNo, InvoiceNo, CustomerNo, InternalInvoiceNo : Integer ;
...
procedure TXMLImportExport.DoExportTally(const aLoadNo: integer);
begin
  PackageExportU.InternalInvoiceNo := aLoadNo;
  sq_GetLONos.Close;
  sq_GetLONos.Parameters.ParamByName('InternalInvoiceNo').Value := aLoadNo;
  sq_GetLONos.Open;
  if ComputerName = 'CARMAK-SPEED' then
    WoodXDir := 'C:\Temp\'
     else
      WoodXDir  := Path ;
  ...
  dmsSystem.DeliveryMessageNumber := sq_GetLONosDeliveryMessageNumber.AsString; //DeliveryMessageNumber = LONo
//LOKAL DIR  OutPutPackageFileName           := 'C:\woodx\' + 'InvoicePackageSpec InvoiceNo-LoadOrderNo' + IntToStr(InvoiceNo) +'-'+ dmsSystem.DeliveryMessageNumber +'.xml' ;
  OutPutPackageFileName := WoodXDir + 'Deliverymessage DeliveryMessageNumber ' + IntToStr(aLoadNo) + '.xml';
  CleareDoc;
...
procedure TXMLImportExport.acExportTallyExecute(Sender: TObject);
begin
  dmsConnector.SaveandSetCursor(crSQLWait);    { Show hourglass cursor }
  Try
  // Stänger av för att xml filerna skall gå över biztalken
  dmsSystem.RunLengthSpec ;
  DoSetPropValue(Sender, 'Enabled', False);
  try
    DoExportTally(LoadNo);
  finally
    DoSetPropValue(Sender, 'Enabled', True);
  end;
 ...
 Finally
  dmsConnector.RestoreCursor;  { Always restore to normal }
 End ;
end;
...
var
 // FormCRExportOneReport: TFormCRExportOneReport;
  ...
  mailTitle, mailMessage, MailFrom, MailToList: string;
  x: Integer;
...
begin
  ...
  Begin
    MailToList := dmsContact.GetEmailAddress(CustomerNo);
    ...
    begin
      ...
      TOAuthMail.OASendMail(udmFR.dmFR.getParams, mailTitle, mailMessage, MailFrom, MailToList, '', Attach, false );
