...
uses
  ...
  Math, StdCtrls,
  Menus, DBXpress, DB, DBCtrls, SqlTimSt, dxBar,
  Grids,
  DBGrids,
...
type
  TfrmAvrop = class(TForm)
    ...
    procedure acExpLoadExecute(Sender: TObject);
    ...
    procedure acNewDeliveryRequestExecute(Sender: TObject);
...
//    function  FindAvrop(Sender : TObject) : Boolean ;
    ...
    procedure Refresh_Avrop(Sender: TObject;const LONo : String;const ErReferens : String);
...
uses
  ...
  VidaUser,
  VidaUtils, //fInvoice,
  dmsDataConn, UnitdmModule1, dmLM1, UnitBookingForm,
  ...
  dmcLoadEntryCSDTemp, UnitLoadEntryCSDTemp,
  uExportLoadPurpose, uWait;


procedure TfrmAvrop.CreateCo(CompanyNo: Integer);
begin
  // Load column widths set last time.
  if dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name+'/'+grdcxAvrop.Name, grdcxAvropDBBandedTableView1) = False then ;
  if dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name+'/'+grdLoad.Name, grdLoadDBTableView1) = False then ;
  if dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name+'/'+grdLO.Name, grdLODBTableView1) = False then ;
  if dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name+'/'+grdAddLO.Name, grdAddLODBTableView1) = False then ;
...
procedure TfrmAvrop.Refresh_Avrop(Sender: TObject;const LONo : String;const ErReferens : String);
var
  Save_Cursor : TCursor;
begin
 With daMoLM1 do
...
//Välj alla lastrader
  ...
  Try
   ADataSet := grdcxAvropDBBandedTableView1.DataController.DataSource.DataSet ;
//   BKMSafe := ADataSet.Bookmark ;
   For I := 0 to grdcxAvropDBBandedTableView1.Controller.SelectedRecordCount - 1 do
   Begin
    RecIDx:= grdcxAvropDBBandedTableView1.Controller.SelectedRecords[i].RecordIndex ;
    RecID:= grdcxAvropDBBandedTableView1.DataController.GetRecordId(RecIdx) ;
...
{    ClientNo      := ADataSet.FieldByName('ClientNo').AsInteger ;
    ...
    if not mtCompSelRows.Locate('SalesRegionNo;LONo;ClientNo;CurrencyNo', VarArrayOf([
    ...
    Begin
     ...
     mtCompSelRowsSalesRegionNo.AsInteger := ADataSet.FieldByName('SalesRegionNo').AsInteger ;
     mtCompSelRowsLONo.AsInteger          := ADataSet.FieldByName('ShippingPlanNo').AsInteger ;
     mtCompSelRowsClientNo.AsInteger      := ADataSet.FieldByName('ClientNo').AsInteger ;
     mtCompSelRowsCurrencyNo.AsInteger    := ADataSet.FieldByName('CurrencyNo').AsInteger ;
     mtCompSelRows.Post ;
    End ;
   End ;//For I...
  ...
  Begin
   ...
   if CurrencyNo    <> mtCompSelRows.FieldByName('CurrencyNo').AsInteger then
    CurrencyDiff:= True ;
   if SalesRegionNo <> mtCompSelRows.FieldByName('SalesRegionNo').AsInteger then
 ...
 begin
  Try
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + '3After sq_GetInvoiceDetailData.Open') ;
   While not sq_GetInvoiceDetailData.Eof do
   Begin
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + '3cdsInvoiceDetailNominalWidthINCH.AsString=' + cdsInvoiceDetailNominalWidthINCH.AsString) ;

    NoOfUnits      := NoOfUnits + sq_GetInvoiceDetailData.FieldByName(FieldName).AsFloat ;

   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After TempLineMet_ACT') ;
...
//   TotalAM3PerLO  := GetTotalAM3PerLO ;
   ...
   sq_GetFreightCost.Open ;
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After add freight cost sq_GetFreightCost.Open') ;
...
Begin
 ...
 Begin
  S:= sq_GetInvTextsDocText.AsVariant ;
...
Begin
 ...
 begin
  ...
  Begin
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before cdsInvoiceHead.Insert') ;
   cdsInvoiceHead.Insert ;
   cdsInvoiceHeadIncludeFreightCostInPrice.AsInteger             := sq_GetInvoiceHeadDataFreightIncluded.AsInteger ;
   cdsInvoiceHeadIntraStatCountryNo.AsInteger                    := sq_GetInvoiceHeadDataIntraStatCountryNo.AsInteger ;
   cdsInvoiceHeadDestinationText.AsString                        := sq_GetInvoiceHeadDataDestinationText.AsString ;
   cdsInvoiceHeadResponsibleSeller.AsInteger                     := sq_GetInvoiceHeadDataResponsibleSeller.AsInteger ;
   ...
   cdsInvoiceHeadSupplierNo.AsInteger                            := daMoLM1.cdsAvropSALESREGIONNO.AsInteger ;//VIDA_WOOD_CLIENTNO ; //Vida Wood mars 16 2006
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'before cdsInvoiceHeadInvoiceText.AsVariant') ;

    GetInvoiceDocTextByOrderNoLocal (sq_GetInvoiceHeadDataOrderNo.AsInteger) ;

   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'after cdsInvoiceHeadInvoiceText.AsVariant') ;
   ...
   cdsInvoiceHeadSpecialMoms.AsInteger                           := sq_GetInvoiceHeadDataSpecialMoms.AsInteger ;
   cdsInvoiceHeadInvoiced.AsInteger                              := 0 ;

   cdsInvoiceHeadBLDueDate.AsInteger                             := sq_GetInvoiceHeadDataSetDueDateBlank.AsInteger ;
   cdsInvoiceHeadFreeDelMonth.AsInteger                          := sq_GetInvoiceHeadDataFreeDelMonth.AsInteger ;

   cdsInvoiceHeadCountryNo.AsInteger                             := sq_GetInvoiceHeadDataBillingCountryNo.AsInteger ;
   cdsInvoiceHeadSalesGroupNo.AsInteger                          := sq_GetInvoiceHeadDataSalesGroupNo.AsInteger ;

   cdsInvoiceHeadDueDate.AsDateTime                              :=
   GetDueDate(Date, sq_GetInvoiceHeadDataNoOfDaysDiscount2.AsInteger,
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before cdsInvoiceHead.Post') ;
   cdsInvoiceHead.Post ;
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceHead.Post') ;
   ...
   End ; 
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceHead.ApplyUpdates(-1)') ;
...
Begin
 ...
 Begin
  Try
  sq_GetLoadID.ParamByName('SupplierNo').AsInteger      := mtCompSelRowsSALESREGIONNO.AsInteger ;//VIDA_WOOD_CLIENTNO ;
  sq_GetLoadID.ParamByName('ShippingPlanNo').AsInteger  := mtCompSelRowsLONo.AsInteger ;
  sq_GetLoadID.ParamByName('CustomerNo').AsInteger      := mtCompSelRowsCLIENTNO.AsInteger ; // Avrop customerNo
  sq_GetLoadID.Open ;
...
//   ShowMessage(E.Message);
  ...
  cdsBooking.ParamByName('ShippingPlanNo').AsInteger:= mtCompSelRowsLONo.AsInteger ; //Juli 6 2006 daMoLM1.cdsAvropShippingPlanNo.AsInteger ;
  cdsBooking.Active:= True ;
  Try

  if (cdsBooking.RecordCount > 0) and (sq_GetLoadID.Active)
  and (sq_GetLoadID.RecordCount > 0)
  and (Length(sq_GetLoadIDLoadID.AsString) > 0)
  and (not sq_GetLoadIDLoadID.IsNull) then
  Begin
   cdsBooking.Edit ;
   ...
   End ;
    if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'cdsBooking.ApplyUpdates(0)') ;
...
Begin
 ...
 begin
  Try
   ...
   sq_GetLOData.Close ;
   sq_GetLOData.ParamByName('ShippingPlanNo').AsInteger := mtCompSelRowsLONo.AsInteger ;
   sq_GetLOData.ParamByName('CustomerNo').AsInteger     := cdsInvoiceHeadCustomerNo.AsInteger ;
   sq_GetLOData.Open ;
   if ThisUser.UserID = 8 then
...
//Insert Invoice ShippingPlanNo
   ...
   Begin
    cdsInvoiceLO.Insert ;
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceLO.Insert') ;

    cdsInvoiceLOInternalInvoiceNo.AsInteger              := cdsInvoiceHeadInternalInvoiceNo.AsInteger ;
    cdsInvoiceLOShippingPlanNo.AsInteger                 := mtCompSelRowsLONo.AsInteger ;// StrToInt(LO_String[x]) ;
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before cdsInvoiceLO.Post') ;
    cdsInvoiceLO.Post ;
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceLO.Post') ;
   End ;
...
//   GetTotalAM3ForLO ;
   ...
   Begin
    ...
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After add freight cdsInvoiceDetail.Insert') ;
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceDetail.Insert') ;
...
//Fraktkostnaden inkluderad i pris om DeliveryTerms säger så
    ...
    Begin
     cdsInvoiceDetailTypeOfRow.AsInteger                := 2 ; //Freigth cost not inkluded in pris
   ...
   sq_GetFreightCost.Close ;
    ...
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before add freight cost cdsInvoiceDetail.Post') ;

    if cdsInvoiceHeadIncludeFreightCostInPrice.AsInteger  = 1 then
    cdsInvoiceDetailProductValue.AsFloat  := cdsInvoiceDetailProductValue.AsFloat * -1
    else
    cdsInvoiceDetailProductValue.AsFloat  := cdsInvoiceDetailProductValue.AsFloat ;
...
//    cdsInvoiceDetailInclInInvoiceTotal.AsInteger        := 0 ;
    ...
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After add freight cost cdsInvoiceDetail.Post') ;
    y:= Succ(y) ;
   End ;
...
//    sq_GetPkgType_Invoice.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
    sq_GetPkgType_Invoice.ParamByName('SupplierNo').AsInteger        := mtCompSelRowsSALESREGIONNO.AsInteger ;//VIDA_WOOD_CLIENTNO ;
    sq_GetPkgType_Invoice.ParamByName('ShippingPlanNo').AsInteger    := mtCompSelRowsLONo.AsInteger ;
    sq_GetPkgType_Invoice.ParamByName('CustomerNo').AsInteger        := mtCompSelRowsCLIENTNO.AsInteger ; // Avrop customerNo
    Try
    ...
    if cdsInvoiceDetailPriceUnit.AsString = 'm3 nDxaL' then
     Begin
      TempVal := sq_GetPkgType_InvoiceNdAl.AsFloat ;
     End
     else
    if cdsInvoiceDetailPriceUnit.AsString = 'Kvm aB' then
     Begin
      TempVal := sq_GetPkgType_InvoiceAM2AW.AsFloat ;
     End
     else
    if cdsInvoiceDetailPriceUnit.AsString = 'Lopm a' then
     Begin
      TempVal := sq_GetPkgType_InvoiceAM1.AsFloat ;
     End
     else
    if cdsInvoiceDetailPriceUnit.AsString = 'Lopm n' then
     Begin
 ...
 begin
  Try
   ...
   Begin
    cdsInvoiceDetail.Insert ;
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After add cost cdsInvoiceDetail.Insert ') ;
    cdsInvoiceDetailShippingPlanNo.AsInteger            := cdsInvoiceLOShippingPlanNo.AsInteger ;
   ...
   if ThisUser.UserID = 8 then
    ...
    cdsInvoiceDetail.Post ;
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After add cost cdsInvoiceDetail.Post ') ;
    y:= Succ(y) ;
   End ;
   sq_GetLOData.Close ;
...
//   Temp_AM1   := 0 ;
   ...
   sq_GetInvoiceDetailData.Open ;
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After sq_GetInvoiceDetailData.Open') ;
   While not sq_GetInvoiceDetailData.Eof do
   Begin
    cdsInvoiceDetail.Insert ;
   if ThisUser.UserID = 8 then
    ...
    cdsInvoiceDetailArticleNo.AsInteger                 := sq_GetInvoiceDetailDataArticleNo.AsInteger ;
   ...
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After ThicknessINCH=' + ThicknessINCH) ;
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After WidthINCH=' + WidthINCH) ;
...
//   ShowMessage(E.Message);
   ...
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'cdsInvoiceDetailNominalThicknessINCH.AsString=' + cdsInvoiceDetailNominalThicknessINCH.AsString) ;
   Try
...
//   ShowMessage(E.Message);
   ...
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'cdsInvoiceDetailNominalWidthINCH.AsString=' + cdsInvoiceDetailNominalWidthINCH.AsString) ;
...
//    Temp_AM1      := RoundTo(sq_GetInvoiceDetailDataACTUAL_LINEALMETER.AsFloat , -3) ;
   ...
   if ThisUser.UserID = 8 then
    ...
    cdsInvoiceDetailActualNetM3.AsFloat                 := RoundTo(sq_GetInvoiceDetailDataM3ACTUAL.AsFloat , -3) ;//Temp_AM3 ; //sq_GetInvoiceDetailDataM3ACTUAL.AsFloat ;
    cdsInvoiceDetailLinealMeter.AsFloat                 := RoundTo(sq_GetInvoiceDetailDataACTUAL_LINEALMETER.AsFloat , -3) ;//Temp_AM1 ; //sq_GetInvoiceDetailDataACTUAL_LINEALMETER.AsFloat ;
    cdsInvoiceDetailNominalM3.AsFloat                   := RoundTo(sq_GetInvoiceDetailDataM3NOMINAL.AsFloat , -3) ;//Temp_NM3 ; //sq_GetInvoiceDetailDataM3NOMINAL.AsFloat ;
    ...
    Temp_VOLUME_ORDERUNIT                               := RoundTo(sq_GetInvoiceDetailDataVOLUME_ORDERUNIT.AsFloat, -3) ;
    cdsInvoiceDetailVolume_OrderUnit.AsFloat            := Temp_VOLUME_ORDERUNIT ;

   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceDetailVolume_OrderUnit.AsFloat') ;


    cdsInvoiceDetailProductValue.AsFloat          := RoundTo(Temp_VOLUME_ORDERUNIT * sq_GetInvoiceDetailDataPRICE.AsFloat, -2) ;

    if cdsInvoiceHeadMoms.AsInteger = 1 then
    cdsInvoiceDetailVatValue.AsFloat              := cdsInvoiceDetailProductValue.AsFloat * c_Moms
    else
...
//Hämta netto och bruttopris
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before cdsInvoiceDetail.Post ') ;
    cdsInvoiceDetail.Post ;
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceDetail.Post ') ;
    sq_GetInvoiceDetailData.Next ;
    y:= Succ(y) ;
   End ; //While not sq_GetInvoiceDetailData.Eof do
   sq_GetInvoiceDetailData.Close ;
...
// DeleteTdmVidaInvoice:= False ;
 ...
 Screen.Cursor := crSQLWait;    { Show hourglass cursor }
 if ThisUser.UserID = 8 then
  mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'if not Assigned(dmVidaInvoice) then') ;
 ...
 Begin
  Try
  InternalInvoiceNo := dmsConnector.NextMaxNo('InvoiceHeader') ;//Get Next InternalInvoiceNo

  if ThisUser.UserID = 8 then
  mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'InternalInvoiceNo='+inttostr(InternalInvoiceNo)) ;
  mtCompSelRows.First ;
  While not mtCompSelRows.Eof do
  Begin
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before open sq_AnyLoadsToInvoice.') ;
   //Check if there is some loads to invoice for these parameters
    Try
    sq_AnyLoadsToInvoice.Active := False ;
    sq_AnyLoadsToInvoice.ParamByName('SupplierNo').AsInteger      := mtCompSelRowsSALESREGIONNO.AsInteger ;//VIDA_WOOD_CLIENTNO ;
    sq_AnyLoadsToInvoice.ParamByName('ShippingPlanNo').AsInteger  := mtCompSelRowsLONo.AsInteger ;
    sq_AnyLoadsToInvoice.ParamByName('CustomerNo').AsInteger      := mtCompSelRowsCLIENTNO.AsInteger ; // Avrop customerNo
  ...
  //   ShowMessage(E.Message);
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After open sq_AnyLoadsToInvoice.') ;
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After open sq_PkgType_Invoice.') ;
    Try
    sq_PkgType_Invoice.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
    sq_PkgType_Invoice.ParamByName('SupplierNo').AsInteger        := mtCompSelRowsSALESREGIONNO.AsInteger ;//VIDA_WOOD_CLIENTNO ;
    sq_PkgType_Invoice.ParamByName('ShippingPlanNo').AsInteger    := mtCompSelRowsLONo.AsInteger ;
    sq_PkgType_Invoice.ParamByName('CustomerNo').AsInteger        := mtCompSelRowsCLIENTNO.AsInteger ; // Avrop customerNo
  ...
  //   ShowMessage(E.Message);
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After open sq_PkgType_Invoice.') ;
    if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'sq_PkgType_Invoice.ExecSQL(False)') ;
   mtCompSelRows.Next ;
 ...
 //Start Transaction
  ...
  Begin
   ...
   sq_GetInvoiceHeadData.ParamByName('ShippingPlanNo').AsInteger:= mtCompSelRowsLONo.AsInteger ;// StrToInt(LO_String[x]) ;
   sq_GetInvoiceHeadData.ParamByName('CustomerNo').AsInteger:= mtCompSelRowsClientNo.AsInteger ;// daMoLM1.cdsAvropCLIENTNO.AsInteger ; // Avrop customerNo
...
//   ShowMessage(E.Message);
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'sq_GetInvoiceHeadData.Open') ;
   mtCompSelRows.Next ;
  End ;
...
//ADD FREIGHT COST IN TO  INVOICE DETAIL if Booking exist for current ShippingPlanNo Number
   ...
   Try
   sq_GetShipToAddress.ParamByName('ShippingPlanNo').AsInteger:= mtCompSelRowsLONo.AsInteger ;// StrToInt(LO_String[x]) ;
...
//   ShowMessage(E.Message);
   ...
   if ThisUser.UserID = 8 then
    mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After sq_GetShipToAddress.Open') ;
   ...
   Begin
    ...
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceShipTo.Post') ;
...
// the entry to Invoice_Load makes it impossible to invoice a load twice.
    ...
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before sq_Invoiced_Load.ExecSQL(False)') ;
   Try
   sq_Invoiced_Load.ParamByName('InternalInvoiceNo').AsInteger  := cdsInvoiceHeadInternalInvoiceNo.AsInteger ;
   sq_Invoiced_Load.ParamByName('SupplierNo').AsInteger         := daMoLM1.cdsAvropSALESREGIONNO.AsInteger ;//VIDA_WOOD_CLIENTNO ;
...
//   ShowMessage(E.Message);
   ...
   End ;
    ...
    if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After sq_Invoiced_Load.ExecSQL(False)') ;
   mtCompSelRows.Next ;
  End ; //While not mtCompSelRows.eof
  ...
  sq_LONoInInvoice.Active  := False ;
   ...
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'Before Summarize(Sender)') ;
   Summarize(Sender) ;
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After Summarize(Sender)') ;
   ...
   End ; 
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After Applyupdates') ;
...
// LockWindowUpdate(0);
 Try
  ...
  ' AND ShippingPlanNo = '+ mtCompSelRowsLONo.AsString ;//  LO_String[0] ;
  ...
  While not mtCompSelRows.Eof do
  Begin
   frmInvoice.TabControl1.Tabs.Add(mtCompSelRowsLONo.AsString) ;// := mtCompSelRowsLONo.AsS LO_String ;
   mtCompSelRows.Next ;
  End ;
...
End ; //
 ...
 Finally
  if dmsSystem.DeleteAssigned('TfrmAvropTdmVidaInvoice', 'dmVidaInvoice') = True then
  Begin
   dmVidaInvoice.Free ;
   dmVidaInvoice := Nil ;
  End ;
...
//Calculate VAT if Country is SWEDEN and cbVAT (Skatteupplag) checked
 if (
 (Trim(cdsInvoiceHeadCountryName.AsString) = 'SWEDEN') and (cdsInvoiceHeadSupplierNo.AsInteger = VIDA_WOOD_COMPANY_NO)
 OR
 (Trim(cdsInvoiceHeadCountryName.AsString) = 'DENMARK') and (cdsInvoiceHeadSupplierNo.AsInteger = VIDA_DANMARK))

    OR (cdsInvoiceHeadSpecialMoms.AsInteger = 1) then
 Begin
  if (cdsInvoiceHeadVAT_OnInvoice.AsInteger = 0) or (cdsInvoiceHeadSpecialMoms.AsInteger = 1) then
   Begin
...
//  DetermineIfInvoiceIsWithMoms ;
  ...
  cdsInvoiceHeadSUM_FreigthCost.AsFloat:= 0 ;
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceHeadSUM_FreigthCost.AsFloat:= 0 ;') ;

   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'InternalInvoiceNo = '+cdsInvoiceHeadInternalInvoiceNo.AsString) ;

  cdsInvoiceDetail.Filtered:= False ;
  cdsInvoiceDetail.Filter:= 'InternalInvoiceNo = '+cdsInvoiceHeadInternalInvoiceNo.AsString ;
  cdsInvoiceDetail.Filtered:= True ;
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceDetail.Filtered:= True ;') ;
...
//  if cdsInvoiceDetailTypeOfRow.AsInteger <> 3 then
  ...
  (cdsInvoiceDetailInclInPrice.AsInteger = 0)) then
   ...
   if ThisUser.UserID = 8 then mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After if cdsInvoiceDetailTypeOfRow.AsInteger <> 3 then') ;
...
//  if cdsInvoiceDetailTypeOfRow.AsInteger = 3 then
 ...
 End ;
  if ThisUser.UserID = 8 then mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After While not cdsInvoiceDetail.Eof do') ;
  //Calculate
 cdsInvoiceHeadInv_Value_To_Be_Paid_2.AsFloat:=  cdsInvoiceHeadTot_Inv_Inc_Freight_Extras.AsFloat ;

  if ThisUser.UserID = 8 then  mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceHeadInv_Value_To_Be_Paid_2.AsFloat') ;
...
//Calculate invoice value with no freight
 ...
 cdsInvoiceHeadTot_Inv_Inc_Freight_Extras.AsFloat + cdsInvoiceHeadSUM_FreigthCost.AsFloat ;
  ...
  if ThisUser.UserID = 8 then mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After cdsInvoiceHeadTotal_Product_Value_No_Freight.AsFloat:=') ;
...
// Calculate Invoice value to be paid, Freight Included in discount calculation
  ...
  if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After if cdsInvoiceHeadFreightInDiscount.AsInteger  = 0 then') ;
...
// Calculate Invoice value to be paid, Freight included in Commission calculation
 ...
 End ;
   ...
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After  if cdsInvoiceHeadCommissionPaidByCustomer.AsInteger = 1 then') ;

 cdsInvoiceHeadCommission_and_Discount.AsFloat:= cdsInvoiceHeadCommission.AsFloat + cdsInvoiceHeadDiscount.AsFloat ;
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After  cdsInvoiceHeadCommission_and_Discount.AsFloat:=') ;
 cdsInvoiceHeadInv_Value_After_Deduct_DIS_COMM.AsFloat:= cdsInvoiceHeadTot_Inv_Inc_Freight_Extras.AsFloat - cdsInvoiceHeadCommission_and_Discount.AsFloat ;
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After  cdsInvoiceHeadInv_Value_After_Deduct_DIS_COMM.AsFloat:=') ;
...
//     (cdsInvoiceHeadTot_Inv_Inc_Freight_Extras.AsFloat * (cdsInvoiceHeadAgentCommission.AsFloat/100)) ;
 ...
 End ;
   ...
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After  if cdsInvoiceHeadCommissionPaidByCustomer.AsInteger = 0 then') ;
...
//Calculate VAT if Country is SWEDEN cbVAT  is checked
  ...
  End
   ...
   if ThisUser.UserID = 8 then
     mLog.Lines.Add(Datetimetostr(now) + ':  ' + 'After   if Trim(cdsInvoiceHeadCountryName.AsString) = SWEDEN then') ;
...
begin
 ...
 Begin
  SelectedLO  := 0 ;
  if (Value = 1) or (dmsSystem.DoesLOHaveNonInvoicedLoads(cdsAvropShippingPlanNo.AsInteger) = False) then
  Begin
...
(*
  ...
  *)
  ...
  End
    ...
    ShowMessage('Kan inte avsluta eller annulera en LO som har ofakturerade laster!');
 End ;//with
end;

procedure TfrmAvrop.eSearchLONrKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var lx, Status  : Integer ;
    S           : String ;
begin
...
//  Refresh_Avrop(Sender,  StrToIntDef(Trim(eSearchLONr.Text),0), '') ;
  ...
  sq_Get_CSH_Status.ParamByName('ShippingPlanNo').AsInteger:= StrToIntDef(Trim(eSearchLONr.Text),0) ;
...
begin
 ...
 Begin
  ...
  try
  FormBookingForm.CreateCo(grdcxAvropDBBandedTableView1.DataController.DataSet.FieldByName('ShippingPlanNo').AsInteger) ;
...
Var FormCRViewReport : TFormCRViewReport ;
begin
 if daMoLM1.cdsAvropShippingPlanNo.AsInteger < 1 then exit ;
 FormCRViewReport:= TFormCRViewReport.Create(Nil);
 Try
  if daMoLM1.cdsAvropORDERTYPE.AsInteger = 0 then
  ...
  if FormCRViewReport.ReportFound then
  Begin
   FormCRViewReport.report.ParameterFields.Item[1].AddCurrentValue(daMoLM1.cdsAvropShippingPlanNo.AsInteger);
...
// SQL.Add('dbo.LoadedCSDPkgs LP WHERE 	LP.CSDNO = csd.CustShipPlanDetailObjectNo),0)  AS LoadedPkgs') ;
 ...
 if ((Length(IntLONo) = 0) and (Length(IntErReferens) = 0)) and (cds_PropsVerkSupplierNo.AsInteger = 1) then
 Begin
 ...
 SQL.Add('	INNER JOIN dbo.Address	ADDR			ON	ADDR.ADDRESSNO	= SSAD.ADDRESSNO') ;
...
// Begin
 ...
 if ((Length(IntLONo) = 0) and (Length(IntErReferens) = 0)) and (cds_PropsVerkSupplierNo.AsInteger = 0) then
 SQL.Add('AND CSH.SHIPPINGPLANSTATUS = ' + cds_PropsStatus.AsString)
 else
  if Length(IntLONo) > 0 then
   AddLOFilter(IntLONo) //SQL.Add('AND CSH.ShippingPlanNo = ' + IntToStr(IntLONo))
    else
     if (Length(IntErReferens) > 0) then
...
// SQL.Add('CSH.SHIPPINGPLANSTATUS = '+IntToStr(bcStatus.ItemIndex)) ;
   ...
   if (cds_PropsSalesPersonNo.AsInteger > 0) and ((Length(IntLONo) = 0) and (Length(IntErReferens) = 0)) then
   SQL.Add('AND smg.SalesGroupNo = ' + cds_PropsSalesPersonNo.AsString) ;


// if (cbFilter.Checked = True) then
//  SQL.Add('AND OH.OrderType = '+IntToStr(rgOrderType.ItemIndex)) ;
 if ((Length(IntLONo) = 0) and (Length(IntErReferens) = 0)) and  (not cds_PropsOrderTypeNo.IsNull) then
 SQL.Add('AND OH.OrderType = ' + cds_PropsOrderTypeNo.AsString) ;

 if ((Length(IntLONo) = 0) and (Length(IntErReferens) = 0)) and (cds_PropsSalesRegionNo.AsInteger > 0) then
 SQL.Add('AND OH.SalesRegionNo = ' + cds_PropsSalesRegionNo.AsString) ;

 if ((Length(IntLONo) = 0) and (Length(IntErReferens) = 0)) and (cds_PropsMarketRegionNo.AsInteger > 0) then
 SQL.Add('AND C.MarketRegionNo = ' + cds_PropsMarketRegionNo.AsString) ;

 if ((Length(IntLONo) = 0) and (Length(IntErReferens) = 0)) and (cds_PropsClientNo.AsInteger > 0) then
...
begin
 ...
 Begin
  ...
  try
    try
      with TfLoadEntryCSD.CreateWithExistingLoad(
        Self,
...
begin
 ...
 Begin
  with TfLoadEntryCSD.CreateWithNewLoad(Self,
   ...
   -1{LoadNo},
   grdcxAvropDBBandedTableView1.DataController.DataSet.FieldByName('ShippingPlanNo').AsInteger) do try
    ShowModal;
   Application.ProcessMessages ;
  finally
   daMoLM1.RefreshAvropLoads;
...
begin
 Try
  dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name+'/'+grdcxAvrop.Name, grdcxAvropDBBandedTableView1) ;
  dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name+'/'+grdLoad.Name, grdLoadDBTableView1) ;
  dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name+'/'+grdLO.Name, grdLODBTableView1) ;
  dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name+'/'+grdAddLO.Name, grdAddLODBTableView1) ;
 Except
 End ;
...
begin
 ...
 FormCRViewReport:= TFormCRViewReport.Create(Nil);
 Try
  dmsSystem.sq_PkgType_InvoiceByCSD.ParamByName('LoadNo').AsInteger:= daMoLM1.cdsLoadsLOADNO.AsInteger ;
  ...
  FormCRViewReport.ShowModal ;
  End ;
  dmsSystem.sq_DelPkgType.ParamByName('LoadNo').AsInteger:= daMoLM1.cdsLoadsLOADNO.AsInteger ;
...
begin
 grdcxAvropDBBandedTableView1.OptionsView.GroupByBox:= not grdcxAvropDBBandedTableView1.OptionsView.GroupByBox ;
end;

procedure TfrmAvrop.acFitAllColumnsInAvropViewExecute(Sender: TObject);
begin
 grdcxAvropDBBandedTableView1.OptionsView.ColumnAutoWidth:= not grdcxAvropDBBandedTableView1.OptionsView.ColumnAutoWidth ;
end;
...
begin
 With daMoLM1 do
 Begin
  acSetLOStatusToCancel.Enabled:=  (cdsAvrop.Active) and (cdsAvrop.RecordCount > 0) ;
 End ;
end;
...
begin
 With daMoLM1 do
 Begin
  acSetLOStatusToAktiv.Enabled:=  (cdsAvrop.Active) and (cdsAvrop.RecordCount > 0) ;
 End ;
end;
...
begin
 With daMoLM1 do
 Begin
  acSetLOStatusToComplete.Enabled:=  (cdsAvrop.Active) and (cdsAvrop.RecordCount > 0) ;
 End ;
end;
...
begin
 With daMoLM1 do
 Begin
  acPrintTallyUSNote.Enabled:=  (cdsLoads.Active) and (cdsLoads.RecordCount > 0) ;
 End ;
end;
...
var
  ...
  InvNo   : integer;
begin
  AColumn := (Sender as TcxGridDBTableView).GetColumnByFieldName('INTERNAL_INVOICENO') ;
...
begin
 grdLoadDBTableView1.OptionsView.Footer:= not grdLoadDBTableView1.OptionsView.Footer ;
end;

procedure TfrmAvrop.acFitAllColumnsInLoadViewExecute(Sender: TObject);
begin
 grdLoadDBTableView1.OptionsView.ColumnAutoWidth:= not grdLoadDBTableView1.OptionsView.ColumnAutoWidth ; 
end;
...
begin
 With daMoLM1 do
 Begin
  acSetLoadStatusToPreliminary.Enabled:= (cdsLoads.Active) and (cdsLoadsLoadOK.AsInteger <> 1) and (cdsLoads.RecordCount > 0)
...
begin
 With daMoLM1 do
 Begin
  acSetLoadStatusToComplete.Enabled:= (cdsLoads.Active) and (cdsLoadsLoadOK.AsInteger <> 1) and (cdsLoads.RecordCount > 0)
  and (cdsLoadsINTERNAL_INVOICENO.AsInteger = -1) ;
 End ;//daMoLM1
end;
...
{ DeleteTdmVidaInvoice:= False ;
 ...
 Begin
  ...
  Try
   ...
   cdsInvoiceHead.ParamByName('InternalInvoiceNo').AsInteger:= cdsLoadsINTERNAL_INVOICENO.AsInteger ;
   cdsInvoiceHead.Open ;
   cdsInvoiceHead.Active:= True ;
   cdsInvoiceLO.Close ;
   cdsInvoiceLO.ParamByName('InternalInvoiceNo').AsInteger:= cdsLoadsINTERNAL_INVOICENO.AsInteger ;
   cdsInvoiceLO.Active:= True ;
   ...
   cds_IH_SpecLoad.Active:= False ;
   cds_IH_SpecLoad.ParamByName('InternalInvoiceNo').AsInteger:= cdsLoadsINTERNAL_INVOICENO.AsInteger ;
   cds_IH_SpecLoad.Active:= True ;
  ...
  cdsInvoiceDetail.Close ;
  cdsInvoiceDetail.ParamByName('InternalInvoiceNo').AsInteger:= cdsLoadsINTERNAL_INVOICENO.AsInteger ;
//  cdsInvoiceDetail.Active:= True ;
//  cdsInvoiceDetail.Active:= False ;
  cdsInvoiceDetail.Filter:= 'InternalInvoiceNo = '+cdsInvoiceHeadInternalInvoiceNo.AsString+
  ' AND ShippingPlanNo = '+frmInvoice.TabControl1.Tabs[0] ;
...
{  if (DeleteTdmVidaInvoice = False) and (cdsInvoiceList.Active) and (cdsInvoiceList.RecordCount > 0) then
 ...
 Finally
  ...
  if dmsSystem.DeleteAssigned('acOpenInvoiceExecute', 'dmVidaInvoice') = True then
  Begin
   dmVidaInvoice.Free ;
   dmVidaInvoice := Nil ;
  End ;
...
Var FormCRViewReport: TFormCRViewReport ;
begin
 if grdLODBTableView1.DataController.DataSource.DataSet.FieldByName('ShippingPlanNo').AsInteger < 1 then exit ;
 FormCRViewReport:= TFormCRViewReport.Create(Nil);
 Try
 FormCRViewReport.CreateCo('LASTORDER_NOTE_ver2.RPT') ;
  ...
  if FormCRViewReport.ReportFound then
  Begin
   FormCRViewReport.report.ParameterFields.Item[1].AddCurrentValue(grdLODBTableView1.DataController.DataSource.DataSet.FieldByName('ShippingPlanNo').AsInteger);
   FormCRViewReport.report.ParameterFields.Item[2].AddCurrentValue(grdLODBTableView1.DataController.DataSource.DataSet.FieldByName('SUPPLIERNO').AsInteger);
   FormCRViewReport.CRViewer91.ReportSource:= FormCRViewReport.Report ;
   FormCRViewReport.CRViewer91.ViewReport ;
   FormCRViewReport.ShowModal ;
  End ;
...
Var FormCRViewReport : TFormCRViewReport ;
begin
 if grdLODBTableView1.DataController.DataSource.DataSet.FieldByName('SUPPLIERNO').AsInteger < 1 then exit ;
...
begin
 grdLODBTableView1.OptionsView.ColumnAutoWidth:= not grdLODBTableView1.OptionsView.ColumnAutoWidth ;
end;

procedure TfrmAvrop.acFitAllColumnsInAddLOViewExecute(Sender: TObject);
begin
 grdAddLODBTableView1.OptionsView.ColumnAutoWidth:= not grdAddLODBTableView1.OptionsView.ColumnAutoWidth ;
end;

procedure TfrmAvrop.acPrintAddLODittVerkExecute(Sender: TObject);
Var FormCRViewReport : TFormCRViewReport ;
begin
 if grdAddLODBTableView1.DataController.DataSource.DataSet.FieldByName('ShippingPlanNo').AsInteger < 1 then exit ;
 ...
 FormCRViewReport.CreateCo('LASTORDER_VERK_NOTE_ver2.RPT') ;
  ...
  if FormCRViewReport.ReportFound then
  Begin
   FormCRViewReport.report.ParameterFields.Item[1].AddCurrentValue(grdAddLODBTableView1.DataController.DataSource.DataSet.FieldByName('ShippingPlanNo').AsInteger);
   FormCRViewReport.report.ParameterFields.Item[2].AddCurrentValue(grdAddLODBTableView1.DataController.DataSource.DataSet.FieldByName('SUPPLIERNO').AsInteger);
   FormCRViewReport.CRViewer91.ReportSource:= FormCRViewReport.Report ;
   FormCRViewReport.CRViewer91.ViewReport ;
  End ;
  FormCRViewReport.ShowModal ;
...
begin
 ...
 if grdAddLODBTableView1.DataController.DataSource.DataSet.FieldByName('SUPPLIERNO').AsInteger < 1 then exit ;
...
begin
 acOpenLoad.Enabled:= (grdLoadDBTableView1.DataController.DataSource.DataSet.Active) and (grdLoadDBTableView1.DataController.DataSource.DataSet.RecordCount > 0) ;
end;
...
begin
 With daMoLM1 do
 Begin
  acOpenInvoice.Enabled:= (cdsLoads.Active) and (cdsLoadsINTERNAL_INVOICENO.AsInteger <> -1) and (cdsLoads.RecordCount > 0) ;
 End ;//daMoLM1
end;
...
begin
 acNewLoad.Enabled:= (grdcxAvropDBBandedTableView1.DataController.DataSource.DataSet.Active)
 and (grdcxAvropDBBandedTableView1.DataController.DataSource.DataSet.RecordCount > 0) ;
end;

procedure TfrmAvrop.acGruppSummeringExecute(Sender: TObject);
begin
 grdcxAvropDBBandedTableView1.OptionsView.GroupFooters:= gfVisibleWhenExpanded ;
end;
...
begin
 acExportToExcel.Enabled:= (daMoLM1.cdsAvrop.Active) and (daMoLM1.cdsAvrop.RecordCount > 0) ;
end;
...
Var FormCRExportOneReport   : TFormCRExportOneReport ;
    ...
    ReportType              : Integer ;
begin
 MailToAddress:= dmsContact.GetEmailAddressForSpeditorByLO(daMoLM1.cdsAvropShippingPlanNo.AsInteger) ;
...
// if dmVidaInvoice.cdsInvoiceListINT_INVNO.AsInteger < 1 then exit ;
 ...
 Try
  ...
  FormCRExportOneReport.CreateCo(daMoLM1.cdsAvropClientNo.AsInteger, ReportType, A, ExcelDir + 'LONo '+daMoLM1.cdsAvropShippingPlanNo.AsString) ;
//  FormCRExportOneReport.CreateCo(dmVidaInvoice.cdsInvoiceListCustomerNo.AsInteger, cPkgSpec, A, ExcelDir + 'Specification '+dmVidaInvoice.cdsInvoiceListINVOICE_NO.AsString) ;
 ...
 End ;
 SetLength(Attach, 1);
 Attach[0]        := ExcelDir + 'LONo '+daMoLM1.cdsAvropShippingPlanNo.AsString+'.pdf' ;
// Attach[1]        := ExcelDir + 'Specification '+dmVidaInvoice.cdsInvoiceListINVOICE_NO.AsString+'.pdf' ;
 ...
 Try
  dm_SendMapiMail.SendMail('Transportorder. LOnr: '+daMoLM1.cdsAvropShippingPlanNo.AsString,
  'Transportorder bifogad. '
  +LF+''
  +'Transportorder attached. '
  +LF+''
  +LF+''
  +LF+'MVH/Best Regards, '
  +LF+''
...
Var FormCRExportOneReport   : TFormCRExportOneReport ;
    ...
    ReportType              : Integer ;
begin
 MailToAddress:= dmsContact.GetEmailAddress(daMoLM1.cdsAvropClientNo.AsInteger) ;
 if Length(MailToAddress) = 0 then
 Begin
  MailToAddress:= 'ange@adress.nu' ;
  ShowMessage('Emailadress saknas för klienten, ange adressen direkt i mailet(outlook)') ;
 ...
 Try
  ...
  if daMoLM1.cdsAvropORDERTYPE.AsInteger = 0 then
   ReportType := cFoljesedel
     ...
     ReportType := cFoljesedelInkop ;
  FormCRExportOneReport.CreateCo(daMoLM1.cdsAvropClientNo.AsInteger, ReportType, A, ExcelDir + 'FS '+daMoLM1.cdsLoadsLOADNO.AsString) ;
//  FormCRExportOneReport.CreateCo(dmVidaInvoice.cdsInvoiceListCustomerNo.AsInteger, cPkgSpec, A, ExcelDir + 'Specification '+dmVidaInvoice.cdsInvoiceListINVOICE_NO.AsString) ;
 if FormCRExportOneReport.ReportFound = False then Exit ;
 Finally
  FreeAndNil(FormCRExportOneReport) ;//.Free ;
...
// Attach[1]        := ExcelDir + 'Specification '+dmVidaInvoice.cdsInvoiceListINVOICE_NO.AsString+'.pdf' ;
 dm_SendMapiMail  := Tdm_SendMapiMail.Create(nil);
 Try
  dm_SendMapiMail.SendMail('Följesedel. FSnr: '+daMoLM1.cdsLoadsLOADNO.AsString,
  'Följesedel bifogad. '
  +LF+''
  +'Load tally attached. '
  +LF+''
...
// if dmVidaInvoice.cdsInvoiceListINT_INVNO.AsInteger < 1 then exit ;
 ...
 Try
  ...
  FormCRExportOneReport.CreateCo(daMoLM1.cdsAvropClientNo.AsInteger, ReportType, A, ExcelDir + 'LO '+daMoLM1.cdsAvropShippingPlanNo.AsString) ;
//  FormCRExportOneReport.CreateCo(dmVidaInvoice.cdsInvoiceListCustomerNo.AsInteger, cPkgSpec, A, ExcelDir + 'Specification '+dmVidaInvoice.cdsInvoiceListINVOICE_NO.AsString) ;
 if FormCRExportOneReport.ReportFound = False then Exit ;
 Finally
  FreeAndNil(FormCRExportOneReport) ;//.Free ;
 End ;
 SetLength(Attach, 1);
 Attach[0]        := ExcelDir + 'LO '+daMoLM1.cdsAvropShippingPlanNo.AsString+'.pdf' ;
// Attach[1]        := ExcelDir + 'Specification '+dmVidaInvoice.cdsInvoiceListINVOICE_NO.AsString+'.pdf' ;
 dm_SendMapiMail  := Tdm_SendMapiMail.Create(nil);
 Try
  dm_SendMapiMail.SendMail('Lastorder. LOnr: '+daMoLM1.cdsAvropShippingPlanNo.AsString,
  'Lastorder bifogad. '
  +LF+''
  +'Loadorder attached. '
  +LF+''
...
Var FormCRExportOneReport   : TFormCRExportOneReport ;
    ...
    ReportType              : Integer ;
begin
 MailToAddress:= 'lars.makiaho@falubo.se' ;
 if Length(MailToAddress) > 0 then
 Begin
// if dmVidaInvoice.cdsInvoiceListINT_INVNO.AsInteger < 1 then exit ;
 ...
 dm_SendMapiMail  := Tdm_SendMapiMail.Create(nil);
 Try
  dm_SendMapiMail.SendMail('Avrop errorlog, LONr: '+daMoLM1.cdsAvropShippingPlanNo.AsString,
  'MVH/Best Regards, '
  +LF+''
  +dmsContact.GetFirstAndLastName(ThisUser.UserID),
  dmsSystem.Get_Dir('MyEmailAddress'),
...
function TfrmAvrop.SelectAvropsNrAttSkapaSalesLoadMot(const POLONo : Integer) : Integer ;
...
//ToDo ! Gör en kontroll att avropen matchar med orderlineno!!
 ...
 With daMoLM1 do
 Begin
  NewLoadNo := dmsSystem.POLoadConfirmed(cdsLoadsLOADNO.AsInteger, Sales_LONo) ;
  if NewLoadNo = 0 then
  Begin
   Sales_LONo:= SelectAvropsNrAttSkapaSalesLoadMot(cdsAvropShippingPlanNo.AsInteger) ;
   if Sales_LONo > 0 then
   Begin
    NewLoadNo:= dmsSystem.CopyPOLoadToSalesLoadAndSetPackagesAsNotAvailable(cdsLoadsLOADNO.AsInteger, Sales_LONo, cdsLoadsLOADNO.AsInteger, 1) ;
    if NewLoadNo > 0 then
     ShowMessage('Inköpslasten kopierad till försäljningsavrop ' + inttostr(Sales_LONo) + ', lastnr ' + inttostr(NewLoadNo)) ;
   End ;
  End
   else
    ShowMessage('Inköpslasten är redan kopierad till försäljningsavrop ' + inttostr(Sales_LONo) + ', lastnr ' + inttostr(NewLoadNo)) ;
 End ;
end;
...
begin
 With daMoLM1 do
 Begin
  acCopyLoadToSales.Enabled:= (cdsAvrop.Active) and (cdsAvrop.RecordCount > 0)
  and (cdsAvropORDERTYPE.AsInteger = 1)
  and (cdsAvropTrading.AsInteger = 1)
  and (cdsLoads.Active) and (cdsLoads.RecordCount > 0) ;
 End ;
end;

procedure TfrmAvrop.eSearchErReferensKeyDown(Sender: TObject;
...
begin
 ...
 Begin
  ...
  Begin
  sq_Get_CSH_Status.Close ;
  sq_Get_CSH_Status.ParamByName('ShippingPlanNo').AsInteger:= cdsAvropShippingPlanNo.AsInteger ;
  sq_Get_CSH_Status.Open ;
  if sq_Get_CSH_Status.Eof = False then
  Status := sq_Get_CSH_StatusShippingPlanStatus.AsInteger ;
  sq_Get_CSH_Status.Close ;
...
{  if not Assigned(dmVidaInvoice) then
 ...
 if (not Assigned(dmVidaInvoice)) then
 dmVidaInvoice  := TdmVidaInvoice.Create(nil);
 dmsSystem.AssignDMToThisWork('acDeletePrelInvoiceExecute', 'dmVidaInvoice') ;
 ...
 if MessageDlg('Är du säker på att du vill ta bort fakturan?',
  ...
  Begin
    ...
    if not cdsInvoiceNumber.Locate('InternalInvoiceNo', cdsLoadsINTERNAL_INVOICENO.AsInteger, []) then
    Begin
     dmsConnector.StartTransaction ;
     Try
...
//     sq_SetLSInvoicedStatus.ExecSQL(false) ;
     sq_DeleteInvoice.ParamByName('InternalInvoiceNo').AsInteger:= cdsLoadsINTERNAL_INVOICENO.AsInteger ;
...
//  FreeAndNil(dmVidaInvoice) ;//.Free ;
  ...
  if dmsSystem.DeleteAssigned('acDeletePrelInvoiceExecute', 'dmVidaInvoice') = True then
  Begin
   dmVidaInvoice.Free ;
   dmVidaInvoice := Nil ;
  End ;
...
begin
 With daMoLM1 do
 Begin
  acDeletePrelInvoice.Enabled:= (cdsLoads.Active) and (cdsLoads.RecordCount > 0)
  and (cdsLoadsINTERNAL_INVOICENO.AsInteger > -1)
...
begin
 ...
 Try
  ...
  if daMoLM1.cdsAvropORDERTYPE.AsInteger = 0 then
   FormCRViewReport.CreateCo('TRP_AVROPSORDER_NOTE_dk.RPT')
    else
     ShowMessage('N/A');
//      FormCRViewReport.CreateCo('TRP_ORDER_INKOP_NOTE.RPT') ;
...
begin
 With daMoLM1 do
 Begin
  acEmailaTrpOrder.Enabled:=  (cdsAvrop.Active) and (cdsAvrop.RecordCount > 0) ;
 End ;
end;
...
begin
 With daMoLM1 do
 Begin
  acEmailaTrpOrderAvropDK.Enabled:=  (cdsAvrop.Active) and (cdsAvrop.RecordCount > 0) ;
 End ;
end;
...
procedure TfrmAvrop.teSearchInvoiceNoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
Var InternalInvoiceNo, Status : Integer ;
    LONo  : String ;
begin
 if Key <> VK_RETURN then
  Exit;
...
// cds_PropsNewItemRow.AsInteger      := 0 ;
 ...
 Begin
  dsrcAvrop.OnDataChange  := nil ;
  Try
  LONo:= IntToStr(GetLOByInvNo (StrToIntDef(Trim(teSearchInvoiceNo.Text),0), InternalInvoiceNo)) ;
  if Length(LONo) > 0 then
  Begin
   Refresh_Avrop(Sender, LONo, '') ;

   if not cdsLoads.Locate('INTERNAL_INVOICENO', InternalInvoiceNo, []) then ;
    ...
    Status := -1 ;
    sq_Get_CSH_Status.Close ;
    sq_Get_CSH_Status.ParamByName('ShippingPlanNo').AsInteger:= StrToInt(LONo) ;
    sq_Get_CSH_Status.Open ;
    if sq_Get_CSH_Status.Eof = False then
    Status := sq_Get_CSH_StatusShippingPlanStatus.AsInteger ;
    sq_Get_CSH_Status.Close ;
...
// cds_PropsNewItemRow.AsInteger      := 0 ;
 ...
 Begin
  dsrcAvrop.OnDataChange  := nil ;
  Try
  LONo  := IntToStr(GetLONoForLoadNo (StrToIntDef(Trim(teSearchLoadNo.Text),0))) ;
  if Length(LONo) > 0 then
  Begin
   Refresh_Avrop(Sender, LONo, '') ;

   if not cdsLoads.Locate('LoadNo', StrToIntDef(Trim(teSearchLoadNo.Text),0), []) then
   if not cdsLoads.Locate('FOLJESEDEL', StrToIntDef(Trim(teSearchLoadNo.Text),0), []) then ;
...
begin
 ...
 LoadNo := grdLoadDBTableView1.DataController.DataSet.FieldByName('LoadNo').AsInteger ;
 ...
 Try
  ...
  Begin
   if (StrToIntDef(Trim(fEntryField.eNoofpkgs.Text),0) > 0) then
    if  daMoLM1.ValidLO(StrToIntDef(Trim(fEntryField.eNoofpkgs.Text),0)) then
    Begin
     Try
     daMoLM1.CngLOonLoad(grdLoadDBTableView1.DataController.DataSet.FieldByName('LoadNo').AsInteger,
...
//     GetLO(StrToIntDef(Trim(fEntryField.eNoofpkgs.Text),0), Sender) ;
     ...
     ShowMessage('LOnr ändrat. Nu Måste du öppna och kontrollera LO-kopplingar i lasten!') ;
...
begin
 ...
 Try
 if (fExportLoadPurpose.ShowModal = mrOK) and (fExportLoadPurpose.rgPurpose.ItemIndex <> -1) then
 Begin
...
{ if not Assigned(dmVidaInvoice) then
  ...
  dmVidaInvoice.ExportTypSoftSet(grdLoadDBTableView1.DataController.DataSet.FieldByName('LoadNo').AsInteger,
  grdLoadDBTableView1.DataController.DataSet.FieldByName('LO').AsInteger, 4{RecordType}, 'LO', fExportLoadPurpose.rgPurpose.ItemIndex) ;
  except
   On E: Exception do
   Begin
    dmsSystem.FDoLog(E.Message) ;
...
//  if DeleteTdmVidaInvoice = True then
//   FreeAndNil(dmVidaInvoice) ;//.Free ;
  if dmsSystem.DeleteAssigned('acExpLoadExecute', 'dmVidaInvoice') = True then
  Begin
   dmVidaInvoice.Free ;
   dmVidaInvoice := Nil ;
  End ;
...
begin
 acExpLoad.Enabled:=
 (daMoLM1.cdsAvrop.Active)
 and (daMoLM1.cdsAvrop.RecordCount > 0)
 and (daMoLM1.cdsAvropORDERTYPE.AsInteger = 1)
 and ((daMoLM1.cdsAvropTrading.AsInteger = 0) or (daMoLM1.cdsAvropTrading.IsNull))
 and (grdLoadDBTableView1.DataController.DataSource.DataSet.Active)
 and (grdLoadDBTableView1.DataController.DataSource.DataSet.RecordCount > 0)
 and (daMoLM1.cdsLoadsINTERNAL_INVOICENO.AsInteger > 0) ;
end;
...
procedure TfrmAvrop.acNewDeliveryRequestExecute(Sender: TObject);
begin
 if (daMoLM1.cdsAvrop.Active) and (daMoLM1.cdsAvrop.RecordCount > 0) then
 Begin
  with TfLoadEntryCSDTemp.CreateWithNewLoad(Self,
   daMoLM1.cdsAvropCLIENTNO.AsInteger,
   daMoLM1.cdsAvropSALESREGIONNO.AsInteger,
   -1{LoadNo},
   grdcxAvropDBBandedTableView1.DataController.DataSet.FieldByName('ShippingPlanNo').AsInteger) do try
...
begin
 acNewDeliveryRequest.Enabled:= (grdcxAvropDBBandedTableView1.DataController.DataSource.DataSet.Active)
 and (grdcxAvropDBBandedTableView1.DataController.DataSource.DataSet.RecordCount > 0) ;
end;
