...
uses
  ...
  dxPScxCommon, dxPScxGrid6Lnk, FMTBcd, DBClient, Provider, SqlExpr,
  ...
  cxCalc, cxButtons, cxPC, cxSplitter, Grids, DBGrids ;
...
type
  TfrmFreightExternLoad = class(TForm)
    ...
    procedure grdLoadsDBBandedTableView1KeyDown(Sender: TObject;
  ...
  private
    ...
    procedure SetMemTableValues (const Supplier_InvoiceNo, Supplier_InvoiceDate, Note : String) ;
...
var frmFreightExternLoad: TfrmFreightExternLoad;
...
{  if (not cds_PropsAutoColWidth.IsNull)
  ...
  Begin
   ...
   Begin
    ...
    sp_AttestFreightList.ParamByName('@InvoiceDate').AsSQLTimeStamp  := cds_PropsStartPeriod.AsSQLTimeStamp ;//DateTimeToSQLTimeStamp(deStart.Date) ;
    sp_AttestFreightList.ParamByName('@ShippingCompanyNo').AsInteger := cds_PropsAgentNo.AsInteger ;
    if cds_PropsMarketRegionNo.AsInteger > 0 then
    sp_AttestFreightList.ParamByName('@MarketRegionNo').AsInteger    := cds_PropsMarketRegionNo.AsInteger
    else
    sp_AttestFreightList.ParamByName('@MarketRegionNo').Clear();
    sp_AttestFreightList.ParamByName('@Description').AsString        := icAttesteraMot.Properties.Items.Items[icAttesteraMot.ItemIndex].Description ;
  ...
  Begin
   ...
   Begin
    ...
    sp_AttestKommList.ParamByName('@InvoiceDate').AsSQLTimeStamp     := cds_PropsStartPeriod.AsSQLTimeStamp ;//DateTimeToSQLTimeStamp(deStart.Date) ;
    sp_AttestKommList.ParamByName('@AgentNo').AsInteger              := cds_PropsAgentNo.AsInteger ;
    if cds_PropsMarketRegionNo.AsInteger > 0 then
    sp_AttestKommList.ParamByName('@MarketRegionNo').AsInteger       := cds_PropsMarketRegionNo.AsInteger
    else
   ...
   Begin
    ...
    if InvoiceNo > 0 then
    sp_AttestKommListSingle.ParamByName('@InvoiceNo').AsInteger            := InvoiceNo
    else
...
begin
 frmFreightExternLoad := NIL ;
  ...
  if dmsSystem.DeleteAssigned('TfrmFreightExternLoad', 'dmVidaInvoice') = True then
  Begin
   dmVidaInvoice.Free ;
   dmVidaInvoice := Nil ;
  End ;
...
// if cds_PropsAgentNo.IsNull then
 ...
 Begin
  if cds_Props.State in [dsBrowse] then
   cds_Props.Edit ;
  cds_PropsAgentNo.AsInteger  := grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('ShippingCompanyNo').AsInteger ;
  cds_Props.Post ;
 End
 else
 ...
 Begin
  if cds_Props.State in [dsBrowse] then
   cds_Props.Edit ;
  cds_PropsAgentNo.AsInteger  := grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('AgentNo').AsInteger ;
  cds_Props.Post ;
 End ;
 Finally
  cds_PropsAgentNo.OnChange   := cds_PropsAgentNoChange ;
 ...
 Begin
  ...
  Try
   ...
   Begin
    ...
    mt_SelectedRowsInvoiceNo.AsInteger  :=  ADataSet.FieldByName('InternalInvoiceNo').AsInteger ;
    mt_SelectedRowsLONo.AsInteger       :=  ADataSet.FieldByName('ShippingPlanNo').AsInteger ;
...
// if cds_PropsAgentNo.IsNull then
 ...
 Begin
  ...
  if dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name + '/Frakt', grdLoadsDBBandedTableView1) = False then ;
 ...
 Begin
  ...
  if dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name + '/Komm', grdLoadsDBBandedTableView1) = False then ;
  grdLoadsDBBandedTableView1ShippingPlanNo.Visible  := False ;
 End ;
 if dmsSystem.LoadGridLayout(ThisUser.UserID, Self.Name, grd_Att_Ext_ServicesIIDBTableView1) = False then ;
End ;
...
Begin
 ...
 Begin
  ...
  if (cds_PropsStartPeriod.IsNull) then //or (cds_PropsAutoColWidth.IsNull) then
  Begin
   cds_Props.Edit ;
...
//    cds_PropsAutoColWidth.AsInteger := 3 ;
 ...
 Begin
  cds_Props.Insert ;
  cds_PropsStartPeriod.AsSQLTimeStamp := DateTimeToSQLTimeStamp(Now - 30) ; //StrToSQLTimeStamp('2010-04-01') ;
//  cds_PropsAutoColWidth.AsInteger     := 3 ;
  cds_Props.Post ;
 End ;
End ;
...
// if cds_PropsAgentNo.IsNull then
 ...
 Begin
  grdLoadsDBBandedTableView1.Bands.Items[1].Visible := False ;
  grdLoadsDBBandedTableView1.Bands.Items[2].Visible := True ;
  dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name + '/Frakt', grdLoadsDBBandedTableView1) ;
 End
 else
 Begin
  grdLoadsDBBandedTableView1.Bands.Items[1].Visible := True ;
  grdLoadsDBBandedTableView1.Bands.Items[2].Visible := False ;
  dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name + '/Komm', grdLoadsDBBandedTableView1) ;
 End ;

 dmsSystem.StoreGridLayout(ThisUser.UserID, Self.Name, grd_Att_Ext_ServicesIIDBTableView1) ;
end;

procedure TfrmFreightExternLoad.acAttestSelectedRowsExecute(
  Sender: TObject);
begin
...
//   if cds_PropsAgentNo.IsNull then
   if cds_PropsAutoColWidth.AsInteger >= c_Frakt then
   Result := Result + grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('Frakt').AsFloat
   else
   Result := Result + grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('Komm').AsFloat ;
...
//  if cds_PropsAgentNo.IsNull then
  if cds_PropsAutoColWidth.AsInteger >= c_Frakt then
   grdLoadsDBBandedTableView1.DataController.DataSet.Filter := 'ConfirmFrakt = 1'
//   cds_LF.Filter   := 'ConfirmFrakt = 1'
   else
   grdLoadsDBBandedTableView1.DataController.DataSet.Filter := 'ConfirmKomm = 1' ;
...
procedure TfrmFreightExternLoad.acAttestSelectedRowsUpdate(
  Sender: TObject);
begin
 acAttestSelectedRows.Enabled := grdLoadsDBBandedTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmFreightExternLoad.SetMemTableValues (const Supplier_InvoiceNo, Supplier_InvoiceDate, Note : String) ;
...
//   if cds_PropsAgentNo.IsNull then
   if cds_PropsAutoColWidth.AsInteger >= c_Frakt then
   Begin
    if grdLoadsDBBandedTableView1.DataController.DataSet.Locate('InternalInvoiceNo;ShippingPlanNo', VarArrayOf([mt_SelectedRowsInvoiceNo.AsInteger, mt_SelectedRowsLONo.AsInteger]), []) then
//    Locate('InternalInvoiceNo;ShippingPlanNo', VarArrayOf([mt_SelectedRowsInvoiceNo.AsInteger,
//    mt_SelectedRowsLONo.AsInteger]), []) then
    Begin
     if grdLoadsDBBandedTableView1.DataController.DataSet.State in [dsBrowse] then
      grdLoadsDBBandedTableView1.DataController.DataSet.Edit ;
     if grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('ConfirmFrakt').AsInteger = 0 then
      grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('ConfirmFrakt').AsInteger := 1
       else
        grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('ConfirmFrakt').AsInteger := 0 ;
      if grdLoadsDBBandedTableView1.DataController.DataSet.State in [dsEdit, dsInsert] then
      grdLoadsDBBandedTableView1.DataController.DataSet.Post ;
    End ;
   End
   else
...
//    mt_SelectedRowsLONo.AsInteger]), []) then
    Begin
     if grdLoadsDBBandedTableView1.DataController.DataSet.State in [dsBrowse] then
      grdLoadsDBBandedTableView1.DataController.DataSet.Edit ;
     if grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('ConfirmKomm').AsInteger = 0 then
      grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('ConfirmKomm').AsInteger := 1
       else
        grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('ConfirmKomm').AsInteger := 0 ;
      if grdLoadsDBBandedTableView1.DataController.DataSet.State in [dsEdit, dsInsert] then
      grdLoadsDBBandedTableView1.DataController.DataSet.Post ;
    End ;
   End ;
...
begin
 with dmModule1 do
 Begin
  if MessageDlg('Vill du ta bort attestnr ' + cds_Att_Ext_ServicesIISequensNo.AsString + '?',
  mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  Begin
...
begin
  with dmModule1 do
  Begin
   OpenInvoice(grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('InternalInvoiceNo').AsInteger, -1, -1) ;
  End ;
end;

procedure TfrmFreightExternLoad.OpenInvoice(const IntInvNo, IntInvNoToAttestAgainst, DelKredit  : Integer) ;
...
begin
 With dmModule1 do
 Begin
  acOpenInvoice.Enabled := (grdLoadsDBBandedTableView1.DataController.DataSet.Active) and (grdLoadsDBBandedTableView1.DataController.DataSet.RecordCount > 0) ;
 End ;
end;
...
// cdsInvoiceShipToAddress.DataSource:= dsrcInvoiceHead_II ;
 Try
  if grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('InvoiceType').AsInteger = 3 then
   frmAttestInvoice.LCustomer.Caption:= 'Leverantör:'
    else
     frmAttestInvoice.LCustomer.Caption:= 'Kund:' ;

  frmAttestInvoice.cbInvoiceType.ItemIndex  := grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('InvoiceType').AsInteger ;
//  frmAttestInvoice.cbInvoiceType.ReadOnly   := True ;
  ...
  OpenInvoiceHeaderForAttest(grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('InternalInvoiceNo').AsInteger, grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('InvoiceType').AsInteger) ;
  ...
  End
    ...
    ShowMessage('Kan inte fokusera internal fakturanr ' + grdLoadsDBBandedTableView1.DataController.DataSet.FieldByName('InternalInvoiceNo').AsString) ;
...
procedure TfrmFreightExternLoad.cds_PropsAutoColWidthChange(
  Sender: TField);
Var PropsAutoColWidth : Integer ;
begin
 cds_PropsAutoColWidth.OnChange  := nil ;
...
procedure TfrmFreightExternLoad.acGotoLOSearchFieldExecute(
  Sender: TObject);
begin
 nSearchLONr.SetFocus ;
end;
...
begin
  with dmModule1 do
  Begin
   acSaveChanges.Enabled  := (cds_Att_Ext_ServicesII.Active)
   and ((cds_Att_Ext_ServicesII.State in [dsEdit])
   or (cds_Att_Ext_ServicesII.ChangeCount > 0)) ;
  End ; //with
end;
